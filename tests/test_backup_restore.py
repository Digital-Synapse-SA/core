...
from smarthub import backup_restore
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            None,
            '{"path": "test", "password": "psw", "remove_after_restore": false, "restore_database": false, "restore_smarthub": true}',
        ...
        (
            None,
            '{"path": "test", "password": null, "remove_after_restore": true, "restore_database": true, "restore_smarthub": false}',
            backup_restore.RestoreBackupFileContent(
                backup_file_path=Path("test"),
                password=None,
...
def test_restoring_backup_that_does_not_exist() -> None:
    ...
    with (
        mock.patch(
            "smarthub.backup_restore.restore_backup_file_content",
...
def test_aborting_for_older_versions() -> None:
    ...
    def _patched_path_read_text(path: Path, **kwargs):
        return '{"smarthub": {"version": "9999.99.99"}, "compressed": false}'
    ...
    with (
        ...
        mock.patch("smarthub.backup_restore.TemporaryDirectory"),
        mock.patch("pathlib.Path.read_text", _patched_path_read_text),
        mock.patch("smarthub.backup_restore.HA_VERSION", "2013.09.17"),
        pytest.raises(
            ValueError,
            match="You need at least SmartHub version 9999.99.99 to restore this backup",
        ),
    ):
        assert backup_restore.restore_backup(config_dir) is True
    assert restore_result_file_content() == {
        "error": (
            "You need at least SmartHub version 9999.99.99 to restore this backup"
        ),
        "error_type": "ValueError",
        "success": False,
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ("smart-hub_v2.db", "smart-hub_v2.db-wal"),
            (),
            ("smart-hub_v2.db", "smart-hub_v2.db-wal"),
            (),
        ),
...
def test_removal_of_current_configuration_when_restoring(
    ...
    mock_config_dir = [
        ...
        {"path": Path(config_dir, "smart-hub_v2.db"), "is_file": True},
        {"path": Path(config_dir, "smart-hub_v2.db-wal"), "is_file": True},
    ...
    def _patched_path_read_text(path: Path, **kwargs):
        return '{"smarthub": {"version": "2013.09.17"}, "compressed": false}'
    ...
    with (
        ...
        mock.patch("securetar.SecureTarFile"),
        mock.patch("smarthub.backup_restore.TemporaryDirectory") as temp_dir_mock,
        mock.patch("smarthub.backup_restore.HA_VERSION", "2013.09.17"),
        mock.patch("pathlib.Path.read_text", _patched_path_read_text),
...
def test_remove_backup_file_after_restore(
    ...
    with (
        ...
        mock.patch("smarthub.backup_restore._extract_backup"),
