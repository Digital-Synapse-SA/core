...
from smarthub.components.heos import DOMAIN
...
def new_heos_mock_fixture(controller: MockHeos) -> Iterator[Mock]:
    ...
    with (
        patch("smarthub.components.heos.coordinator.Heos", new=new_mock),
        patch("smarthub.components.heos.config_flow.Heos", new=new_mock),
    ):
        yield new_mock
