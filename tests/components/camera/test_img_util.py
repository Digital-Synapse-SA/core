...
from smarthub.components.camera.img_util import (
    TurboJPEGSingleton,
...
def test_scale_jpeg_camera_image() -> None:
    ...
    with patch(
        "smarthub.components.camera.img_util.TurboJPEG", return_value=False
    ):
        TurboJPEGSingleton()
    ...
    with patch(
        "smarthub.components.camera.img_util.TurboJPEG", return_value=turbo_jpeg
    ):
        TurboJPEGSingleton()
        assert scale_jpeg_camera_image(camera_image, 16, 12) == camera_image.content
...
def test_turbojpeg_load_failure() -> None:
    ...
    _clear_turbojpeg_singleton()
    with patch(
        "smarthub.components.camera.img_util.TurboJPEG", side_effect=Exception
    ):
        TurboJPEGSingleton()
