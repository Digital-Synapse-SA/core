...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.touchline_sl.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_touchlinesl_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.touchline_sl.TouchlineSL",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.touchline_sl.config_flow.TouchlineSL",
            new=mock_client,
        ),
    ):
