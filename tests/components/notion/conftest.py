...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.notion.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def config_entry_fixture(hass: SmartHub, config):
...
async def mock_aionotion_fixture(client):
    ...
    with (
        patch(
            "smarthub.components.notion.async_get_client_with_credentials",
            AsyncMock(return_value=client),
        ),
        patch(
            "smarthub.components.notion.async_get_client_with_refresh_token",
            AsyncMock(return_value=client),
        ),
        patch(
...
async def setup_config_entry_fixture(hass: SmartHub, config_entry, mock_aionotion):
