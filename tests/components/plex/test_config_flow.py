...
from smarthub.components.plex import config_flow
...
async def test_bad_credentials(hass: SmartHub) -> None:
...
@pytest.mark.usefixtures("current_request_with_host")
async def test_bad_hostname(hass: SmartHub, mock_plex_calls) -> None:
...
async def test_option_flow(hass: SmartHub, entry, mock_plex_server) -> None:
...
@pytest.mark.usefixtures("current_request_with_host")
async def test_external_timed_out(hass: SmartHub) -> None:
...
@pytest.mark.usefixtures("current_request_with_host")
async def test_manual_config(hass: SmartHub, mock_plex_calls) -> None:
    ...
    with (
        patch("smarthub.components.plex.PlexWebsocket", autospec=True),
        patch("smarthub.components.plex.GDM", return_value=MockGDM(disabled=True)),
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], user_input=MANUAL_SERVER
        )
...
async def test_integration_discovery(hass: SmartHub) -> None:
    ...
    with patch("smarthub.components.plex.config_flow.GDM", return_value=mock_gdm):
...
async def test_client_request_missing(hass: SmartHub) -> None:
...
@pytest.mark.usefixtures("current_request_with_host")
async def test_client_header_issues(hass: SmartHub) -> None:
    """Test when client headers are not set properly."""
    ...
    with (
        ...
        patch("plexauth.PlexAuth.token", return_value=None),
        patch(
            "smarthub.helpers.http.current_request.get",
