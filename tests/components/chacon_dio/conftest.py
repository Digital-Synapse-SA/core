...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.chacon_dio.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_dio_chacon_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.chacon_dio.DIOChaconAPIClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.chacon_dio.config_flow.DIOChaconAPIClient",
            new=mock_client,
        ),
    ):
