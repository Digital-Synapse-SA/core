...
async def test_full_user_flow(hass: SmartHub) -> None:
    ...
    with (
        patch(
            "smarthub.components.p1_monitor.config_flow.P1Monitor.smartmeter"
        ) as mock_p1monitor,
        patch(
            "smarthub.components.p1_monitor.async_setup_entry", return_value=True
        ) as mock_setup_entry,
    ):
...
async def test_api_error(hass: SmartHub) -> None:
    """Test we handle cannot connect error."""
    with patch(
        "smarthub.components.p1_monitor.coordinator.P1Monitor.smartmeter",
