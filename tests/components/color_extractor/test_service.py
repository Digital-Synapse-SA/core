...
from smarthub.components.color_extractor import (
    ATTR_PATH,
    ATTR_URL,
    DOMAIN,
    SERVICE_TURN_ON,
...
@pytest.fixture(autouse=True)
async def setup_light(hass: SmartHub):
...
async def test_missing_url_and_path(hass: SmartHub, setup_integration) -> None:
...
async def _async_execute_service(hass: SmartHub, service_data: dict[str, Any]):
    # Validate pre service call
    state = hass.states.get(LIGHT_ENTITY)
    assert state
    assert state.state == STATE_OFF
...
async def test_url_success(
    hass: SmartHub, aioclient_mock: AiohttpClientMocker, setup_integration
) -> None:
...
async def test_file(hass: SmartHub, setup_integration) -> None:
    ...
    with patch("smarthub.components.color_extractor._get_file", _get_file_mock):
...
@patch("os.path.isfile", Mock(return_value=True))
@patch("os.access", Mock(return_value=True))
async def test_file_denied_dir(hass: SmartHub, setup_integration) -> None:
