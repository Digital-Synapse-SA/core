...
from smarthub.components.weheat.const import DOMAIN
...
def mock_setup_entry():
    ...
    with patch(
        "smarthub.components.weheat.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_weheat_discover(mock_heat_pump_info) -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.weheat.HeatPumpDiscovery.async_discover_active",
            autospec=True,
        ) as mock_discover,
    ):
...
def mock_weheat_heat_pump(mock_weheat_heat_pump_instance) -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.weheat.coordinator.HeatPump",
        ) as mock_heat_pump,
    ):
