...
from smarthub.components.thermobeacon.const import DOMAIN
...
async def test_async_step_bluetooth_valid_device(hass: SmartHub) -> None:
    ...
    with patch(
        "smarthub.components.thermobeacon.async_setup_entry", return_value=True
    ):
...
async def test_async_step_bluetooth_not_thermobeacon(hass: SmartHub) -> None:
...
async def test_async_step_user_with_found_devices(hass: SmartHub) -> None:
    ...
    with patch(
        "smarthub.components.thermobeacon.config_flow.async_discovered_service_info",
