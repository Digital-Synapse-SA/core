...
from smarthub.components.tailscale.const import CONF_TAILNET, DOMAIN
...
def mock_config_entry() -> MockConfigEntry:
    ...
    return MockConfigEntry(
        title="smarthub.github",
        domain=DOMAIN,
        data={CONF_TAILNET: "smarthub.github", CONF_API_KEY: "tskey-MOCK"},
        unique_id="smarthub.github",
    )
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.tailscale.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_tailscale_config_flow() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.tailscale.config_flow.Tailscale", autospec=True
    ) as tailscale_mock:
...
def mock_tailscale(request: pytest.FixtureRequest) -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.tailscale.coordinator.Tailscale", autospec=True
    ) as tailscale_mock:
        tailscale = tailscale_mock.return_value
...
async def init_integration(
    hass: SmartHub, mock_config_entry: MockConfigEntry, mock_tailscale: MagicMock
) -> MockConfigEntry:
