...
async def test_setup_connect_exception(
    hass: SmartHub, mock_desk_api: MagicMock, exception: Exception
) -> None:
...
async def test_no_ble_device(hass: SmartHub, mock_desk_api: MagicMock) -> None:
    """Test setup with no BLEDevice from address."""
    with mock.patch(
        "smarthub.components.idasen_desk.bluetooth.async_ble_device_from_address",
        return_value=None,
    ):
        entry = await init_integration(hass)
        assert len(hass.config_entries.async_entries(DOMAIN)) == 1
...
async def test_reconnect_on_bluetooth_callback(
    ...
    with mock.patch(
        "smarthub.components.idasen_desk.bluetooth.async_register_callback"
...
async def test_unload_entry(hass: SmartHub, mock_desk_api: MagicMock) -> None:
