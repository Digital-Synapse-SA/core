...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.imgw_pib.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_imgw_pib_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.imgw_pib.ImgwPib", autospec=True
        ) as mock_client,
        patch(
            "smarthub.components.imgw_pib.config_flow.ImgwPib",
            new=mock_client,
        ),
    ):
