...
from smarthub.components.azure_devops.const import DOMAIN
...
async def mock_devops_client() -> AsyncGenerator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.azure_devops.coordinator.DevOpsClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.azure_devops.config_flow.DevOpsClient",
            new=mock_client,
        ),
    ):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.azure_devops.async_setup_entry",
        return_value=True,
    ) as mock_entry:
        yield mock_entry
