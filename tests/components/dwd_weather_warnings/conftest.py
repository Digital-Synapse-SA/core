...
from smarthub.const import CONF_MONITORED_CONDITIONS, CONF_NAME
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.dwd_weather_warnings.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_dwdwfsapi() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.dwd_weather_warnings.coordinator.DwdWeatherWarningsAPI",
            autospec=True,
        ) as mock_api,
        patch(
            "smarthub.components.dwd_weather_warnings.config_flow.DwdWeatherWarningsAPI",
            new=mock_api,
        ),
    ):
