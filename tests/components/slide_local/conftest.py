...
from smarthub.const import CONF_API_VERSION, CONF_HOST, CONF_MAC
...
def mock_slide_api() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.slide_local.coordinator.SlideLocalApi",
            autospec=True,
        ) as mock_slide_local_api,
        patch(
            "smarthub.components.slide_local.config_flow.SlideLocalApi",
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.slide_local.async_setup_entry", return_value=True
    ) as mock_setup_entry:
