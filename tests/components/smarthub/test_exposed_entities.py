"""Test SmartHub exposed entities helper."""
...
from smarthub.const import CLOUD_NEVER_EXPOSED_ENTITIES, EntityCategory
...
def entities_no_unique_id(hass: SmartHub) -> dict[str, str]:
...
async def test_expose_new_entities(
    ...
    await ws_client.send_json_auto_id(
        {
            "type": "smarthub/expose_new_entities/get",
    ...
    await ws_client.send_json_auto_id(
        {
            "type": "smarthub/expose_new_entities/set",
            "assistant": "cloud.alexa",
...
async def test_list_exposed_entities(
    ...
    await ws_client.send_json_auto_id({"type": "smarthub/expose_entity/list"})
    response = await ws_client.receive_json()
    assert response["success"]
