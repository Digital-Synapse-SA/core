...
from smarthub import config, core as ha
...
from smarthub.helpers import entity, entity_registry as er, issue_registry as ir
...
async def test_turn_on_without_entities(hass: SmartHub) -> None:
...
@patch("smarthub.config.os.path.isfile", Mock(return_value=True))
async def test_reload_core_conf(hass: SmartHub) -> None:
...
@patch("smarthub.config.os.path.isfile", Mock(return_value=True))
@patch("smarthub.components.smarthub._LOGGER.error")
@patch("smarthub.core_config.async_process_ha_core_config")
async def test_reload_core_with_wrong_conf(
    mock_process, mock_error, hass: SmartHub
) -> None:
    """Test reload core conf service."""
...
@patch("smarthub.core.SmartHub.async_stop", return_value=None)
@patch(
    "smarthub.config.async_check_ha_config_file",
    side_effect=SmartHubError("Test error"),
)
async def test_restart_smarthub_wrong_conf(
    mock_check, mock_restart, hass: SmartHub
) -> None:
    """Test restart service with error."""
    await async_setup_component(hass, ha.DOMAIN, {})
    with pytest.raises(SmartHubError, match="Test error"):
        await hass.services.async_call(
...
@patch("smarthub.core.SmartHub.async_stop", return_value=None)
@patch("smarthub.config.async_check_ha_config_file", return_value=None)
async def test_check_config(mock_check, mock_stop, hass: SmartHub) -> None:
    """Test stop service."""
    await async_setup_component(hass, ha.DOMAIN, {})
...
async def test_turn_on_skips_domains_without_service(
    ...
    service = hass.services.async_services_for_domain("smarthub")["turn_on"]
    ...
    assert (
        "The service smarthub.turn_on does not support entities binary_sensor.blub, sensor.bla"
        in caplog.text
    )


async def test_entity_update(hass: SmartHub) -> None:
    """Test being able to call entity update."""
    await async_setup_component(hass, "smarthub", {})

    with patch(
        "smarthub.components.smarthub.async_update_entity",
        return_value=None,
    ) as mock_update:
        await hass.services.async_call(
            "smarthub",
...
async def test_setting_location(hass: SmartHub) -> None:
...
async def test_not_allowing_recursion(
    ...
    for service in SERVICE_TURN_ON, SERVICE_TURN_OFF, SERVICE_TOGGLE:
        await hass.services.async_call(
            ha.DOMAIN,
            service,
            {"entity_id": "smarthub.light"},
            blocking=True,
        )
        assert (
            f"Called service smarthub.{service} with invalid entities smarthub.light"
            in caplog.text
        ), service
...
async def test_reload_config_entry_by_entry_id(hass: SmartHub) -> None:
...
async def test_raises_when_db_upgrade_in_progress(
    hass: SmartHub, service, caplog: pytest.LogCaptureFixture
) -> None:
    ...
    with (
        ...
        patch(
            "smarthub.helpers.recorder.async_migration_in_progress",
            return_value=True,
...
async def test_raises_when_config_is_invalid(
    ...
    with (
        ...
        patch(
            "smarthub.config.async_check_ha_config_file", return_value=["Error 1"]
...
async def test_restart_smarthub(
    hass: SmartHub, service_data: dict, safe_mode_enabled: bool
) -> None:
    ...
    with (
        ...
        patch("smarthub.config.async_enable_safe_mode") as mock_safe_mode,
        patch(
            "smarthub.core.SmartHub.async_stop", return_value=None
        ) as mock_restart,
    ):
        await hass.services.async_call(
            "smarthub",
...
async def test_stop_smarthub(hass: SmartHub) -> None:
...
async def test_save_persistent_states(hass: SmartHub) -> None:
    ...
    await async_setup_component(hass, "smarthub", {})
    with patch(
        "smarthub.helpers.restore_state.RestoreStateData.async_save_persistent_states",
        return_value=None,
    ) as mock_save:
        await hass.services.async_call(
            "smarthub",
...
async def test_reload_custom_templates(hass: SmartHub) -> None:
    """Test we can call reload_custom_templates."""
    await async_setup_component(hass, "smarthub", {})
    with patch(
        "smarthub.components.smarthub.async_load_custom_templates",
        return_value=None,
...
async def test_reload_all(
    ...
    core_config = async_mock_service(hass, "smarthub", "reload_core_config")
    themes = async_mock_service(hass, "frontend", "reload_themes")
    jinja = async_mock_service(hass, "smarthub", "reload_custom_templates")
...
async def test_deprecated_installation_issue_32bit_core(
    ...
    with (
        ...
        patch(
            "smarthub.components.smarthub._is_32_bit",
            return_value=True,
        ),
    ):
...
async def test_deprecated_installation_issue_32bit(
    ...
    with (
        patch(
            "smarthub.components.smarthub.async_get_system_info",
            return_value={
                "installation_type": "SmartHub Container",
                "arch": arch,
            },
        ),
        patch(
        ...
        patch(
            "smarthub.components.smarthub._get_arch",
            return_value=arch,
        ),
    ):
        assert await async_setup_component(hass, DOMAIN, {})
