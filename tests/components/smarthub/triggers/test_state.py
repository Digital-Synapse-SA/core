...
def setup_comp(hass: SmartHub) -> None:
...
async def test_if_action(hass: SmartHub, service_calls: list[ServiceCall]) -> None:
    """Test for to action."""
...
async def test_if_fails_setup_if_to_boolean_value(hass: SmartHub) -> None:
        ...
        assert await async_setup_component(
            ...
            automation.DOMAIN,
            {
                automation.DOMAIN: {
                    ...
                    "action": {"service": "smarthub.turn_on"},
                }
            },
        )
...
async def test_if_fails_setup_if_from_boolean_value(hass: SmartHub) -> None:
...
async def test_if_fails_setup_bad_for(hass: SmartHub) -> None:
...
async def test_if_fires_on_for_condition(
    ...
    with patch("smarthub.core.dt_util.utcnow") as mock_utcnow:
...
async def test_if_fails_setup_for_without_time(hass: SmartHub) -> None:
...
async def test_if_fails_setup_for_without_entity(hass: SmartHub) -> None:
...
async def test_invalid_for_template_1(hass: SmartHub) -> None:
