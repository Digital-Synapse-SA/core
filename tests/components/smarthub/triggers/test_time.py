...
from smarthub.components.smarthub.triggers import time
...
async def test_if_action_before(
    ...
    with patch("smarthub.helpers.condition.dt_util.now", return_value=before_10):
        hass.bus.async_fire("test_event")
        await hass.async_block_till_done()

    assert len(service_calls) == 1

    with patch("smarthub.helpers.condition.dt_util.now", return_value=after_10):
        hass.bus.async_fire("test_event")
        await hass.async_block_till_done()
...
async def test_if_action_one_weekday(
    ...
    with patch("smarthub.helpers.condition.dt_util.now", return_value=monday):
        hass.bus.async_fire("test_event")
        await hass.async_block_till_done()

    assert len(service_calls) == 1

    with patch("smarthub.helpers.condition.dt_util.now", return_value=tuesday):
        hass.bus.async_fire("test_event")
        await hass.async_block_till_done()
...
async def test_if_action_list_weekday(
    ...
    with patch("smarthub.helpers.condition.dt_util.now", return_value=wednesday):
        hass.bus.async_fire("test_event")
        await hass.async_block_till_done()
...
async def test_untrack_time_change(hass: SmartHub) -> None:
    """Test for removing tracked time changes."""
    mock_track_time_change = Mock()
    with patch(
        "smarthub.components.smarthub.triggers.time.async_track_time_change",
