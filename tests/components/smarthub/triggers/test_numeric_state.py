...
from smarthub.components.smarthub.triggers import (
...
async def test_if_not_fires_on_entity_removal(
    hass: SmartHub, service_calls: list[ServiceCall], below: int | str
) -> None:
...
async def test_if_not_fires_on_initial_entity_above(
    hass: SmartHub, service_calls: list[ServiceCall], above: int | str
) -> None:
    """Test the firing when starting with a match."""
    hass.states.async_set("test.entity", 11)
    await hass.async_block_till_done()
...
async def test_template_string(
    hass: SmartHub, service_calls: list[ServiceCall], below: float | str
) -> None:
...
async def test_if_fails_setup_bad_for(
    hass: SmartHub, above: int | str, below: int | str
) -> None:
...
async def test_if_fails_setup_for_without_above_below(hass: SmartHub) -> None:
...
async def test_below_above(hass: SmartHub) -> None:
...
async def test_schema_unacceptable_entities(hass: SmartHub) -> None:
...
async def test_template_variable(
    hass: SmartHub, service_calls: list[ServiceCall], multiplier: int
) -> None:
