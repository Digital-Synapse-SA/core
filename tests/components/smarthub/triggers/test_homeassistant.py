    ...
    [
        {
            automation.DOMAIN: {
                "alias": "hello",
                "trigger": {"platform": "smarthub", "event": "start"},
                "action": {
...
async def test_if_fires_on_hass_start(
    hass: SmartHub, hass_config: ConfigType
) -> None:
    """Test the firing when SmartHub starts."""
...
async def test_if_fires_on_hass_shutdown(hass: SmartHub) -> None:
    """Test the firing when SmartHub shuts down."""
    calls = async_mock_service(hass, "test", "automation")
    hass.set_state(CoreState.not_running)
    ...
    assert await async_setup_component(
        ...
        automation.DOMAIN,
        {
            automation.DOMAIN: {
                "alias": "hello",
                "trigger": {"platform": "smarthub", "event": "shutdown"},
                "action": {
                    "service": "test.automation",
                    "data_template": {"id": "{{ trigger.id}}"},
                },
