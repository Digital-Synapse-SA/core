...
from smarthub.components.luftdaten.const import CONF_SENSOR_ID, DOMAIN
...
def mock_setup_entry() -> Generator[None]:
    ...
    with patch(
        "smarthub.components.luftdaten.async_setup_entry", return_value=True
    ):
        yield
...
def mock_luftdaten() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.luftdaten.Luftdaten", autospec=True
        ) as luftdaten_mock,
        patch(
            "smarthub.components.luftdaten.config_flow.Luftdaten",
            new=luftdaten_mock,
        ),
    ):
...
async def init_integration(
    hass: SmartHub, mock_config_entry: MockConfigEntry, mock_luftdaten: MagicMock
) -> MockConfigEntry:
