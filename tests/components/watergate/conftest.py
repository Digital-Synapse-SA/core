...
def mock_watergate_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.watergate.WatergateLocalApiClient",
            autospec=True,
        ) as mock_client_main,
        patch(
            "smarthub.components.watergate.config_flow.WatergateLocalApiClient",
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.watergate.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_webhook_id_generation() -> Generator[None]:
    ...
    with patch(
        "smarthub.components.watergate.config_flow.webhook_generate_id",
