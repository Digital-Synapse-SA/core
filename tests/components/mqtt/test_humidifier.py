...
from smarthub.components import humidifier, mqtt
...
from smarthub.components.mqtt.const import CONF_CURRENT_HUMIDITY_TOPIC
from smarthub.components.mqtt.humidifier import (
...
async def async_set_mode(
    hass: SmartHub, entity_id: str = ENTITY_MATCH_ALL, mode: str | None = None
) -> None:
...
async def async_set_humidity(
    hass: SmartHub, entity_id: str = ENTITY_MATCH_ALL, humidity: int | None = None
) -> None:
...
async def test_discovery_update_unchanged_humidifier(
    ...
    with patch(
        "smarthub.components.mqtt.fan.MqttFan.discovery_update"
    ) as discovery_update:
