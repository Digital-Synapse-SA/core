...
from smarthub.components import binary_sensor, mqtt
...
async def expires_helper(hass: SmartHub) -> None:
...
async def test_expiration_on_discovery_and_discovery_update_of_binary_sensor(
    ...
    freezer.move_to(now)
    async_fire_time_changed(hass, now)
    async_fire_mqtt_message(hass, "smarthub/binary_sensor/bla/config", config_msg)
    await hass.async_block_till_done()
    ...
    with patch(("smarthub.helpers.event.dt_util.utcnow"), return_value=now):
        async_fire_time_changed(hass, now)
        async_fire_mqtt_message(
            hass, "smarthub/binary_sensor/bla/config", config_msg
        )
        await hass.async_block_till_done()
...
async def test_discovery_update_unchanged_binary_sensor(
    ...
    with patch(
        "smarthub.components.mqtt.binary_sensor.MqttBinarySensor.discovery_update"
    ) as discovery_update:
