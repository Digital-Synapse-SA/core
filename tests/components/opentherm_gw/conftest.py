...
from smarthub.const import CONF_DEVICE, CONF_ID, CONF_NAME
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.opentherm_gw.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_pyotgw() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.opentherm_gw.OpenThermGateway",
        ...
        patch(
            "smarthub.components.opentherm_gw.config_flow.pyotgw.OpenThermGateway",
            new=mock_gateway,
        ),
    ):
        yield mock_gateway
