...
def patch_get_device(pyegps_device_mock: MagicMock) -> Generator[MagicMock]:
    ...
    with (
        patch("smarthub.components.energenie_power_sockets.get_device") as m1,
        patch(
            "smarthub.components.energenie_power_sockets.config_flow.get_device",
            new=m1,
        ) as mock,
    ):
...
def patch_search_devices(
    ...
    with patch(
        "smarthub.components.energenie_power_sockets.config_flow.search_for_devices",
