...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.enphase_envoy.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def config_entry_fixture(
    hass: SmartHub, config: dict[str, str]
) -> MockConfigEntry:
...
async def mock_envoy(
    ...
    with (
        patch(
            "smarthub.components.enphase_envoy.config_flow.Envoy",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.enphase_envoy.Envoy",
            new=mock_client,
        ),
        patch(
