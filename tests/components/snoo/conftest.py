...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.snoo.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def bypass_api() -> Generator[AsyncMock]:
    """Bypass the Snoo api."""
    with (
        patch("smarthub.components.snoo.Snoo", autospec=True) as mock_client,
        patch("smarthub.components.snoo.config_flow.Snoo", new=mock_client),
    ):
