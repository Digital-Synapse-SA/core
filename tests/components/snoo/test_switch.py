...
async def test_switch(hass: SmartHub, bypass_api: AsyncMock) -> None:
...
async def test_update_failed(
    hass: SmartHub, bypass_api: AsyncMock, command: str, error_str: str
) -> None:
    ...
    with pytest.raises(SmartHubError, match=error_str):
        await hass.services.async_call(
            "switch",
            command,
            blocking=True,
