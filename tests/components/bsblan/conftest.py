...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.bsblan.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_bsblan() -> Generator[MagicMock]:
    ...
    with (
        patch("smarthub.components.bsblan.BSBLAN", autospec=True) as bsblan_mock,
        patch("smarthub.components.bsblan.config_flow.BSBLAN", new=bsblan_mock),
    ):
...
async def init_integration(
    hass: SmartHub, mock_config_entry: MockConfigEntry, mock_bsblan: MagicMock
) -> MockConfigEntry:
