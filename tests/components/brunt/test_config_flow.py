...
from smarthub.components.brunt.const import DOMAIN
...
async def test_form(hass: SmartHub, mock_setup_entry: AsyncMock) -> None:
    ...
    with patch(
        "smarthub.components.brunt.config_flow.BruntClientAsync.async_login",
        return_value=None,
    ):
...
async def test_form_duplicate_login(hass: SmartHub) -> None:
...
async def test_form_error(hass: SmartHub, side_effect, error_message) -> None:
...
async def test_reauth(
    hass: SmartHub, side_effect, result_type, password, step_id, reason
) -> None:
    """Test uniqueness of username."""
    entry = MockConfigEntry(
        domain=DOMAIN,
