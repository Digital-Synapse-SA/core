...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.simplefin.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_claim_setup_token() -> str:
    ...
    with patch(
        "smarthub.components.simplefin.config_flow.SimpleFin.claim_setup_token",
...
def mock_decode_claim_token_invalid_then_good() -> str:
    ...
    with patch(
        "smarthub.components.simplefin.config_flow.SimpleFin.decode_claim_token",
...
def mock_simplefin_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.simplefin.SimpleFin",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.simplefin.config_flow.SimpleFin",
            new=mock_client,
        ),
    ):
