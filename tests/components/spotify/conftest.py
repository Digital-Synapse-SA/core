...
from smarthub.components.spotify.const import DOMAIN, SPOTIFY_SCOPES
...
def mock_config_entry(expires_at: int) -> MockConfigEntry:
    """Create Spotify entry in SmartHub."""
...
async def patch_sleep() -> Generator[AsyncMock]:
    """Fixture to setup credentials."""
    with patch("smarthub.components.spotify.media_player.AFTER_REQUEST_SLEEP", 0):
        yield
...
def mock_spotify() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.spotify.SpotifyClient", autospec=True
        ) as spotify_mock,
        patch(
            "smarthub.components.spotify.config_flow.SpotifyClient",
            new=spotify_mock,
        ),
    ):
