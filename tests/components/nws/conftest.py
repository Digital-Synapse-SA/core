...
def mock_simple_nws():
    ...
    with (
        patch("smarthub.components.nws.SimpleNWS") as mock_nws,
        patch("smarthub.components.nws.coordinator.RETRY_STOP", 0),
        patch("smarthub.components.nws.coordinator.RETRY_INTERVAL", 0),
    ):
...
def mock_simple_nws_config():
    ...
    with patch("smarthub.components.nws.config_flow.SimpleNWS") as mock_nws:
        instance = mock_nws.return_value
        instance.set_station = AsyncMock(return_value=None)
        instance.station = "ABC"
        instance.stations = ["ABC"]
...
def no_sensor():
    """Remove sensors."""
    with patch(
        "smarthub.components.nws.sensor.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def no_weather():
    """Remove weather."""
    with patch(
        "smarthub.components.nws.weather.async_setup_entry", return_value=True
    ) as mock_setup_entry:
        yield mock_setup_entry
