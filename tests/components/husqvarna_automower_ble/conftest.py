...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.husqvarna_automower_ble.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_automower_client(enable_bluetooth: None) -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.husqvarna_automower_ble.Mower",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.husqvarna_automower_ble.config_flow.Mower",
            new=mock_client,
        ),
    ):
