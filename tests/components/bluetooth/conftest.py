...
def mock_operating_system_85():
    """Mock running SmartHub Operating system 8.5."""
    with (
        patch("smarthub.components.hassio.is_hassio", return_value=True),
        patch(
        ...
        patch("smarthub.components.hassio.get_info", return_value={}),
        patch("smarthub.components.hassio.get_host_info", return_value={}),
    ):
        yield
...
def mock_operating_system_90():
    """Mock running SmartHub Operating system 9.0."""
    with (
        patch("smarthub.components.hassio.is_hassio", return_value=True),
        patch(
            "smarthub.components.hassio.get_os_info",
...
def macos_adapter() -> Generator[None]:
    ...
    with (
        ...
        patch(
            "smarthub.components.bluetooth.platform.system",
            return_value="Darwin",
        ),
        patch(
...
def two_adapters_fixture() -> Generator[None]:
    ...
    with (
        patch(
            "smarthub.components.bluetooth.platform.system", return_value="Linux"
        ),
        patch(
            "habluetooth.scanner.platform.system",
            return_value="Linux",
...
def disable_new_discovery_flows_fixture():
    ...
    """
    with patch(
        "smarthub.components.bluetooth.manager.discovery_flow.async_create_flow"
    ) as mock_create_flow:
        yield mock_create_flow


@pytest.fixture
def register_hci0_scanner(hass: SmartHub) -> Generator[None]:
...
@pytest.fixture
def register_hci1_scanner(hass: SmartHub) -> Generator[None]:
...
@pytest.fixture
def register_non_connectable_scanner(hass: SmartHub) -> Generator[None]:
