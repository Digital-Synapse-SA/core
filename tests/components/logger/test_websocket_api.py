...
from smarthub.components.logger.helpers import DATA_LOGGER
...
async def test_integration_log_info(
    ...
    logging.getLogger("smarthub.components.http").setLevel(logging.DEBUG)
    logging.getLogger("smarthub.components.websocket_api").setLevel(logging.DEBUG)
...
async def test_integration_log_level(
    ...
    assert hass.data[DATA_LOGGER].overrides == {
        "smarthub.components.websocket_api": logging.DEBUG
    }
...
async def test_custom_integration_log_level(
    ...
    with (
        patch(
            "smarthub.components.logger.helpers.async_get_integration",
            return_value=integration,
        ),
        patch(
            "smarthub.components.logger.websocket_api.async_get_integration",
            return_value=integration,
        ),
    ):
        await websocket_client.send_json(
        ...
        assert hass.data[DATA_LOGGER].overrides == {
            "smarthub.components.hue": logging.DEBUG,
...
async def test_module_log_level(
    ...
    assert await async_setup_component(
        hass,
        "logger",
        {"logger": {"logs": {"smarthub.components.other_component": "warning"}}},
    )
    ...
    await websocket_client.send_json(
        {
            "id": 7,
            "type": "logger/log_level",
            "module": "smarthub.components.websocket_api",
            "level": "DEBUG",
            "persistence": "none",
        }
    )
    ...
    assert hass.data[DATA_LOGGER].overrides == {
        "smarthub.components.websocket_api": logging.DEBUG,
        "smarthub.components.other_component": logging.WARNING,
    }
...
async def test_module_log_level_override(
    ...
    assert await async_setup_component(
        hass,
        "logger",
        {"logger": {"logs": {"smarthub.components.websocket_api": "warning"}}},
    )

    assert hass.data[DATA_LOGGER].overrides == {
        "smarthub.components.websocket_api": logging.WARNING
    }
    ...
    assert hass.data[DATA_LOGGER].overrides == {
        "smarthub.components.websocket_api": logging.ERROR
    }
    ...
    assert hass.data[DATA_LOGGER].overrides == {
        "smarthub.components.websocket_api": logging.NOTSET
    }
