...
from smarthub.components.simplisafe.config_flow import CONF_AUTH_CODE
from smarthub.config_entries import SOURCE_USER
from smarthub.const import CONF_CODE, CONF_TOKEN, CONF_USERNAME
...
async def test_duplicate_error(
    config_entry, hass: SmartHub, setup_simplisafe
) -> None:
    """Test that errors are shown when duplicates are added."""
    with patch(
        "smarthub.components.simplisafe.async_setup_entry", return_value=True
    ):
...
async def test_invalid_auth_code_length(hass: SmartHub) -> None:
...
async def test_options_flow(config_entry, hass: SmartHub) -> None:
...
async def test_step_reauth(
    config_entry: MockConfigEntry, hass: SmartHub, setup_simplisafe
) -> None:
    ...
    with (
        ...
        patch("smarthub.config_entries.ConfigEntries.async_reload"),
    ):
        result = await hass.config_entries.flow.async_configure(
            result["flow_id"], user_input={CONF_AUTH_CODE: VALID_AUTH_CODE}
        )
...
async def test_unknown_error(hass: SmartHub, setup_simplisafe) -> None:
