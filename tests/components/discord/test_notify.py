...
from smarthub.components.discord.notify import DiscordNotificationService
...
async def test_send_message_without_target_logs_error(
    ...
    with caplog.at_level(
        logging.ERROR, logger="smarthub.components.discord.notify"
    ):
...
async def test_get_file_from_url_not_on_allowlist(
    ...
    with caplog.at_level(
        logging.WARNING, logger="smarthub.components.discord.notify"
    ):
        result = await discord_notification_service.async_get_file_from_url(
