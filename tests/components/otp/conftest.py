...
from smarthub.components.otp.const import DOMAIN
from smarthub.const import CONF_NAME, CONF_TOKEN
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.otp.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_pyotp() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.otp.config_flow.pyotp",
        ) as mock_client,
        patch("smarthub.components.otp.sensor.pyotp", new=mock_client),
    ):
        mock_totp = MagicMock()
