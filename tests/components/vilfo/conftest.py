...
from smarthub.components.vilfo import DOMAIN
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.vilfo.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_vilfo_client() -> Generator[AsyncMock]:
    """Mock a Vilfo client."""
    with patch(
        "smarthub.components.vilfo.config_flow.VilfoClient",
        autospec=True,
    ) as mock_client:
...
def mock_is_valid_host() -> Generator[AsyncMock]:
    """Mock is_valid_host."""
    with patch(
        "smarthub.components.vilfo.config_flow.is_host_valid",
        return_value=True,
