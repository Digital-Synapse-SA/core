...
from smarthub.components.zwave_js.scripts.convert_device_diagnostics_to_fixture import (
...
def test_main(capfd: pytest.CaptureFixture[str]) -> None:
    ...
    # Test dump to stdout
    args = [
        "smarthub/components/zwave_js/scripts/convert_device_diagnostics_to_fixture.py",
    ...
    with (
        ...
        patch(
            "smarthub.components.zwave_js.scripts.convert_device_diagnostics_to_fixture.Path.write_text"
        ) as write_text_mock,
    ):
        main()
