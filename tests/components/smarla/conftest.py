...
from smarthub.components.smarla.const import DOMAIN
...
def mock_setup_entry() -> Generator:
    ...
    with patch("smarthub.components.smarla.async_setup_entry", return_value=True):
        yield
...
def mock_connection() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.smarla.config_flow.Connection", autospec=True
        ) as mock_connection,
        patch(
            "smarthub.components.smarla.Connection",
            mock_connection,
        ),
    ):
...
def mock_federwiege(mock_connection: MagicMock) -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.smarla.Federwiege", autospec=True
    ) as mock_federwiege:
