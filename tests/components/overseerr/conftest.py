...
from smarthub.components.overseerr import CONF_CLOUDHOOK_URL
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.overseerr.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_overseerr_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.overseerr.coordinator.OverseerrClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.overseerr.config_flow.OverseerrClient",
            new=mock_client,
        ),
    ):
