...
from smarthub.components.fan import ATTR_PRESET_MODE, DOMAIN as FAN_DOMAIN
...
async def test_turn_on_off_success(
    ...
    with (
        ...
        with patch(
            "smarthub.components.vesync.fan.VeSyncFanHA.schedule_update_ha_state"
        ) as update_mock:
...
async def test_turn_on_off_raises_error(
    ...
    # returns False indicating failure in which case raises SmartHubError.
    with (
        patch(
            command,
            return_value=False,
...
async def test_set_preset_mode(
    ...
    # If VeSyncTowerFan.normal_mode fails (returns False), then SmartHubError is raised
    with (
        expectation,
        patch(
