...
from smarthub.components.ld2410_ble.const import DOMAIN
...
async def test_user_step_success(hass: SmartHub) -> None:
    ...
    with patch(
        "smarthub.components.ld2410_ble.config_flow.async_discovered_service_info",
    ...
    with (
        patch(
            "smarthub.components.ld2410_ble.config_flow.LD2410BLE.initialise",
        ),
        patch(
            "smarthub.components.ld2410_ble.async_setup_entry",
            return_value=True,
        ) as mock_setup_entry,
    ):
