...
from smarthub.components.harmony.const import ACTIVITY_POWER_OFF, DOMAIN
...
def mock_hc(harmony_client: FakeHarmonyClient) -> Generator[None]:
    ...
    with patch(
        "smarthub.components.harmony.data.HarmonyClient",
...
def mock_write_config():
    ...
    with patch(
        "smarthub.components.harmony.remote.HarmonyRemote.write_config_file",
    ) as mock:
        yield mock
