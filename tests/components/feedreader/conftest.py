...
from smarthub.components.feedreader.coordinator import EVENT_FEEDREADER
...
def fixture_feed_identically_timed_events(hass: SmartHub) -> bytes:
...
def fixture_feed_htmlentities(hass: SmartHub) -> bytes:
...
def fixture_feed_atom_htmlentities(hass: SmartHub) -> bytes:
    """Load test ATOM feed data with HTML Entities."""
    return load_fixture_bytes("feedreader10.xml", DOMAIN)


@pytest.fixture(name="events")
async def fixture_events(hass: SmartHub) -> list[Event]:
