...
from smarthub.const import CONF_HOST, CONF_MAC, CONF_PORT
...
@pytest.fixture
async def local_oauth_impl(hass: SmartHub):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.iotty.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_iotty() -> Generator[MagicMock]:
    """Mock IottyProxy."""
    with patch(
        "smarthub.components.iotty.api.IottyProxy", autospec=True
    ) as iotty_mock:
        yield iotty_mock
...
def mock_coordinator() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.iotty.IottyDataUpdateCoordinator",
        autospec=True,
    ) as coordinator_mock:
