...
from smarthub.const import ATTR_ENTITY_ID, UnitOfTime
...
async def test_ventilator_min_on_home_attributes(hass: SmartHub) -> None:
...
async def test_ventilator_min_on_away_attributes(hass: SmartHub) -> None:
...
async def test_set_min_time_home(hass: SmartHub) -> None:
    """Test the number can set min time home."""
    target_value = 40
    with patch(
        "smarthub.components.ecobee.Ecobee.set_ventilator_min_on_time_home"
...
async def test_set_min_time_away(hass: SmartHub) -> None:
    """Test the number can set min time away."""
    target_value = 0
    with patch(
        "smarthub.components.ecobee.Ecobee.set_ventilator_min_on_time_away"
...
async def test_compressor_protection_min_temp_attributes(hass: SmartHub) -> None:
...
async def test_set_compressor_protection_min_temp(hass: SmartHub) -> None:
    ...
    target_value = 0
    with patch(
        "smarthub.components.ecobee.Ecobee.set_aux_cutover_threshold"
