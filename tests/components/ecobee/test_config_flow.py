...
from smarthub.components.ecobee import config_flow
...
async def test_user_step_without_user_input(hass: SmartHub) -> None:
...
async def test_pin_request_succeeds(hass: SmartHub) -> None:
    """Test expected result if pin request succeeds."""
    flow = config_flow.EcobeeFlowHandler()
    flow.hass = hass

    with patch("smarthub.components.ecobee.config_flow.Ecobee") as mock_ecobee:
...
async def test_pin_request_fails(hass: SmartHub) -> None:
...
async def test_token_request_succeeds(hass: SmartHub) -> None:
...
async def test_token_request_fails(hass: SmartHub) -> None:
