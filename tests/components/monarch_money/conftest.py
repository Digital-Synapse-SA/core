...
from smarthub.components.monarch_money.const import DOMAIN
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.monarch_money.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_config_api() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.monarch_money.config_flow.TypedMonarchMoney",
            autospec=True,
        ) as mock_class,
        patch(
            "smarthub.components.monarch_money.TypedMonarchMoney", new=mock_class
        ),
    ):
