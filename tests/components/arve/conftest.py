...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.arve.async_setup_entry", return_value=True
    ) as mock_setup_entry:
        yield mock_setup_entry


@pytest.fixture
def mock_config_entry(hass: SmartHub, mock_arve: MagicMock) -> MockConfigEntry:
...
def mock_arve():
    ...
    with (
        patch(
            "smarthub.components.arve.coordinator.Arve", autospec=True
        ) as arve_mock,
        patch("smarthub.components.arve.config_flow.Arve", new=arve_mock),
    ):
