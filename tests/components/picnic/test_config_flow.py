...
from smarthub.const import CONF_ACCESS_TOKEN, CONF_COUNTRY_CODE
...
def picnic_api():
    ...
    with patch(
        "smarthub.components.picnic.config_flow.PicnicAPI",
    ) as picnic_mock:
...
async def test_form(hass: SmartHub, picnic_api) -> None:
    ...
    with patch(
        "smarthub.components.picnic.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
async def test_form_invalid_auth(hass: SmartHub) -> None:
    ...
    with patch(
        "smarthub.components.picnic.config_flow.PicnicHub.authenticate",
...
async def test_form_already_configured(hass: SmartHub, picnic_api) -> None:
...
async def test_step_reauth(hass: SmartHub, picnic_api) -> None:
...
async def test_step_reauth_failed(hass: SmartHub) -> None:
...
async def test_step_reauth_different_account(hass: SmartHub, picnic_api) -> None:
