...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.lg_thinq.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_uuid() -> Generator[AsyncMock]:
    ...
    with (
        ...
        patch(
            "smarthub.components.lg_thinq.config_flow.uuid.uuid4",
            new=mock_uuid,
        ),
    ):
...
def mock_config_thinq_api() -> Generator[AsyncMock]:
    """Mock a thinq api."""
    with (
        patch("smarthub.components.lg_thinq.ThinQApi", autospec=True) as mock_api,
        patch(
            "smarthub.components.lg_thinq.config_flow.ThinQApi",
            new=mock_api,
        ),
    ):
...
def mock_thinq_api(mock_thinq_mqtt_client: None) -> Generator[AsyncMock]:
    """Mock a thinq api."""
    with patch("smarthub.components.lg_thinq.ThinQApi", autospec=True) as mock_api:
        thinq_api = mock_api.return_value
...
def mock_thinq_mqtt_client() -> Generator[None]:
    ...
    with patch(
        "smarthub.components.lg_thinq.mqtt.ThinQMQTTClient",
        autospec=True,
        return_value=True,
    ):
        yield
