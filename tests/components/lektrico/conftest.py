...
def mock_device() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.lektrico.Device",
            autospec=True,
        ) as mock_device,
        patch(
            "smarthub.components.lektrico.config_flow.Device",
            new=mock_device,
        ),
        patch(
            "smarthub.components.lektrico.coordinator.Device",
            new=mock_device,
        ),
    ):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    """Mock setup entry."""
    with patch(
        "smarthub.components.lektrico.async_setup_entry", return_value=True
    ) as mock_setup_entry:
