...
def mock_get_server_info() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.music_assistant.config_flow.get_server_info"
...
async def music_assistant_client_fixture() -> AsyncGenerator[MagicMock]:
    ...
    with patch(
        "smarthub.components.music_assistant.MusicAssistantClient", autospec=True
    ) as client_class:
