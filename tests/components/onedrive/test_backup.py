...
from smarthub.components.onedrive.backup import (
    async_register_backup_agents_listener,
)
from smarthub.components.onedrive.const import DATA_BACKUP_AGENT_LISTENERS, DOMAIN
...
async def test_error_on_agents_download(
    ...
    with patch("smarthub.components.onedrive.backup.CACHE_TTL", -1):
        resp = await client.get(
            f"/api/backup/download/{backup_id}?agent_id={DOMAIN}.{mock_config_entry.unique_id}"
        )
