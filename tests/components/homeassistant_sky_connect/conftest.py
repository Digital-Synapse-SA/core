"""Test fixtures for the SmartHub SkyConnect integration."""
...
def mock_usb_serial_by_id_fixture() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.zha.config_flow.usb.get_serial_by_id"
...
def mock_zha():
    ...
    with (
        ...
        patch(
            "smarthub.components.zha.async_setup_entry",
            return_value=True,
        ),
    ):
        yield
...
def mock_zha_get_last_network_settings() -> Generator[None]:
    ...
    with patch(
        "smarthub.components.zha.api.async_get_last_network_settings",
