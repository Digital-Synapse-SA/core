...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.alexa_devices.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_amazon_devices_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.alexa_devices.coordinator.AmazonEchoApi",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.alexa_devices.config_flow.AmazonEchoApi",
            new=mock_client,
        ),
    ):
