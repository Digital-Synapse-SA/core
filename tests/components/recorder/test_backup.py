...
from smarthub.components.recorder.backup import async_post_backup, async_pre_backup
from smarthub.core import CoreState, SmartHub
from smarthub.exceptions import SmartHubError


async def test_async_pre_backup(recorder_mock: Recorder, hass: SmartHub) -> None:
    """Test pre backup."""
    with patch(
        "smarthub.components.recorder.core.Recorder.lock_database"
    ) as lock_mock:
...
RAISES_HASS_NOT_RUNNING = pytest.raises(
    SmartHubError, match="SmartHub is not running"
)
...
async def test_async_pre_backup_with_migration(
    ...
    with (
        ...
        patch(
            "smarthub.components.recorder.backup.async_migration_in_progress",
            return_value=True,
        ),
        pytest.raises(SmartHubError, match="Database migration in progress"),
    ):
        await async_pre_backup(hass)
    assert not lock_mock.called


async def test_async_post_backup(recorder_mock: Recorder, hass: SmartHub) -> None:
    """Test post backup."""
    with patch(
        "smarthub.components.recorder.core.Recorder.unlock_database"
    ) as unlock_mock:
...
async def test_async_post_backup_failure(
    ...
    with (
        ...
        pytest.raises(
            SmartHubError, match="Could not release database write lock"
        ),
    ):
        await async_post_backup(hass)
    assert unlock_mock.called
