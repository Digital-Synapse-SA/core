...
from smarthub.components.fail2ban.sensor import (
    STATE_ALL_BANS,
    STATE_CURRENT_BANS,
    BanLogParser,
    BanSensor,
...
async def test_setup(hass: SmartHub) -> None:
    ...
    mock_fh = mock_open()
    with patch("smarthub.components.fail2ban.sensor.open", mock_fh, create=True):
...
@patch("os.path.isfile", Mock(return_value=True))
async def test_multi_jails(hass: SmartHub) -> None:
...
async def test_single_ban(hass: SmartHub) -> None:
...
async def test_ipv6_ban(hass: SmartHub) -> None:
...
async def test_multiple_ban(hass: SmartHub) -> None:
...
async def test_unban_all(hass: SmartHub) -> None:
...
async def test_unban_one(hass: SmartHub) -> None:
...
async def test_multi_jail(hass: SmartHub) -> None:
...
async def test_ban_active_after_update(hass: SmartHub) -> None:
