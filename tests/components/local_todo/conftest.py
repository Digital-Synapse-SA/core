...
from smarthub.components.local_todo import LocalTodoListStore
from smarthub.components.local_todo.const import (
    CONF_STORAGE_KEY,
    CONF_TODO_LIST_NAME,
    DOMAIN,
)
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.local_todo.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_store(ics_content: str, store_read_side_effect: Any | None) -> Generator[None]:
    ...
    with patch("smarthub.components.local_todo.LocalTodoListStore", new=new_store):
        yield
