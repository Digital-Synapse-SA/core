...
from smarthub.components.govee_light_local.coordinator import GoveeController
...
def fixture_mock_govee_api() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.govee_light_local.coordinator.GoveeController",
            return_value=mock_api,
        ) as mock_controller,
        patch(
            "smarthub.components.govee_light_local.config_flow.GoveeController",
            return_value=mock_api,
        ),
    ):
...
def fixture_mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.govee_light_local.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
