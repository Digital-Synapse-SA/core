...
def mock_tado_api() -> Generator[MagicMock]:
    """Mock the Tado API."""
    with (
        patch("smarthub.components.tado.Tado") as mock_tado,
        patch("smarthub.components.tado.config_flow.Tado", new=mock_tado),
    ):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.tado.async_setup_entry", return_value=True
    ) as mock_setup_entry:
