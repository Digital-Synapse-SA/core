...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.intellifire.async_setup_entry", return_value=True
    ) as mock_setup:
        yield mock_setup
...
def mock_fireplace_finder_none() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.intellifire.config_flow.UDPFireplaceFinder.search_fireplace"
    ):
...
def mock_cloud_interface() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.intellifire.IntelliFireCloudInterface",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.intellifire.config_flow.IntelliFireCloudInterface",
            new=mock_client,
        ),
        patch(
...
def mock_local_interface() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.intellifire.config_flow.IntelliFireAPILocal",
        autospec=True,
    ) as mock_client:
        mock_client = mock_client.return_value
...
def mock_fp(mock_common_data_local) -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.intellifire.UnifiedFireplace"
