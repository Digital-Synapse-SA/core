...
def tomorrowio_config_entry_update_fixture():
    ...
    with (
        patch(
            "smarthub.components.tomorrowio.TomorrowioV4.realtime_and_all_forecasts",
            return_value=json.loads(load_fixture("v4.json", "tomorrowio")),
        ) as mock_update,
        patch(
            "smarthub.components.tomorrowio.TomorrowioV4.max_requests_per_day",
            new_callable=PropertyMock,
        ) as mock_max_requests_per_day,
        patch(
            "smarthub.components.tomorrowio.TomorrowioV4.num_api_requests",
            new_callable=PropertyMock,
