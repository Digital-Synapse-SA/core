...
from smarthub.components.srp_energy.const import DOMAIN, PHOENIX_TIME_ZONE
...
async def fixture_setup_hass_config(hass: SmartHub) -> None:
...
def fixture_hass_tz_info(hass: SmartHub, setup_hass_config) -> dt.tzinfo | None:
    """Return timezone info for the hass timezone."""
    return dt_util.get_time_zone(hass.config.time_zone)


@pytest.fixture(name="test_date")
def fixture_test_date(hass: SmartHub, hass_tz_info) -> dt.datetime | None:
...
def fixture_mock_srp_energy_config_flow() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.srp_energy.config_flow.SrpEnergyClient", autospec=True
    ) as srp_energy_mock:
        client = srp_energy_mock.return_value
        client.validate.return_value = True
        client.usage.return_value = MOCK_USAGE
