...
def pydeako_deako_mock() -> Generator[MagicMock]:
    ...
    with patch("smarthub.components.deako.Deako", autospec=True) as mock:
        yield mock
...
def pydeako_discoverer_mock(mock_async_zeroconf: MagicMock) -> Generator[MagicMock]:
    ...
    with (
        patch("smarthub.components.deako.DeakoDiscoverer", autospec=True) as mock,
        patch("smarthub.components.deako.config_flow.DeakoDiscoverer", new=mock),
    ):
        yield mock
...
def mock_deako_setup() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.deako.async_setup_entry",
        return_value=True,
    ) as mock_setup:
        yield mock_setup
