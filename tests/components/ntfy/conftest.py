...
from smarthub.components.ntfy.const import CONF_TOPIC, DOMAIN
from smarthub.config_entries import ConfigSubentryData
from smarthub.const import CONF_TOKEN, CONF_URL, CONF_USERNAME, CONF_VERIFY_SSL
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.ntfy.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_aiontfy() -> Generator[AsyncMock]:
    ...
    with (
        patch("smarthub.components.ntfy.Ntfy", autospec=True) as mock_client,
        patch("smarthub.components.ntfy.config_flow.Ntfy", new=mock_client),
    ):
...
def mock_random() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.ntfy.config_flow.random.choices",
