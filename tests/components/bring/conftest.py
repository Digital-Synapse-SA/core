...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.bring.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_bring_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.bring.Bring",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.bring.config_flow.Bring",
            new=mock_client,
        ),
    ):
...
def mock_uuid() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.bring.todo.uuid.uuid4",
        autospec=True,
    ) as mock_client:
