...
async def test_setup_entry(hass: SmartHub) -> None:
    ...
    with patch("smarthub.components.devolo_home_control.HomeControl"):
        await hass.config_entries.async_setup(entry.entry_id)
        assert entry.state is ConfigEntryState.LOADED


@pytest.mark.parametrize("credentials_valid", [False])
async def test_setup_entry_credentials_invalid(hass: SmartHub) -> None:
...
async def test_setup_entry_maintenance(hass: SmartHub) -> None:
...
async def test_setup_gateway_offline(hass: SmartHub) -> None:
...
async def test_home_assistant_stop(hass: SmartHub) -> None:
