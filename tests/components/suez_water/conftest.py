...
def mock_setup_entry(recorder_mock: Recorder) -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.suez_water.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_suez_client(recorder_mock: Recorder) -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.suez_water.coordinator.SuezClient", autospec=True
        ) as mock_client,
        patch(
            "smarthub.components.suez_water.config_flow.SuezClient",
            new=mock_client,
        ),
    ):
