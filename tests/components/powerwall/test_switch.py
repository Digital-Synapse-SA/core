...
from smarthub.const import ATTR_ENTITY_ID, CONF_IP_ADDRESS, STATE_OFF, STATE_ON
...
async def mock_powerwall_fixture(hass: SmartHub) -> MagicMock:
...
async def test_entity_registry(
    hass: SmartHub, mock_powerwall, entity_registry: er.EntityRegistry
) -> None:
...
async def test_initial(hass: SmartHub, mock_powerwall) -> None:
...
async def test_on(hass: SmartHub, mock_powerwall) -> None:
...
async def test_off(hass: SmartHub, mock_powerwall) -> None:
...
async def test_exception_on_powerwall_error(
    hass: SmartHub, mock_powerwall
) -> None:
    """Ensure that an exception in the tesla_powerwall library causes a SmartHubError."""

    mock_powerwall.set_island_mode.side_effect = PowerwallError("Mock exception")
    with pytest.raises(SmartHubError, match="Setting off-grid operation to"):
        await hass.services.async_call(
            SWITCH_DOMAIN,
            SERVICE_TURN_ON,
            {ATTR_ENTITY_ID: ENTITY_ID},
