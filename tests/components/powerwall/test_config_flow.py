...
async def test_form_source_user(hass: SmartHub) -> None:
    ...
    with (
        ...
        patch(
            "smarthub.components.powerwall.async_setup_entry",
            return_value=True,
        ) as mock_setup_entry,
    ):
...
async def test_form_cannot_connect(hass: SmartHub, exc: Exception) -> None:
...
async def test_form_wrong_version(hass: SmartHub) -> None:
...
async def test_dhcp_discovery_manual_configure(hass: SmartHub) -> None:
    """Test we can process the discovery from dhcp and manually configure."""
    mock_powerwall = await _mock_powerwall_site_name(hass, "Some site")

    with patch(
        "smarthub.components.powerwall.config_flow.Powerwall.login",
...
async def test_dhcp_discovery_auto_configure(hass: SmartHub) -> None:
...
async def test_dhcp_discovery_cannot_connect(hass: SmartHub) -> None:
...
async def test_dhcp_discovery_update_ip_address(hass: SmartHub) -> None:
...
async def test_dhcp_discovery_updates_unique_id(hass: SmartHub) -> None:
