...
from smarthub.components.samsungtv.const import DOMAIN, WEBSOCKET_SSL_PORT
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.samsungtv.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def samsungtv_mock_async_get_local_ip() -> Generator[None]:
    ...
    with patch(
        "smarthub.components.samsungtv.media_player.async_get_local_ip",
...
def app_list_delay_fixture() -> Generator[None]:
    ...
    with patch("smarthub.components.samsungtv.media_player.APP_LIST_DELAY", 0):
        yield
...
def upnp_factory_fixture() -> Generator[Mock]:
    """Patch UpnpFactory."""
    with patch(
        "smarthub.components.samsungtv.media_player.UpnpFactory",
        autospec=True,
...
def dmr_device_fixture(upnp_device: Mock) -> Generator[Mock]:
    """Patch async_upnp_client."""
    with patch(
        "smarthub.components.samsungtv.media_player.DmrDevice",
        autospec=True,
    ) as dmr_device_class:
...
def upnp_notify_server_fixture(upnp_factory: Mock) -> Generator[Mock]:
    """Patch async_upnp_client."""
    with patch(
        "smarthub.components.samsungtv.media_player.AiohttpNotifyServer",
        autospec=True,
...
def remote_legacy_fixture() -> Generator[Mock]:
    ...
    with patch(
        "smarthub.components.samsungtv.bridge.Remote", return_value=remote_legacy
    ):
        yield remote_legacy
...
def rest_api_fixture_non_ssl_only(hass: SmartHub) -> Generator[None]:
