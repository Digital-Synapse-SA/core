...
from smarthub.components.rainmachine import (
...
async def test_invalid_password(hass: SmartHub, config) -> None:
...
async def test_migrate_1_2(
    ...
    with (
        patch(
            "smarthub.components.rainmachine.async_setup_entry", return_value=True
        ),
        patch(
...
async def test_options_flow(hass: SmartHub, config, config_entry) -> None:
...
async def test_step_user(hass: SmartHub, config, setup_rainmachine) -> None:
...
async def test_step_homekit_zeroconf_ip_already_exists(
    hass: SmartHub, client, config, config_entry, source
) -> None:
    """Test homekit and zeroconf with an ip that already exists."""
    with patch(
        "smarthub.components.rainmachine.config_flow.Client", return_value=client
    ):
        result = await hass.config_entries.flow.async_init(
            DOMAIN,
...
async def test_step_homekit_zeroconf_ip_change(
    hass: SmartHub, client, config_entry, source
) -> None:
...
async def test_step_homekit_zeroconf_new_controller_when_some_exist(
    hass: SmartHub, client, config, source
) -> None:
