...
@patch("smarthub.components.gree.PLATFORMS", [SWITCH_DOMAIN])
...
async def test_send_switch_on(hass: SmartHub, entity: str) -> None:
...
async def test_send_switch_on_device_timeout(
    hass: SmartHub, device, entity: str
) -> None:
...
@pytest.mark.usefixtures("entity_registry_enabled_by_default")
async def test_send_switch_off(hass: SmartHub, entity: str) -> None:
    """Test for sending power on command to the device."""
    await async_setup_gree(hass)
...
@pytest.mark.usefixtures("entity_registry_enabled_by_default")
async def test_send_switch_toggle(hass: SmartHub, entity: str) -> None:
    """Test for sending power on command to the device."""
    await async_setup_gree(hass)
