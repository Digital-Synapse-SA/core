...
from smarthub.const import CONF_API_TOKEN, CONF_HOST
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.mealie.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_mealie_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.mealie.MealieClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.mealie.config_flow.MealieClient",
            new=mock_client,
        ),
    ):
