...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.cambridge_audio.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_stream_magic_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.cambridge_audio.StreamMagicClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.cambridge_audio.config_flow.StreamMagicClient",
            new=mock_client,
        ),
    ):
