...
def mock_async_create_device():
    ...
    with patch(
        "smarthub.components.upnp.device.UpnpFactory.async_create_device"
    ) as mock_create:
        yield mock_create
...
def mock_igd_device(mock_async_create_device) -> IgdDevice:
    ...
    with (
        patch(
            "smarthub.components.upnp.device.async_get_local_ip",
            return_value=(socket.AF_INET, "127.0.0.1"),
        ),
        patch(
            "smarthub.components.upnp.device.IgdDevice.__new__",
            return_value=mock_igd_device,
        ),
        patch(
            "smarthub.components.upnp.device.AiohttpNotifyServer.__new__",
...
def mock_setup_entry():
    ...
    with patch(
        "smarthub.components.upnp.async_setup_entry",
