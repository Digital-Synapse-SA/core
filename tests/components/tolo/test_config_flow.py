...
from smarthub.components.tolo.const import DOMAIN
...
def toloclient_fixture() -> Mock:
    """Patch libraries."""
    with patch("smarthub.components.tolo.config_flow.ToloClient") as toloclient:
        yield toloclient
...
def coordinator_toloclient() -> Mock:
    ...
    """
    with patch(
        "smarthub.components.tolo.coordinator.ToloClient", side_effect=Exception
    ) as toloclient:
        yield toloclient


async def test_user_with_timed_out_host(hass: SmartHub, toloclient: Mock) -> None:
...
async def test_user_walkthrough(
    hass: SmartHub, toloclient: Mock, coordinator_toloclient: Mock
) -> None:
...
async def test_dhcp_invalid_device(hass: SmartHub, toloclient: Mock) -> None:
    """Test starting a flow from discovery."""
    toloclient().get_status.side_effect = lambda *args, **kwargs: None
