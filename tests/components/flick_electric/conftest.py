...
from smarthub.components.flick_electric.const import CONF_ACCOUNT_ID, DOMAIN
...
def mock_flick_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.flick_electric.FlickAPI",
            autospec=True,
        ) as mock_api,
        patch(
            "smarthub.components.flick_electric.config_flow.FlickAPI",
            new=mock_api,
        ),
        patch(
