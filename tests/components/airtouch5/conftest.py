...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.airtouch5.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_airtouch5_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.airtouch5.Airtouch5SimpleClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.airtouch5.config_flow.Airtouch5SimpleClient",
            new=mock_client,
        ),
    ):
