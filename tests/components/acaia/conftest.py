...
from smarthub.components.acaia.const import CONF_IS_NEW_STYLE_SCALE, DOMAIN
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.acaia.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_verify() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.acaia.config_flow.is_new_scale", return_value=True
    ) as mock_verify:
        yield mock_verify
...
async def init_integration(
    hass: SmartHub, mock_config_entry: MockConfigEntry, mock_scale: MagicMock
) -> MockConfigEntry:
...
def mock_scale() -> Generator[MagicMock]:
    ...
    with (
        patch(
            "smarthub.components.acaia.coordinator.AcaiaScale",
            autospec=True,
        ) as scale_mock,
    ):
