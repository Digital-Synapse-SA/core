...
def patch_load_json_object() -> Generator[MagicMock]:
    ...
    with patch(
        "smarthub.components.ps4.load_json_object", return_value={}
    ) as mock_load:
        yield mock_load
...
def patch_save_json() -> Generator[MagicMock]:
    ...
    with patch("smarthub.components.ps4.save_json") as mock_save:
        yield mock_save
...
def mock_ddp_endpoint() -> Generator[None]:
    ...
    with patch(
        "smarthub.components.ps4.async_create_ddp_endpoint",
