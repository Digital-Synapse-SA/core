...
def mock_solarlog_connector():
    ...
    with (
        patch(
            "smarthub.components.solarlog.coordinator.SolarLogConnector",
            autospec=True,
            return_value=mock_solarlog_api,
        ),
        patch(
            "smarthub.components.solarlog.config_flow.SolarLogConnector",
            autospec=True,
            return_value=mock_solarlog_api,
        ),
    ):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.solarlog.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_test_connection():
    ...
    with patch(
        "smarthub.components.solarlog.config_flow.SolarLogConfigFlow._test_connection",
        return_value=True,
    ):
        yield
