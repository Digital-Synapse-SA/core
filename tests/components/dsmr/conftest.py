...
def dsmr_connection_fixture() -> Generator[tuple[MagicMock, MagicMock, MagicMock]]:
    ...
    with (
        patch(
            "smarthub.components.dsmr.sensor.create_dsmr_reader",
            connection_factory,
        ),
        patch(
            "smarthub.components.dsmr.sensor.create_tcp_dsmr_reader",
            connection_factory,
        ),
    ):
...
def rfxtrx_dsmr_connection_fixture() -> Generator[
    ...
    with (
        patch(
            "smarthub.components.dsmr.sensor.create_rfxtrx_dsmr_reader",
            connection_factory,
        ),
        patch(
            "smarthub.components.dsmr.sensor.create_rfxtrx_tcp_dsmr_reader",
            connection_factory,
        ),
    ):
        yield (connection_factory, transport, protocol)
...
def dsmr_connection_send_validate_fixture() -> Generator[
    ...
    with (
        ...
        patch(
            "smarthub.components.dsmr.config_flow.create_tcp_dsmr_reader",
            connection_factory,
        ),
    ):
        yield (connection_factory, transport, protocol)
...
def rfxtrx_dsmr_connection_send_validate_fixture() -> Generator[
    ...
    with (
        patch(
            "smarthub.components.dsmr.config_flow.create_rfxtrx_dsmr_reader",
            connection_factory,
        ),
        patch(
            "smarthub.components.dsmr.config_flow.create_rfxtrx_tcp_dsmr_reader",
            connection_factory,
        ),
    ):
        yield (connection_factory, transport, protocol)
