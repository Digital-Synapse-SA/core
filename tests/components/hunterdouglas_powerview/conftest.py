...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.hunterdouglas_powerview.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_hunterdouglas_hub(
    ...
    with (
        ...
        patch(
            "smarthub.components.hunterdouglas_powerview.util.Hub.request_raw_firmware",
            return_value=load_json_object_fixture(firmware_json, DOMAIN),
        ),
        patch(
            "smarthub.components.hunterdouglas_powerview.Rooms.get_resources",
            return_value=load_json_value_fixture(rooms_json, DOMAIN),
        ),
        patch(
            "smarthub.components.hunterdouglas_powerview.Scenes.get_resources",
            return_value=load_json_value_fixture(scenes_json, DOMAIN),
        ),
        patch(
            "smarthub.components.hunterdouglas_powerview.Shades.get_resources",
            return_value=load_json_value_fixture(shades_json, DOMAIN),
        ),
        patch(
            "smarthub.components.hunterdouglas_powerview.cover.BaseShade.refresh",
        ),
        patch(
            "smarthub.components.hunterdouglas_powerview.cover.BaseShade.current_position",
