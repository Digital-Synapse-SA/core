...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.madvr.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
...
def mock_madvr_client() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.madvr.config_flow.Madvr", autospec=True
        ) as mock_client,
        patch("smarthub.components.madvr.Madvr", new=mock_client),
    ):
