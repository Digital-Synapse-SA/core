...
from smarthub.const import CONF_ACCESS_TOKEN, CONF_EMAIL
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.letpot.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_client(device_type: str) -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.letpot.LetPotClient",
            autospec=True,
        ) as mock_client,
        patch(
            "smarthub.components.letpot.config_flow.LetPotClient",
            new=mock_client,
        ),
    ):
...
def mock_device_client(device_type: str) -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.letpot.coordinator.LetPotDeviceClient",
        autospec=True,
