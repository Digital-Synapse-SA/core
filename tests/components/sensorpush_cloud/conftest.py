...
from smarthub.const import CONF_EMAIL
...
def mock_api() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.sensorpush_cloud.config_flow.SensorPushCloudApi",
...
def mock_helper() -> Generator[AsyncMock]:
    ...
    with (
        patch(
            "smarthub.components.sensorpush_cloud.coordinator.SensorPushCloudHelper",
            autospec=True,
        ) as mock_helper,
    ):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.sensorpush_cloud.async_setup_entry", return_value=True
    ) as mock_setup_entry:
