...
async def test_not_setup_views_if_onboarded(
    ...
    with patch("smarthub.components.onboarding.views.async_setup") as mock_setup:
...
async def test_setup_views_if_not_onboarded(hass: SmartHub) -> None:
    """Test if onboarding is not done, we setup views."""
    with patch(
        "smarthub.components.onboarding.views.async_setup",
    ) as mock_setup:
        assert await async_setup_component(hass, "onboarding", {})
...
async def test_having_owner_finishes_user_step(
    ...
    with (
        patch("smarthub.components.onboarding.views.async_setup") as mock_setup,
...
async def test_migration(hass: SmartHub, hass_storage: dict[str, Any]) -> None:
