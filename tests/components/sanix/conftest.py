...
from smarthub.components.sanix.const import CONF_SERIAL_NUMBER, DOMAIN
...
def mock_sanix():
    ...
    with (
        patch(
            "smarthub.components.sanix.config_flow.Sanix",
            autospec=True,
        ) as mock_sanix_api,
        patch(
            "smarthub.components.sanix.Sanix",
            new=mock_sanix_api,
        ),
    ):
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.sanix.async_setup_entry",
        return_value=True,
    ) as mock_setup_entry:
