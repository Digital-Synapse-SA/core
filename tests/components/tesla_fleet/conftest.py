...
from smarthub.components.tesla_fleet.const import DOMAIN, SCOPES
...
def create_config_entry(
    ...
    """Create Tesla Fleet entry in SmartHub."""
...
def noscope_config_entry(expires_at: int) -> MockConfigEntry:
    """Create Tesla Fleet entry in SmartHub without scopes."""
...
def mock_products() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.tesla_fleet.TeslaFleetApi.products",
...
def mock_find_server() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.tesla_fleet.TeslaFleetApi.find_server",
    ) as mock_find_server:
...
def mock_request():
    ...
    with patch(
        "smarthub.components.tesla_fleet.TeslaFleetApi._request",
