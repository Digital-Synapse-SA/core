...
from smarthub.components.zha.backup import async_post_backup, async_pre_backup
from smarthub.core import SmartHub


async def test_pre_backup(
    hass: SmartHub, zigpy_app_controller: ControllerApplication, setup_zha
) -> None:
...
@patch("smarthub.components.zha.backup.get_zha_gateway", side_effect=ValueError())
async def test_pre_backup_no_gateway(hass: SmartHub, setup_zha) -> None:
    """Test graceful backup failure when no gateway exists."""
    await setup_zha()
    await async_pre_backup(hass)


async def test_post_backup(hass: SmartHub, setup_zha) -> None:
