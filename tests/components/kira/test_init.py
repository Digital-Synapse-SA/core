...
from smarthub.components import kira
...
def setup_comp():
    ...
    with patch("smarthub.components.kira.pykira.KiraReceiver"):
        yield
...
async def test_kira_empty_config(hass: SmartHub) -> None:
...
async def test_kira_setup(hass: SmartHub) -> None:
...
async def test_load_codes(hass: SmartHub, work_dir) -> None:
