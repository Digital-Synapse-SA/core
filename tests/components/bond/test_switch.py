...
async def test_turn_on_switch(hass: SmartHub) -> None:
...
async def test_turn_off_switch(hass: SmartHub) -> None:
...
async def test_switch_set_power_belief(hass: SmartHub) -> None:
...
async def test_switch_set_power_belief_api_error(hass: SmartHub) -> None:
    """Tests that the set power belief service throws SmartHubError in the event of an api error."""
    await setup_platform(
        hass, SWITCH_DOMAIN, generic_device("name-1"), bond_device_id="test-device-id"
    )
...
async def test_update_reports_switch_is_on(hass: SmartHub) -> None:
...
async def test_update_reports_switch_is_off(hass: SmartHub) -> None:
...
async def test_switch_available(hass: SmartHub) -> None:
