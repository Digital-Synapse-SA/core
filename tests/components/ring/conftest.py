...
from smarthub.components.ring.const import CONF_CONFIG_ENTRY_MINOR_VERSION
from smarthub.const import CONF_DEVICE_ID, CONF_USERNAME
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "smarthub.components.ring.async_setup_entry", return_value=True
    ) as mock_setup_entry:
...
def mock_ring_init_auth_class():
    ...
    with patch("smarthub.components.ring.Auth", autospec=True) as mock_ring_auth:
...
def mock_ring_auth():
    ...
    with patch(
        "smarthub.components.ring.config_flow.Auth", autospec=True
    ) as mock_ring_auth:
        mock_ring_auth.return_value.async_fetch_token.return_value = {
            "access_token": "mock-token"
        }
...
def mock_ring_client(mock_ring_auth, mock_ring_devices):
    ...
    with patch("smarthub.components.ring.Ring", new=mock_client):
...
def mock_ring_event_listener_class():
    ...
    with patch(
        "smarthub.components.ring.coordinator.RingEventListener", autospec=True
