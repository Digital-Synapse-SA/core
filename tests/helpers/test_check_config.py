...
from smarthub.helpers.check_config import (
    CheckConfigError,
    SmartHubConfig,
...
async def test_bad_core_config(hass: SmartHub) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        error = CheckConfigError(
            (
                f"Invalid config for 'smarthub' at {YAML_CONFIG_FILE}, line 2:"
...
async def test_config_platform_valid(hass: SmartHub) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        assert res.keys() == {"smarthub", "light"}
        assert res["light"] == [{"platform": "demo"}]
        _assert_warnings_errors(res, [], [])


async def test_integration_not_found(hass: SmartHub) -> None:
...
async def test_integrationt_requirement_not_found(hass: SmartHub) -> None:
    ...
    with (
        patch(
            "smarthub.helpers.check_config.async_get_integration_with_requirements",
...
async def test_integration_not_found_recovery_mode(hass: SmartHub) -> None:
...
async def test_integration_not_found_safe_mode(hass: SmartHub) -> None:
...
async def test_integration_import_error(hass: SmartHub) -> None:
    """Test errors if integration with a requirement not found not found."""
    # Make sure they don't exist
    files = {YAML_CONFIG_FILE: BASE_CONFIG + "light:"}
    with (
        patch(
            "smarthub.loader.Integration.async_get_component",
...
async def test_integration_schema_error(
    hass: SmartHub, integration: str, errors: int, warnings: int, message: str
) -> None:
...
async def test_platform_not_found(hass: SmartHub) -> None:
...
async def test_platform_not_found_recovery_mode(hass: SmartHub) -> None:
...
async def test_platform_not_found_safe_mode(hass: SmartHub) -> None:
...
async def test_config_platform_import_error(hass: SmartHub) -> None:
    """Test errors if config platform fails to import."""
    # Make sure they don't exist
    files = {YAML_CONFIG_FILE: BASE_CONFIG + "light:\n  platform: beer"}
    with (
        patch(
            "smarthub.loader.Integration.async_get_platform",
            side_effect=ImportError("blablabla"),
        ),
        patch("os.path.isfile", return_value=True),
        patch("smarthub.loader.Integration.platforms_exists", return_value=True),
        patch_yaml_files(files),
    ):
        res = await async_check_ha_config_file(hass)
        log_ha_config(res)
...
async def test_platform_import_error(hass: SmartHub) -> None:
    """Test errors if platform not found."""
    # Make sure they don't exist
    files = {YAML_CONFIG_FILE: BASE_CONFIG + "light:\n  platform: demo"}
    with (
...
async def test_package_invalid(hass: SmartHub) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        warning = CheckConfigError(
            ...
            "smarthub.packages.p1.group",
            {"group": ["a"]},
        )
        _assert_warnings_errors(res, [warning], [])


async def test_package_definition_invalid_slug_keys(hass: SmartHub) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        warning = CheckConfigError(
            ...
            "smarthub.packages.not a slug",
            {"group": ["a"]},
        )
        _assert_warnings_errors(res, [warning], [])


async def test_package_definition_invalid_dict(hass: SmartHub) -> None:
    ...
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        ...
        warning = CheckConfigError(
            ...
            "smarthub.packages.not_a_dict",
            [{"group": ["a"]}],
        )
        _assert_warnings_errors(res, [warning], [])


async def test_package_schema_invalid(hass: SmartHub) -> None:
    """Test an invalid platform config because of severely broken packages section."""
    files = {
        YAML_CONFIG_FILE: "smarthub:\n  packages:\n    - must\n    - not\n    - be\n    - a\n    - list"
    }
    with patch("os.path.isfile", return_value=True), patch_yaml_files(files):
        res = await async_check_ha_config_file(hass)
        log_ha_config(res)
...
async def test_missing_included_file(hass: SmartHub) -> None:
...
async def test_automation_config_platform(hass: SmartHub) -> None:
...
async def test_removed_yaml_support(hass: SmartHub) -> None:
