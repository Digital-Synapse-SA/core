"""Test to verify that SmartHub exceptions work."""
...
async def test_home_assistant_error(
    ...
    """Test edge cases with SmartHubError."""
    ...
    with patch(
        ...
        with pytest.raises(SmartHubError) as exc:
            raise SmartHubError(*exception_args, **exception_kwargs)
...
async def test_home_assistant_error_subclass(hass: SmartHub) -> None:
    """Test __str__ method on an SmartHubError subclass."""

    class _SubExceptionDefault(SmartHubError):
        """Sub class, default with generated message."""

    class _SubExceptionConstructor(SmartHubError):
    ...
    class _SubExceptionConstructorGenerate(SmartHubError):
    ...
    class _SubExceptionGenerate(SmartHubError):
        """Sub class, no generated message."""

        generate_message: bool = True

    class _SubClassWithExceptionGroup(SmartHubError, BaseExceptionGroup):
        """Sub class with exception group, no generated message."""

    class _SubClassWithExceptionGroupGenerate(SmartHubError, BaseExceptionGroup):
