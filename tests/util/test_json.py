"""Test SmartHub json utility functions."""
...
def test_load_bad_data(tmp_path: Path) -> None:
    ...
    with pytest.raises(SmartHubError, match=re.escape(str(fname))) as err:
        load_json(fname)
...
def test_load_json_value_data(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        SmartHubError, match="Expected JSON to be parsed as a dict"
    ):
        load_json_object(fname)
    with pytest.raises(
        SmartHubError, match="Expected JSON to be parsed as a list"
    ):
