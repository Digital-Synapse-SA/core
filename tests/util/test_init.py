...
def test_repr_helper() -> None:
    ...
    with patch("smarthub.util.dt.DEFAULT_TIME_ZONE", tz):
        assert (
...
def test_throttle() -> None:
    ...
    with patch("smarthub.util.utcnow", return_value=plus3):
        test_throttle1()
        test_throttle2()
    ...
    with patch("smarthub.util.utcnow", return_value=plus5):
        test_throttle1()
        test_throttle2()
