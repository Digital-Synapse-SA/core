"""Test SmartHub package util methods."""
...
from smarthub.util import package
...
def mock_sys() -> Generator[MagicMock]:
    """Mock sys."""
    with patch("smarthub.util.package.sys", spec=object) as sys_mock:
...
def mock_popen(lib_dir) -> Generator[MagicMock]:
    """Return a Popen mock."""
    with patch("smarthub.util.package.Popen") as popen_mock:
...
def mock_env_copy() -> Generator[Mock]:
    ...
    with patch("smarthub.util.package.os.environ.copy") as env_copy:
...
def mock_venv() -> Generator[MagicMock]:
    """Mock smarthub.util.package.is_virtual_env."""
    with patch("smarthub.util.package.is_virtual_env") as mock:
...
def test_install_pip_compatibility_use_workaround(
    ...
    with patch("smarthub.util.package.site", autospec=True) as site_mock:
...
async def test_async_get_user_site(mock_env_copy) -> None:
    ...
    with patch(
        "smarthub.util.package.asyncio.create_subprocess_exec",
...
def test_check_package_previous_failed_install() -> None:
    ...
    with patch("smarthub.util.package.version", return_value=None):
...
async def test_is_docker_env(
    ...
    with (
        patch("smarthub.util.package.Path", side_effect=new_path_mock),
        patch(
            "smarthub.util.package.is_official_image",
