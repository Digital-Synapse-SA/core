"""Test SmartHub secret substitution in YAML files."""
...
def test_secrets_from_unrelated_fails(
    ...
    with pytest.raises(SmartHubError, match="Secret test not defined"):
        load_config_file(
...
def test_secrets_logger_removed(
    ...
    with pytest.raises(SmartHubError, match="Secret logger not defined"):
...
def test_secrets_are_not_dict(
    ...
    with pytest.raises(SmartHubError, match="Secrets is not a dictionary"):
