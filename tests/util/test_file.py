"""Test SmartHub file utility functions."""
...
from smarthub.util.file import WriteError, write_utf8_file, write_utf8_file_atomic
...
def test_write_utf8_file_fails_at_creation(tmpdir: py.path.local) -> None:
    ...
    with (
        ...
        patch(
            "smarthub.util.file.tempfile.NamedTemporaryFile", side_effect=OSError
        ),
    ):
...
def test_write_utf8_file_fails_at_rename(
    ...
    with (
        pytest.raises(WriteError),
        patch("smarthub.util.file.os.replace", side_effect=OSError),
    ):
        write_utf8_file(test_file, '{"some":"data"}', False)
...
def test_write_utf8_file_fails_at_rename_and_remove(
    ...
    with (
        pytest.raises(WriteError),
        patch("smarthub.util.file.os.remove", side_effect=OSError),
        patch("smarthub.util.file.os.replace", side_effect=OSError),
    ):
        write_utf8_file(test_file, '{"some":"data"}', False)
...
def test_write_utf8_file_atomic_fails(tmpdir: py.path.local) -> None:
    ...
    with (
        pytest.raises(WriteError),
        patch("smarthub.util.file.AtomicWriter.open", side_effect=OSError),
    ):
