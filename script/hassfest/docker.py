...
from smarthub.components.go2rtc.const import RECOMMENDED_VERSION as GO2RTC_VERSION
...
# Synchronize with smarthub/core.py:async_stop
ENV \
...
## Setup SmartHub Core dependencies
COPY requirements.txt smarthub/
COPY smarthub/package_constraints.txt smarthub/smarthub/
RUN \
    uv pip install \
        --no-build \
        -r smarthub/requirements.txt

COPY requirements_all.txt home_assistant_frontend-* home_assistant_intents-* smarthub/
RUN \
    if ls smarthub/home_assistant_*.whl 1> /dev/null 2>&1; then \
        uv pip install smarthub/home_assistant_*.whl; \
    fi \
    && uv pip install \
        --no-build \
        -r smarthub/requirements_all.txt
...
ENV \
    UV_SYSTEM_PYTHON=true \
    UV_EXTRA_INDEX_URL="https://wheels.smart-hub.io/musllinux-index/"

SHELL ["/bin/sh", "-o", "pipefail", "-c"]
ENTRYPOINT ["/usr/src/smarthub/script/hassfest/docker/entrypoint.sh"]
WORKDIR "/github/workspace"

COPY . /usr/src/smarthub
...
RUN --mount=from=ghcr.io/astral-sh/uv:{uv},source=/uv,target=/bin/uv \
    ...
    && uv pip install \
        --no-build \
        --no-cache \
        -c /usr/src/smarthub/smarthub/package_constraints.txt \
        -r /usr/src/smarthub/requirements.txt \
        stdlib-list==0.10.0 \
...
LABEL "name"="hassfest"
LABEL "maintainer"="SmartHub <hello@smart-hub.io>"
