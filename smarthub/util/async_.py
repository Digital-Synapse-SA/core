    ...
    *,
    ...
    if not loop:
        try:
            ...
            from smarthub.helpers import frame  # noqa: PLC0415
...
def run_callback_threadsafe[_T, *_Ts](
    ...
    if hasattr(loop, _SHUTDOWN_RUN_CALLBACK_THREADSAFE):
        #
        # If the final `SmartHub.async_block_till_done` in
        # `SmartHub.async_stop` has already been called, the callback
...
def shutdown_run_callback_threadsafe(loop: AbstractEventLoop) -> None:
    ...
    be called when SmartHub is going to shutdown and
