...
class SerializationError(SmartHubError):
...
def load_json(
    ...
    try:
        ...
        raise SmartHubError(f"Error while loading {filename}: {error}") from error
...
def load_json_array(
    ...
    raise SmartHubError(f"Expected JSON to be parsed as a list got {type(value)}")
...
def load_json_object(
    ...
    raise SmartHubError(f"Expected JSON to be parsed as a dict got {type(value)}")
