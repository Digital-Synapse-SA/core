...
class SmartHubQueueListener(logging.handlers.QueueListener):
    ...
    EXCLUDED_LOG_COUNT_MODULES = [
        "smarthub.components.automation",
        "smarthub.components.script",
        "smarthub.setup",
        "smarthub.util.logging",
    ]
...
class SmartHubQueueHandler(logging.handlers.QueueHandler):
...
@callback
def async_activate_log_queue_handler(hass: SmartHub) -> None:
    ...
    queue_handler = SmartHubQueueHandler(simple_queue)
    ...
    listener = SmartHubQueueListener(simple_queue, *migrated_handlers)
