"""Provide methods to bootstrap a SmartHub instance."""
...
from .exceptions import SmartHubError
from .helpers import (
    area_registry,
    backup,
    category_registry,
...
ERROR_LOG_FILENAME = "smart-hub.log"
...
CORE_INTEGRATIONS = {"smarthub", "persistent_notification"}
...
async def async_setup_hass(
    ...
    async def create_hass() -> core.SmartHub:
        """Create the hass object and do basic setup."""
        hass = core.SmartHub(runtime_config.config_dir)
...
def open_hass_ui(hass: core.SmartHub) -> None:
    """Open the UI."""
    ...
    if not webbrowser.open(url):
        _LOGGER.warning(
            "Unable to open the SmartHub UI in a browser. Open it yourself at %s",
            url,
        )
...
async def async_load_base_functionality(hass: core.SmartHub) -> None:
...
async def async_from_config_dict(
    config: ConfigType, hass: core.SmartHub
) -> core.SmartHub | None:
    """Try to configure SmartHub from a configuration dictionary.
    ...
    if not all(
        ...
        _LOGGER.error("SmartHub core failed to initialize. ")
        return None

    _LOGGER.debug("SmartHub core initialized")
    ...
    try:
        ...
        _LOGGER.error(
            "SmartHub core failed to initialize. Further initialization aborted"
        )
        return None
    ...
    stop = monotonic()
    _LOGGER.info("SmartHub initialized in %.2fs", stop - start)
...
def _get_domains(hass: core.SmartHub, config: dict[str, Any]) -> set[str]:
    """Get domains of components to set up."""
    # Filter out the repeating and common config section [smarthub]
    domains = {
...
async def _async_resolve_domains_and_preload(
    hass: core.SmartHub, config: dict[str, Any]
