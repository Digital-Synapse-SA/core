"""Core components of SmartHub.

SmartHub is a Home Automation framework for observing the state
...
if TYPE_CHECKING:
    ...
    from .components.http import SmartHubHTTP
...
# The Config class was moved to core_config in SmartHub 2024.11
_DEPRECATED_Config = DeferredDeprecatedAlias(
    _deprecated_core_config, "smarthub.core_config.Config", "2025.11"
)
...
class _Hass(threading.local):
    """Container which makes a SmartHub instance available to the event loop."""
...
@callback
def async_get_hass() -> SmartHub:
    """Return the SmartHub instance.

    Raises SmartHubError when called from the wrong thread.
    ...
    if not (hass := async_get_hass_or_none()):
        raise SmartHubError("async_get_hass called from the wrong thread")
    return hass


def async_get_hass_or_none() -> SmartHub | None:
    """Return the SmartHub instance or None.
...
class CoreState(enum.Enum):
    """Represent the current state of SmartHub."""
...
class SmartHub:
    """Root object of the SmartHub home automation."""

    auth: AuthManager
    http: SmartHubHTTP = None  # type: ignore[assignment]
    ...
    def __repr__(self) -> str:
        ...
        return f"<SmartHub {self.state}>"

    def __init__(self, config_dir: str) -> None:
        """Initialize new SmartHub object."""
    ...
    def is_running(self) -> bool:
        """Return if SmartHub is running."""
        return self.state in (CoreState.starting, CoreState.running)

    @cached_property
    def is_stopping(self) -> bool:
        """Return if SmartHub is stopping."""
    ...
    def start(self) -> int:
        ...
        # Run forever
        # Block until stopped
        _LOGGER.info("Starting SmartHub core loop")
    ...
    async def async_run(self, *, attach_signals: bool = True) -> int:
        """SmartHub main entry point.

        Start SmartHub and block until stopped.

        This method is a coroutine.
        """
        if self.state is not CoreState.not_running:
            raise RuntimeError("SmartHub is already running")
    ...
    async def async_start(self) -> None:
        ...
        This method is a coroutine.
        """
        _LOGGER.info("Starting SmartHub")
        ...
        if pending:
            _LOGGER.warning(
                (
                    "Something is blocking SmartHub from wrapping up the start up"
                    " phase. We're going to continue anyway. Please report the"
                    " following info at"
                    " https://github.com/smart-hub/core/issues: %s"
        ...
        if self.state is not CoreState.starting:
            _LOGGER.warning(
                "SmartHub startup has been interrupted. "
    ...
    def _async_add_hass_job[_R](
        ...
        *args: Any,
        ...
        # https://github.com/smart-hub/core/pull/71960
    ...
    def async_create_background_task[_R](
        ...
        the lifetime of SmartHub or an integration's setup.
    ...
    def stop(self) -> None:
        """Stop SmartHub and shuts down all threads."""
    ...
    async def async_stop(self, exit_code: int = 0, *, force: bool = False) -> None:
        ...
        SmartHub's current state. You should not set this flag
        ...
        if not force:
            ...
            if self.state is CoreState.starting:
                # This may not work
                _LOGGER.warning(
                    "Stopping SmartHub before startup has completed may fail"
                )
        ...
        for task in self._background_tasks:
            self._tasks.add(task)
            task.add_done_callback(self._tasks.remove)
            task.cancel("SmartHub is stopping")
        ...
        for task in list(running_tasks):
            ...
            task.cancel("SmartHub final writes shutdown stage")
            try:
...
class EventBus:
    ...
    def async_listen(
        ...
        if event_filter is not None and not is_callback_check_partial(event_filter):
            raise SmartHubError(f"Event filter {event_filter} is not a callback")
...
class ServiceRegistry:
    ...
    def async_services_internal(self) -> dict[str, dict[str, Service]]:
        ...
        It is only expected to be called from the SmartHub internals
