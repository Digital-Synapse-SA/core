...
from smarthub.components.websocket_api import require_admin
from smarthub.core import SmartHub, callback
from smarthub.exceptions import SmartHubError
from smarthub.helpers import device_registry as dr
from smarthub.helpers.device_registry import DeviceEntry, DeviceEntryDisabler
...
async def websocket_remove_config_entry_from_device(
    ...
    if (config_entry := hass.config_entries.async_get_entry(config_entry_id)) is None:
        raise SmartHubError("Unknown config entry")

    if not config_entry.supports_remove_device:
        raise SmartHubError("Config entry does not support device removal")

    if (device_entry := registry.async_get(device_id)) is None:
        raise SmartHubError("Unknown device")

    if config_entry_id not in device_entry.config_entries:
        raise SmartHubError("Config entry not in device")
    ...
    try:
        ...
        raise SmartHubError("Integration not found") from exc
