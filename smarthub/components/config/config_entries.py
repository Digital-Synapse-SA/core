...
from smarthub.auth.permissions.const import CAT_CONFIG_ENTRIES, POLICY_EDIT
...
from smarthub.exceptions import DependencyError, Unauthorized
from smarthub.helpers import config_validation as cv
from smarthub.helpers.data_entry_flow import (
    FlowManagerIndexView,
    FlowManagerResourceView,
)
from smarthub.helpers.dispatcher import async_dispatcher_connect
from smarthub.helpers.json import json_fragment
...
class ConfigManagerEntryIndexView(SmartHubView):
...
class ConfigManagerEntryResourceView(SmartHubView):
...
class ConfigManagerEntryResourceReloadView(SmartHubView):
...
class ConfigManagerAvailableFlowView(SmartHubView):
...
async def _async_matching_config_entries_json_fragments(
    hass: SmartHub, type_filter: list[str] | None, domain: str | None
) -> list[json_fragment]:
    """Return matching config entries by type and/or domain."""
    if domain:
