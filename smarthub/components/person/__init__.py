...
from smarthub.auth import EVENT_USER_REMOVED
from smarthub.components import persistent_notification, websocket_api
...
async def async_add_user_device_tracker(
    hass: SmartHub, user_id: str, device_tracker_entity_id: str
) -> None:
...
@callback
def persons_with_entity(hass: SmartHub, entity_id: str) -> list[str]:
...
@callback
def entities_in_person(hass: SmartHub, entity_id: str) -> list[str]:
...
async def filter_yaml_data(hass: SmartHub, persons: list[dict]) -> list[dict]:
