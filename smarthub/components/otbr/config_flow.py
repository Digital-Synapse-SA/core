...
from smarthub.components.hassio import AddonError, AddonManager
from smarthub.components.smarthub_yellow import hardware as yellow_hardware
from smarthub.components.thread import async_get_preferred_dataset
...
class AlreadyConfigured(SmartHubError):
    """Raised when the router is already configured."""


@callback
def get_addon_manager(hass: SmartHub, slug: str) -> AddonManager:
    """Get the add-on manager."""
    return AddonManager(hass, _LOGGER, "OpenThread Border Router", slug)


def _is_yellow(hass: SmartHub) -> bool:
    """Return True if SmartHub is running on a SmartHub Yellow."""
    try:
...
async def _title(hass: SmartHub, discovery_info: HassioServiceInfo) -> str:
    ...
    if _is_yellow(hass) and device == "/dev/ttyAMA1":
        return f"SmartHub Yellow ({discovery_info.name})"

    if device and "SkyConnect" in device:
        return f"SmartHub SkyConnect ({discovery_info.name})"

    if device and "Connect_ZBT-1" in device:
        return f"SmartHub Connect ZBT-1 ({discovery_info.name})"
