...
class GetBorderAgentIdNotSupported(SmartHubError):
...
def _handle_otbr_error[**_P, _R](
    ...
    async def _func(self: OTBRData, *args: _P.args, **kwargs: _P.kwargs) -> _R:
        try:
            ...
            raise SmartHubError("Failed to call OTBR API") from exc
...
class OTBRData:
    ...
    @_handle_otbr_error
    async def factory_reset(self, hass: SmartHub) -> None:
        """Reset the router."""
        try:
...
async def get_allowed_channel(hass: SmartHub, otbr_url: str) -> int | None:
...
async def _warn_on_channel_collision(
    hass: SmartHub, otbrdata: OTBRData, dataset_tlvs: bytes
) -> None:
...
async def update_unique_id(
    hass: SmartHub, entry: OTBRConfigEntry | None, border_agent_id: bytes
) -> None:
