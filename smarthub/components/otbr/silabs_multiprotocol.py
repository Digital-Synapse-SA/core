...
def async_get_otbr_data[**_P, _R, _R_Def](
    ...
    [Callable[Concatenate[SmartHub, OTBRData, _P], Coroutine[Any, Any, _R]]],
    Callable[Concatenate[SmartHub, _P], Coroutine[Any, Any, _R | _R_Def]],
]:
    """Decorate function to get OTBR data."""

    def _async_get_otbr_data(
        orig_func: Callable[
            Concatenate[SmartHub, OTBRData, _P],
...
@async_get_otbr_data(None)
async def async_get_channel(hass: SmartHub, data: OTBRData) -> int | None:
    """Return the channel.
...
async def async_using_multipan(hass: SmartHub, data: OTBRData) -> bool:
