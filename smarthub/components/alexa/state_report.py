...
from smarthub.components import event
from smarthub.const import EVENT_STATE_CHANGED, STATE_ON
...
from smarthub.helpers.significant_change import create_checker
...
class AlexaDirective:
    ...
    def load_entity(self, hass: SmartHub, config: AbstractConfig) -> None:
...
async def async_enable_proactive_mode(
    hass: SmartHub, smart_home_config: AbstractConfig
) -> CALLBACK_TYPE | None:
...
async def async_send_add_or_update_message(
    hass: SmartHub, config: AbstractConfig, entity_ids: list[str]
...
async def async_send_doorbell_event_message(
    hass: SmartHub, config: AbstractConfig, alexa_entity: AlexaEntity
) -> None:
