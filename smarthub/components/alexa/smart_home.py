...
async def async_setup(hass: SmartHub, config: ConfigType) -> None:
...
class SmartHomeView(SmartHubView):
    ...
    async def post(self, request: SmartHubRequest) -> web.Response | bytes:
    ...
    try:
        if not enabled:
            raise AlexaBridgeUnreachableError(  # noqa: TRY301
                "Alexa API not enabled in SmartHub configuration"
            )
