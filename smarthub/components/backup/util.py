...
from smarthub.backup_restore import password_to_key
from smarthub.core import SmartHub
from smarthub.exceptions import SmartHubError
from smarthub.util import dt as dt_util
from smarthub.util.json import JsonObjectType, json_loads_object

from .const import BUF_SIZE, LOGGER
from .models import AddonInfo, AgentBackup, Folder


class DecryptError(SmartHubError):
    """Error during decryption."""

    _message = "Unexpected error during decryption."


class EncryptError(SmartHubError):
...
class AbortCipher(SmartHubError):
...
def read_backup(backup_path: Path) -> AgentBackup:
    ...
    with tarfile.open(backup_path, "r:", bufsize=BUF_SIZE) as backup_file:
        ...
        smarthub_version: str | None = None
        database_included = False
        if (
            smarthub := cast(JsonObjectType, data.get("smarthub"))
        ) and "version" in smarthub:
            smarthub_included = True
            smarthub_version = cast(str, smarthub["version"])
            database_included = not cast(
                bool, smarthub.get("exclude_database", False)
            )
        ...
        return AgentBackup(
            ...
            smarthub_version=smarthub_version,
...
def validate_password(path: Path, password: str | None) -> bool:
    ...
    with tarfile.open(path, "r:", bufsize=BUF_SIZE) as backup_file:
        compressed = False
        ha_tar_name = "smarthub.tar"
        try:
            ha_tar = backup_file.extractfile(ha_tar_name)
        except KeyError:
            compressed = True
            ha_tar_name = "smarthub.tar.gz"
            try:
                ha_tar = backup_file.extractfile(ha_tar_name)
            except KeyError:
                LOGGER.error("No smarthub.tar or smarthub.tar.gz found")
                return False
        try:
            with SecureTarFile(
                path,  # Not used
...
class AsyncIteratorReader:
    ...
    def __init__(self, hass: SmartHub, stream: AsyncIterator[bytes]) -> None:
...
def _get_expected_archives(backup: AgentBackup) -> set[str]:
    ...
    if backup.smarthub_included:
        expected_archives.add("smarthub")
...
class _CipherBackupStreamer:
    ...
    def _num_tar_files(self) -> int:
        ...
        b = self._backup
        return len(b.addons) + len(b.folders) + b.smarthub_included + 1
...
async def receive_file(
    hass: SmartHub, contents: aiohttp.BodyPartReader, path: Path
) -> None:
