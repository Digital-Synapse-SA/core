...
def refresh_library(hass: SmartHub, service_call: ServiceCall) -> None:
...
def get_plex_server(
    ...
    if DOMAIN not in hass.data:
        raise SmartHubError("Plex integration not configured")
    servers: dict[str, PlexServer] = get_plex_data(hass)[SERVERS]
    if not servers:
        raise SmartHubError("No Plex servers available")
...
def process_plex_payload(
    ...
    if content_type == "station":
        if not supports_playqueues:
            raise SmartHubError("Plex stations are not supported on this device")
    ...
    if playqueue_id := content.pop("playqueue_id", None):
        if not supports_playqueues:
            raise SmartHubError("Plex playqueues are not supported on this device")
        try:
