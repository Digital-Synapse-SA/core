...
from smarthub.core import CALLBACK_TYPE, Context, SmartHub, State, callback
...
from smarthub.helpers.redact import partial_redact
...
class GoogleEntity:
    ...
    def __init__(
        self, hass: SmartHub, config: AbstractConfig, state: State
    ) -> None:
    ...
    def sync_serialize(self, agent_user_id, instance_uuid):
        ...
        if "matter" in self.hass.config.components and any(
            ...
            from smarthub.components.matter import (  # noqa: PLC0415
    ...
    def async_update(self):
        """Update the entity with latest info from SmartHub."""
...
def async_get_google_entity_if_supported_cached(
    hass: SmartHub, config: AbstractConfig, state: State
) -> GoogleEntity | None:
...
def async_get_entities(
    hass: SmartHub, config: AbstractConfig
) -> list[GoogleEntity]:
