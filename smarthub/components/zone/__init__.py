...
from smarthub.util.location import distance
...
def async_active_zone(
    hass: SmartHub, latitude: float, longitude: float, radius: float = 0
) -> State | None:
...
@callback
def async_setup_track_zone_entity_ids(hass: SmartHub) -> None:
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    """Set up configured zones as well as SmartHub zone if necessary."""
...
@callback
def _home_conf(hass: SmartHub) -> dict:
