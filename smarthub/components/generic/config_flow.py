...
from smarthub.helpers import config_validation as cv, template as template_helper
...
class InvalidStreamException(SmartHubError):
...
async def async_test_still(
    hass: SmartHub, info: Mapping[str, Any]
...
def slug(
    hass: SmartHub, template: str | template_helper.Template | None
) -> str | None:
...
def register_still_preview(hass: SmartHub) -> None:
...
class CameraImagePreview(SmartHubView):
