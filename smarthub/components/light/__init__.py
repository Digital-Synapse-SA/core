...
from smarthub.helpers.deprecation import (
    DeprecatedConstant,
...
from smarthub.helpers.entity import ToggleEntity, ToggleEntityDescription
from smarthub.helpers.entity_component import EntityComponent
from smarthub.helpers.frame import ReportBehavior, report_usage
from smarthub.helpers.typing import ConfigType, VolDictType
...
# These SUPPORT_* constants are deprecated as of SmartHub 2022.5.
...
# These COLOR_MODE_* constants are deprecated as of SmartHub 2022.5.
# Please use the LightEntityFeature enum instead.
...
def get_supported_color_modes(hass: SmartHub, entity_id: str) -> set[str] | None:
...
def preprocess_turn_on_alternatives(
    hass: SmartHub, params: dict[str, Any]
) -> None:
    ...
    if (mired := params.pop(_DEPRECATED_ATTR_COLOR_TEMP.value, None)) is not None:
        _LOGGER.warning(
            ...
            "and will break in SmartHub 2026.1, please use "
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:  # noqa: C901
    ...
    async def async_handle_light_on_service(  # noqa: C901
        ...
        # Add a warning in SmartHub Core 2024.3 if the brightness is set to an
        # integer.
...
class LightEntity(ToggleEntity, cached_properties=CACHED_PROPERTIES_WITH_ATTR_):
    ...
    # Deprecated, see https://github.com/smart-hub/core/pull/79591
            ...
            if not self.__color_mode_reported and self.__should_report_light_issue():
                ...
                _LOGGER.warning(
                    (
                        ...
                        "in SmartHub Core 2025.3, please %s"
                    ),
                    self.entity_id,
                    type(self),
                    report_issue,
    ...
    def color_temp_kelvin(self) -> int | None:
        ...
        if self._attr_color_temp_kelvin is None and (color_temp := self.color_temp):
            report_usage(
                ...
                "https://github.com/smart-hub/core/pull/79591)",
    ...
    def min_color_temp_kelvin(self) -> int:
        ...
        if self._attr_min_color_temp_kelvin is None:
            report_usage(
                ...
                "(see https://github.com/smart-hub/core/pull/79591)",
                breaks_in_ha_version="2026.1",
                core_behavior=ReportBehavior.LOG,
                integration_domain=self.platform.platform_name
                if self.platform
            ...
            if not self.__color_mode_reported and self.__should_report_light_issue():
                ...
                _LOGGER.warning(
                    (
                        ...
                        "this will stop working in SmartHub Core 2025.3, "
                        "please %s"
                    ),
                    self.entity_id,
                    type(self),
        ...
        if not self.__color_mode_reported and self.__should_report_light_issue():
            ...
            _LOGGER.warning(
                (
                    ...
                    " expected one of %s, this will stop working in SmartHub "
            ...
            if not self.__color_mode_reported and self.__should_report_light_issue():
                ...
                _LOGGER.warning(
                    (
                        ...
                        "working in SmartHub Core 2025.3, please %s"
                    ),
                    self.entity_id,
                    type(self),
                    supported_color_modes,
