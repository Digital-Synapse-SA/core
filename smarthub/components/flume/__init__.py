...
from smarthub.helpers.selector import ConfigEntrySelector
...
def _setup_entry(
    hass: SmartHub, entry: FlumeConfigEntry
...
async def async_setup_entry(hass: SmartHub, entry: FlumeConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: FlumeConfigEntry) -> bool:
    """Unload a config entry."""
    entry.runtime_data.http_session.close()
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


def setup_service(hass: SmartHub) -> None:
