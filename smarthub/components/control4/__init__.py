...
from smarthub.helpers import aiohttp_client, device_registry as dr
...
async def async_setup_entry(hass: SmartHub, entry: Control4ConfigEntry) -> bool:
...
async def update_listener(
    hass: SmartHub, config_entry: Control4ConfigEntry
) -> None:
...
async def async_unload_entry(hass: SmartHub, entry: Control4ConfigEntry) -> bool:
    """Unload a config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def get_items_of_category(
    hass: SmartHub, entry: Control4ConfigEntry, category: str
):
