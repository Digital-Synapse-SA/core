"""SmartHub wrapper for a pyWeMo device."""
...
from smarthub.helpers.device_registry import CONNECTION_UPNP, DeviceInfo
...
class DeviceCoordinator(DataUpdateCoordinator[None]):
    ...
    def __init__(
        self, hass: SmartHub, config_entry: ConfigEntry, wemo: WeMoDevice
    ) -> None:
    ...
    def should_poll(self) -> bool:
        ...
        "push updates" to update the device state in SmartHub.
        """
...
async def async_register_device(
    hass: SmartHub, config_entry: ConfigEntry, wemo: WeMoDevice
) -> DeviceCoordinator:
...
@callback
def async_get_coordinator(hass: SmartHub, device_id: str) -> DeviceCoordinator:
    """Return DeviceCoordinator for device_id."""
    return _async_coordinators(hass)[device_id]


@callback
def _async_coordinators(hass: SmartHub) -> dict[str, DeviceCoordinator]:
    return hass.data[DATA_WEMO].config_entry_data.device_coordinators


@callback
def _async_registry(hass: SmartHub) -> SubscriptionRegistry:
