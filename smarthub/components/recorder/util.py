...
from smarthub.helpers.recorder import (  # noqa: F401
    DATA_INSTANCE,
    get_instance,
    session_scope,
)
...
def _fail_unsupported_dialect(dialect_name: str) -> NoReturn:
    ...
    _LOGGER.error(
        (
            "Database %s is not supported; SmartHub supports %s. "
            "Starting with SmartHub 2022.6 this prevents the recorder from "
...
def _async_create_mariadb_range_index_regression_issue(
    hass: SmartHub, version: AwesomeVersion
) -> None:
...
def async_create_backup_failure_issue(
    ...
    ir.async_create_issue(
        ...
        is_fixable=False,
        severity=ir.IssueSeverity.CRITICAL,
        learn_more_url="https://www.smart-hub.io/integrations/recorder",
    ...
    if dialect_name == SupportedDialect.SQLITE:
        ...
        if first_connection:
            ...
            if "mariadb" in version_string.lower():
                ...
                # MySQL
                # https://github.com/smart-hub/core/issues/137178
        ...
        # https://github.com/smart-hub/core/issues/126084
...
def async_migration_in_progress(hass: SmartHub) -> bool:
...
def async_migration_is_live(hass: SmartHub) -> bool:
    ...
    def _filter_unique_constraint_integrity_error(err: Exception) -> bool:
        ...
        if ignore:
            _LOGGER.warning(
                ...
                row_type,
                "https://github.com/smart-hub/core/issues?q=is%3Aopen+is%3Aissue+label%3A%22integration%3A+recorder%22",
                exc_info=err,
            )
