...
from smarthub.components import conversation, media_source, stt, tts
...
from smarthub.components.media_player import async_process_play_media_url
from smarthub.core import Context, callback
from smarthub.exceptions import SmartHubError
from smarthub.helpers import chat_session, entity
...
class AssistSatelliteState(StrEnum):
    ...
    LISTENING = "listening"
    """Device is streaming audio with the voice command to SmartHub."""

    PROCESSING = "processing"
    """SmartHub is processing the voice command."""
...
class AssistSatelliteEntity(entity.Entity):
    ...
    async def async_internal_start_conversation(
        ...
        # The SmartHub built-in agent doesn't support conversations.
    ...
    async def async_internal_ask_question(
        ...
        try:
            ...
            if response_text is None:
                raise SmartHubError("No answer from question")
    ...
    async def async_accept_pipeline_from_satellite(
        ...
        """Triggers an Assist pipeline in SmartHub from a satellite."""
        await self._cancel_running_pipeline()
    ...
    async def _resolve_announcement_media_id(
        ...
        if media_id:
            ...
            if engine is None:
                raise SmartHubError(f"TTS engine {pipeline.tts_engine} not found")
