...
def async_last_update_was_successful(
    hass: SmartHub, entry: UFPConfigEntry
) -> bool:
...
class ProtectData:
    ...
    def _async_process_ws_message(self, message: WSSubscriptionMessage) -> None:
        ...
        if model_type is ModelType.LIVEVIEW and len(self.api.bootstrap.viewers) > 0:
            ...
            _LOGGER.warning(
                "Liveviews updated. Restart SmartHub to update Viewport select"
                " options"
            )
            return
...
def async_ufp_instance_for_config_entry_ids(
    hass: SmartHub, config_entry_ids: set[str]
...
@callback
def async_get_ufp_entries(hass: SmartHub) -> list[UFPConfigEntry]:
...
@callback
def async_get_data_for_nvr_id(hass: SmartHub, nvr_id: str) -> ProtectData | None:
