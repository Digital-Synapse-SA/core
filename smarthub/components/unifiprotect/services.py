...
from smarthub.const import ATTR_DEVICE_ID, ATTR_NAME, Platform
...
from smarthub.helpers.service import async_extract_referenced_entity_ids
...
@callback
def _async_get_ufp_instance(hass: SmartHub, device_id: str) -> ProtectApiClient:
...
async def _async_service_call_nvr(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        raise SmartHubError(str(err)) from err
...
async def get_user_keyring_info(call: ServiceCall) -> ServiceResponse:
    ...
    if not ulp_users:
        raise SmartHubError("No users found, please check Protect permissions.")
