...
@callback
def async_get_device_id_from_entity_id(hass: SmartHub, entity_id: str) -> str:
    """Get device ID from an entity ID.

    Raises SmartHubError if entity or device ID is invalid.
    """
...
def async_get_client_by_device_entry(
    hass: SmartHub, device: DeviceEntry
) -> WebOsClient:
...
def update_client_key(hass: SmartHub, entry: WebOsTvConfigEntry) -> None:
