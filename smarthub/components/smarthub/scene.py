...
from smarthub import config as conf_util
from smarthub.components.light import ATTR_TRANSITION
from smarthub.components.scene import DOMAIN as SCENE_DOMAIN, STATES, Scene
...
from smarthub.core import SmartHub, ServiceCall, State, callback
from smarthub.exceptions import SmartHubError, ServiceValidationError
from smarthub.helpers import config_validation as cv, entity_platform
from smarthub.helpers.entity_platform import AddEntitiesCallback, EntityPlatform
...
@callback
def scenes_with_entity(hass: SmartHub, entity_id: str) -> list[str]:
    ...
    scene_entities = cast(ValuesView[SmartHubScene], platform.entities.values())
    return [
...
@callback
def entities_in_scene(hass: SmartHub, entity_id: str) -> list[str]:
    ...
    return list(cast(SmartHubScene, entity).scene_config.states)
...
async def async_setup_platform(
    ...
    async def reload_config(call: ServiceCall) -> None:
        ...
        # Extract only the config for the SmartHub platform, ignore the rest.
    ...
    async def create_service(call: ServiceCall) -> None:
        ...
        if (old := platform.entities.get(entity_id)) is not None:
            if not isinstance(old, SmartHubScene) or not old.from_service:
                _LOGGER.warning("The scene %s already exists", entity_id)
                return
            await platform.async_remove_entity(entity_id)
        async_add_entities([SmartHubScene(hass, scene_config, from_service=True)])
    ...
    async def delete_service(call: ServiceCall) -> None:
        ...
        for entity_id in entity_ids:
            ...
            assert isinstance(scene, SmartHubScene)
...
def _process_scenes_config(
    hass: SmartHub, async_add_entities: AddEntitiesCallback, config: dict[str, Any]
) -> None:
...
class SmartHubScene(Scene):
    """A scene is a group of entities and the states we want them to be."""

    def __init__(
        self, hass: SmartHub, scene_config: SceneConfig, from_service: bool = False
    ) -> None:
