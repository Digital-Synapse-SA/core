...
from smarthub import config as conf_util, core_config
from smarthub.auth.permissions.const import CAT_ENTITIES, POLICY_CONTROL
...
from smarthub.exceptions import SmartHubError, Unauthorized, UnknownUser
...
from smarthub.helpers.issue_registry import IssueSeverity
from smarthub.helpers.service import (
    async_extract_config_entry_ids,
    async_extract_referenced_entity_ids,
    async_register_admin_service,
)
from smarthub.helpers.signal import KEY_HA_STOP
from smarthub.helpers.system_info import async_get_system_info
from smarthub.helpers.template import async_load_custom_templates
...
DEPRECATION_URL = (
    "https://www.smart-hub.io/blog/2025/05/22/"
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:  # noqa: C901
    """Set up general services related to SmartHub."""

    async def async_save_persistent_states(service: ServiceCall) -> None:
        """Handle calls to smarthub.save_persistent_states."""
    ...
    async def async_handle_turn_service(service: ServiceCall) -> None:
        """Handle calls to smarthub.turn_on/off."""
        ...
        if not all_referenced:
            _LOGGER.error(
                "The service smarthub.%s cannot be called without a target",
                service.service,
            )
            return
        ...
        for domain, ent_ids in by_domain:
            ...
            if domain == DOMAIN:
                _LOGGER.warning(
                    "Called service smarthub.%s with invalid entities %s",
                    service.service,
                    ", ".join(ent_ids),
                )
                continue
        ...
        if unsupported_entities:
            _LOGGER.warning(
                "The service smarthub.%s does not support entities %s",
                service.service,
    ...
    async def async_handle_core_service(call: ServiceCall) -> None:
        ...
        stop_handler: Callable[[SmartHub, bool], Coroutine[Any, Any, None]]
    ...
    async def async_handle_reload_all(call: ServiceCall) -> None:
        ...
        `smarthub.reload_custom_templates` service that reloads any custom
        jinja into memory.
...
async def _async_stop(hass: SmartHub, restart: bool) -> None:
    """Stop home assistant."""
