...
from smarthub.components.light import ATTR_RGBW_COLOR, ColorMode, LightEntity
...
class MBLight(MicroBeesActuatorEntity, LightEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if not sendCommand:
            raise SmartHubError(f"Failed to turn on {self.name}")
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        if not sendCommand:
            raise SmartHubError(f"Failed to turn off {self.name}")
