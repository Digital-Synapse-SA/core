...
from smarthub.components.ffmpeg import CONF_EXTRA_ARGUMENTS
from smarthub.components.stream import CONF_RTSP_TRANSPORT, RTSP_TRANSPORTS
...
async def async_populate_snapshot_auth(
    hass: SmartHub, device: ONVIFDevice, entry: ConfigEntry
) -> None:
...
async def async_populate_options(hass: SmartHub, entry: ConfigEntry) -> None:
...
def _async_migrate_camera_entities_unique_ids(
    hass: SmartHub, config_entry: ConfigEntry, device: ONVIFDevice
) -> None:
