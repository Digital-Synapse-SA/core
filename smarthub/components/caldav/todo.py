...
class WebDavTodoListEntity(TodoListEntity):
    ...
    async def async_create_todo_item(self, item: TodoItem) -> None:
        ...
        try:
            ...
            raise SmartHubError(f"CalDAV save error: {err}") from err
    ...
    async def async_update_todo_item(self, item: TodoItem) -> None:
        ...
        try:
            ...
            raise SmartHubError(f"Could not find To-do item {uid}") from err
        except (requests.ConnectionError, DAVError) as err:
            raise SmartHubError(f"CalDAV lookup error: {err}") from err
    ...
    async def async_delete_todo_items(self, uids: list[str]) -> None:
        ...
        try:
            ...
            raise SmartHubError("Could not find To-do item") from err
        except (requests.ConnectionError, DAVError) as err:
            raise SmartHubError(f"CalDAV lookup error: {err}") from err
        ...
        for item in items:
            try:
                ...
                raise SmartHubError(f"CalDAV delete error: {err}") from err
        # refreshing async otherwise it would take too much time
        self.hass.async_create_task(self.async_update_ha_state(force_refresh=True))
