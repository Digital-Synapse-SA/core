...
from smarthub.components.fan import FanEntity, FanEntityFeature
...
from smarthub.util.percentage import (
    percentage_to_ranged_value,
    ranged_value_to_percentage,
)
from smarthub.util.scaling import int_states_in_range
...
class VeSyncFanHA(VeSyncBaseEntity, FanEntity):
    ...
    def set_percentage(self, percentage: int) -> None:
        ...
        if percentage == 0:
            ...
            if not success:
                raise SmartHubError("An error occurred while turning off.")
        elif not self.device.is_on:
            success = self.device.turn_on()
            if not success:
                raise SmartHubError("An error occurred while turning on.")

        success = self.device.manual_mode()
        if not success:
            raise SmartHubError("An error occurred while manual mode.")
        ...
        if not success:
            raise SmartHubError("An error occurred while changing fan speed.")
    ...
    def set_preset_mode(self, preset_mode: str) -> None:
        ...
        if not success:
            raise SmartHubError("An error occurred while setting preset mode.")
