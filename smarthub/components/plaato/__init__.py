...
@callback
def async_setup_webhook(hass: SmartHub, entry: ConfigEntry):
...
async def async_setup_coordinator(hass: SmartHub, entry: ConfigEntry):
...
async def async_unload_webhook(hass: SmartHub, entry: ConfigEntry):
...
async def async_unload_coordinator(hass: SmartHub, entry: ConfigEntry):
    """Unload auth token based entry."""
    coordinator = hass.data[DOMAIN][entry.entry_id][COORDINATOR]
    return await async_unload_platforms(hass, entry, coordinator.platforms)


async def async_unload_platforms(hass: SmartHub, entry: ConfigEntry, platforms):
    """Unload platforms."""
...
async def _async_update_listener(hass: SmartHub, entry: ConfigEntry) -> None:
