...
from smarthub.const import ATTR_DEVICE_ID, ATTR_ID, ATTR_NAME
...
@callback
def async_device_supports_timers(hass: SmartHub, device_id: str) -> bool:
    """Return True if device has been registered to handle timer events."""
...
def async_register_timer_handler(
    hass: SmartHub, device_id: str, handler: TimerHandler
) -> Callable[[], None]:
...
def _find_timers(
    hass: SmartHub, device_id: str | None, slots: dict[str, Any]
) -> list[TimerInfo]:
