...
from smarthub.components import http, sensor
...
from smarthub.components.input_button import DOMAIN as INPUT_BUTTON_DOMAIN
...
class IntentPlatformProtocol(Protocol):
    ...
    async def async_setup_intents(self, hass: SmartHub) -> None:
...
class OnOffIntentHandler(intent.ServiceIntentHandler):
    ...
    async def async_call_service(
        ...
        # Fall back to smarthub.turn_on/off
...
async def _async_process_intent(
    hass: SmartHub, domain: str, platform: IntentPlatformProtocol
) -> None:
    """Process the intents of an integration."""
    await platform.async_setup_intents(hass)


class IntentHandleView(http.SmartHubView):
