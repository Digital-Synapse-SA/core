...
from smarthub.const import ATTR_VIA_DEVICE, EVENT_HOMEASSISTANT_STARTED
from smarthub.core import CALLBACK_TYPE, CoreState, Event, SmartHub, callback
...
from smarthub.helpers.event import async_call_later, async_track_time_interval
...
class HKDevice:
    ...
    async def async_setup(self) -> None:
        """Prepare to use a paired HomeKit device in SmartHub."""
    ...
    async def put_characteristics(
        ...
        """Control a HomeKit device state from SmartHub."""
    ...
    async def async_thread_provision(self) -> None:
        ...
        if self.pairing.controller.transport_type == TransportType.COAP:
            raise SmartHubError("Already connected to a thread network")

        if not (dataset := await async_get_preferred_dataset(self.hass)):
            raise SmartHubError("No thread network credentials available")
        ...
        try:
            ...
            raise SmartHubError("Could not migrate device to Thread") from exc
