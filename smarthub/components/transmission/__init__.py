...
async def async_setup_entry(
    hass: SmartHub, config_entry: TransmissionConfigEntry
) -> bool:
    """Set up the Transmission Component."""
...
def _get_coordinator_from_service_data(
    ...
    if entry is None or entry.state is not ConfigEntryState.LOADED:
        raise SmartHubError(f"Config entry {entry_id} is not found or not loaded")
    return entry.runtime_data


def setup_hass_services(hass: SmartHub) -> None:
    """SmartHub services."""
...
async def get_api(
    hass: SmartHub, entry: dict[str, Any]
