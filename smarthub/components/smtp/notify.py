...
from smarthub.helpers.reload import setup_reload_service
...
def _attach_file(
    hass: SmartHub, atch_name: str, content_id: str | None = None
    ...
    try:
        ...
        if os.path.exists(file_path) and not hass.config.is_allowed_path(
            ...
            url = "https://www.smart-hub.io/docs/configuration/basic/"
...
def _build_multipart_msg(
    hass: SmartHub, message: str, images: list[str]
) -> MIMEMultipart:
...
def _build_html_msg(
    hass: SmartHub, text: str, html: str, images: list[str]
) -> MIMEMultipart:
