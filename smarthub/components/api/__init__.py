...
from smarthub.auth.permissions.const import POLICY_READ
from smarthub.components.http import (
    KEY_HASS,
    KEY_HASS_USER,
    SmartHubView,
    require_admin,
...
from smarthub.core import Event, EventStateChangedData, SmartHub
...
from smarthub.helpers import config_validation as cv, recorder, template
from smarthub.helpers.json import json_dumps, json_fragment
...
class APIStatusView(SmartHubView):
...
class APICoreStateView(SmartHubView):
    ...
    def get(self, request: web.Request) -> web.Response:
        ...
        SmartHub core is running. Its primary use case is for supervisor
        to check if SmartHub is running.
        """
...
class APIEventStream(SmartHubView):
...
class APIConfigView(SmartHubView):
...
class APIStatesView(SmartHubView):
...
class APIEntityStateView(SmartHubView):
...
class APIEventListenersView(SmartHubView):
...
class APIEventView(SmartHubView):
...
class APIServicesView(SmartHubView):
...
class APIDomainServicesView(SmartHubView):
...
class APIComponentsView(SmartHubView):
...
@lru_cache
def _cached_template(template_str: str, hass: SmartHub) -> template.Template:
    """Return a cached template."""
    return template.Template(template_str, hass)


class APITemplateView(SmartHubView):
...
class APIErrorLog(SmartHubView):
...
async def async_services_json(hass: SmartHub) -> list[dict[str, Any]]:
...
@ha.callback
def async_events_json(hass: SmartHub) -> list[dict[str, Any]]:
