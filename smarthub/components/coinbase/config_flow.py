...
from smarthub.config_entries import ConfigFlow, ConfigFlowResult, OptionsFlow
...
async def validate_api(hass: SmartHub, data):
...
async def validate_options(
    hass: SmartHub, config_entry: CoinbaseConfigEntry, options
):
...
class InvalidSecret(SmartHubError):
    """Error to indicate auth failed due to invalid secret."""


class InvalidKey(SmartHubError):
...
class CurrencyUnavailable(SmartHubError):
    """Error to indicate the requested currency resource is not provided by the API."""


class ExchangeRateUnavailable(SmartHubError):
