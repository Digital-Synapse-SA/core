...
from smarthub.components.media_player import MediaPlayerState, MediaType
from smarthub.config_entries import SOURCE_IMPORT, ConfigEntry
from smarthub.const import CONF_HOST, CONF_NAME, CONF_PORT, Platform
...
async def async_setup_entry(hass: SmartHub, config_entry: ConfigEntry) -> bool:
    ...
    if ATTR_DEVICE_INFO not in config or config[ATTR_DEVICE_INFO] is None:
        ...
        if device_info is None:
            _LOGGER.error(
                "Couldn't gather device info; Please restart SmartHub with your"
