...
async def async_setup_entry(hass: SmartHub, entry: ThinqConfigEntry) -> bool:
    """Set up an entry."""
...
@callback
def async_cleanup_device_registry(hass: SmartHub, entry: ThinqConfigEntry) -> None:
...
async def async_setup_mqtt(
    hass: SmartHub, entry: ThinqConfigEntry, thinq_api: ThinQApi, client_id: str
) -> None:
    """Set up MQTT connection."""
...
async def async_unload_entry(hass: SmartHub, entry: ThinqConfigEntry) -> bool:
    """Unload the entry."""
