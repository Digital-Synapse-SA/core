...
from smarthub.const import CONF_API_KEY, CONF_URL, Platform
...
def _migrate_unique_id(
    hass: SmartHub, entry: EmonCMSConfigEntry, emoncms_unique_id: str
) -> None:
...
async def _check_unique_id_migration(
    hass: SmartHub, entry: EmonCMSConfigEntry, emoncms_client: EmoncmsClient
) -> None:
...
async def async_setup_entry(hass: SmartHub, entry: EmonCMSConfigEntry) -> bool:
    """Load a config entry."""
...
async def update_listener(hass: SmartHub, entry: EmonCMSConfigEntry):
    """Handle options update."""
    await hass.config_entries.async_reload(entry.entry_id)


async def async_unload_entry(hass: SmartHub, entry: EmonCMSConfigEntry) -> bool:
