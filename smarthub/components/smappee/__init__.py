...
from smarthub.util import Throttle
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    # decide platform
    platform = "PRODUCTION"
    if client_id == "smarthub_f2":
        platform = "ACCEPTANCE"
    elif client_id == "smarthub_f3":
...
async def async_setup_entry(hass: SmartHub, entry: SmappeeConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: SmappeeConfigEntry) -> bool:
...
class SmappeeBase:
    ...
    def __init__(self, hass: SmartHub, smappee: Smappee) -> None:
