...
from smarthub.components.ffmpeg import get_ffmpeg_manager
from smarthub.components.media_player import BrowseError, MediaClass, MediaType
...
async def async_get_media_event_store(
    hass: SmartHub, subscriber: GoogleNestSubscriber
) -> EventMediaStore:
...
async def async_get_transcoder(hass: SmartHub) -> Transcoder:
...
async def async_get_media_source(hass: SmartHub) -> MediaSource:
    """Set up Nest media source."""
    return NestMediaSource(hass)


@callback
def async_get_media_source_devices(hass: SmartHub) -> Mapping[str, Device]:
