...
from smarthub.components.camera import Image, img_util
from smarthub.components.http import KEY_HASS_USER
...
from smarthub.helpers.entity_registry import async_entries_for_device
...
async def async_setup_entry(hass: SmartHub, entry: NestConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: NestConfigEntry) -> bool:
    """Unload a config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_remove_entry(hass: SmartHub, entry: NestConfigEntry) -> None:
...
class NestEventViewBase(SmartHubView, ABC):
    ...
    async def get(
        ...
        try:
            ...
            raise SmartHubError("Unable to fetch media for event") from err
        if not media:
            return self._json_error(
