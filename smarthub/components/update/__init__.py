...
from smarthub.const import ATTR_ENTITY_PICTURE, STATE_OFF, STATE_ON, EntityCategory
from smarthub.core import SmartHub, ServiceCall
from smarthub.exceptions import SmartHubError
from smarthub.helpers import config_validation as cv
from smarthub.helpers.entity import ABCCachedProperties, EntityDescription
...
async def async_install(entity: UpdateEntity, service_call: ServiceCall) -> None:
    ...
    if (version := service_call.data.get(ATTR_VERSION)) is None and (
        ...
        raise SmartHubError(f"No update available for {entity.entity_id}")
    ...
    if (
        ...
        raise SmartHubError(f"Backup is not supported for {entity.entity_id}")
...
class UpdateEntity(
    ...
    def entity_picture(self) -> str | None:
        ...
        """
        return (
            f"https://brands.smart-hub.io/_/{self.platform.platform_name}/icon.png"
        )
    ...
    async def async_skip(self) -> None:
        ...
        if (latest_version := self.latest_version) is None:
            raise SmartHubError(f"Cannot skip an unknown version for {self.name}")
        if self.installed_version == latest_version:
            raise SmartHubError(f"No update available to skip for {self.name}")
