...
from smarthub.components.http.auth import async_sign_path
...
def async_process_play_media_url(
    ...
    *,
    ...
    """Update a media URL with authentication if it points at SmartHub."""
    ...
    if not parsed.is_absolute() and not allow_relative_url:
        ...
        if not base_url:
            try:
                ...
                msg = "Unable to determine SmartHub URL to send to device"
                if (
                    hass.config.api
