...
from smarthub.components.http import KEY_AUTHENTICATED, SmartHubView
...
from smarthub.core import Event, SmartHub, ServiceCall, callback
...
from smarthub.helpers.entity import Entity, EntityDescription
...
# These constants are deprecated as of SmartHub 2024.10
...
@bind_hass
async def async_request_stream(hass: SmartHub, entity_id: str, fmt: str) -> str:
...
async def _async_get_image(
    ...
    raise SmartHubError("Unable to get image")
...
@bind_hass
async def async_get_stream_source(hass: SmartHub, entity_id: str) -> str | None:
...
async def async_get_mjpeg_stream(
    hass: SmartHub, request: web.Request, entity_id: str
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    def get_ice_servers() -> list[RTCIceServer]:
        ...
        return [
            RTCIceServer(
                urls=[
                    "stun:stun.smart-hub.io:80",
                    "stun:stun.smart-hub.io:3478",
                ]
            ),
        ]
...
class Camera(Entity, cached_properties=CACHED_PROPERTIES_WITH_ATTR_):
    ...
    async def async_handle_async_webrtc_offer(
        ...
        raise SmartHubError("Camera does not support WebRTC")
    ...
    async def _async_get_supported_webrtc_provider[_T](
        self, fn: Callable[[SmartHub, Camera], Coroutine[None, None, _T | None]]
    ) -> _T | None:
    ...
    async def async_on_webrtc_candidate(
        ...
        if self._webrtc_provider:
            ...
            raise SmartHubError("Cannot handle WebRTC candidate")
...
class CameraImageView(CameraView):
    ...
    async def handle(self, request: web.Request, camera: Camera) -> web.Response:
        ...
        except (SmartHubError, ValueError) as ex:
...
async def async_handle_snapshot_service(
    ...
    try:
        ...
        raise SmartHubError(f"Can't write image to file: {err}") from err
...
async def _async_stream_endpoint_url(
    hass: SmartHub, camera: Camera, fmt: str
) -> str:
    stream = await camera.async_create_stream()
    if not stream:
        raise SmartHubError(
...
async def async_handle_record_service(
    ...
    if not stream:
        raise SmartHubError(f"{camera.entity_id} does not support record service")
