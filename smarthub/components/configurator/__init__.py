...
from smarthub.const import ATTR_ENTITY_PICTURE, ATTR_FRIENDLY_NAME
...
@bind_hass
def request_config(hass: SmartHub, *args: Any, **kwargs: Any) -> str:
    """Create a new request for configuration.
...
@bind_hass
@async_callback
def async_notify_errors(hass: SmartHub, request_id: str, error: str) -> None:
...
@bind_hass
def notify_errors(hass: SmartHub, request_id: str, error: str) -> None:
    """Add errors to a config request."""
    return run_callback_threadsafe(
...
@bind_hass
@async_callback
def async_request_done(hass: SmartHub, request_id: str) -> None:
...
@bind_hass
def request_done(hass: SmartHub, request_id: str) -> None:
    """Mark a configuration request as done."""
    return run_callback_threadsafe(
...
def _get_requests(hass: SmartHub) -> dict[str, Configurator]:
