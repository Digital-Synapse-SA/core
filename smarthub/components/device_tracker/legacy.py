...
from smarthub.helpers.typing import ConfigType, GPSType, StateType
...
@callback
def async_setup_integration(hass: SmartHub, config: ConfigType) -> None:
        ...
        with async_start_setup(
            ...
            try:
                ...
                if hasattr(self.platform, "async_get_scanner"):
                    ...
                    raise SmartHubError("Invalid legacy device_tracker platform.")  # noqa: TRY301
...
async def async_create_platform_type(
    hass: SmartHub, config: ConfigType, p_type: str, p_config: dict
...
async def get_tracker(hass: SmartHub, config: ConfigType) -> DeviceTracker:
    """Create a tracker."""
...
class DeviceTracker:
    ...
    async def async_see(
        ...
        if mac is None and dev_id is None:
            raise SmartHubError("Neither mac or device id passed in")
        if mac is not None:
            mac = str(mac).upper()
...
async def async_load_config(
    path: str, hass: SmartHub, consider_home: timedelta
) -> list[Device]:
...
def remove_device_from_config(hass: SmartHub, device_id: str) -> None:
