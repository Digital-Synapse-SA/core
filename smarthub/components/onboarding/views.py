...
from smarthub.components.http import KEY_HASS, KEY_HASS_REFRESH_TOKEN_ID
from smarthub.components.http.data_validator import RequestDataValidator
from smarthub.components.http.view import SmartHubView
from smarthub.core import SmartHub, callback
from smarthub.helpers import area_registry as ar, integration_platform
...
from smarthub.setup import async_setup_component, async_wait_component
...
async def async_setup(
    hass: SmartHub, data: OnboardingStoreData, store: OnboardingStorage
) -> None:
...
class OnboardingPlatformProtocol(Protocol):
    ...
    async def async_setup_views(
        self, hass: SmartHub, data: OnboardingStoreData
    ) -> None:
        """Set up onboarding views."""


async def async_process_onboarding_platforms(hass: SmartHub) -> None:
...
async def _register_onboarding_platform(
    hass: SmartHub, integration_domain: str, platform: OnboardingPlatformProtocol
) -> None:
...
class BaseOnboardingView(SmartHubView):
...
class _BaseOnboardingStepView(BaseOnboardingView):
    ...
    async def _async_mark_done(self, hass: SmartHub) -> None:
        """Mark step as done."""
...
class UserOnboardingView(_BaseOnboardingStepView):
    ...
    async def post(self, request: web.Request, data: dict[str, str]) -> web.Response:
        ...
        async with self._lock:
            ...
            # during onboarding.
            from smarthub.components.auth import create_auth_code  # noqa: PLC0415
...
@callback
def _async_get_hass_provider(hass: SmartHub) -> HassAuthProvider:
    """Get the SmartHub auth provider."""
    for prv in hass.auth.auth_providers:
        if prv.type == "smarthub":
            return cast(HassAuthProvider, prv)

    raise RuntimeError("No SmartHub provider found")
