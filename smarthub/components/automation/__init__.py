...
from smarthub.components.blueprint import CONF_USE_BLUEPRINT
...
from smarthub.exceptions import SmartHubError, ServiceNotFound, TemplateError
from smarthub.helpers import condition, config_validation as cv
from smarthub.helpers.entity import ToggleEntity
from smarthub.helpers.entity_component import EntityComponent
from smarthub.helpers.issue_registry import (
    IssueSeverity,
    async_create_issue,
    async_delete_issue,
)
from smarthub.helpers.restore_state import RestoreEntity
...
from smarthub.helpers.script_variables import ScriptVariables
from smarthub.helpers.service import (
    ReloadServiceHelper,
    async_register_admin_service,
)
from smarthub.helpers.trace import (
    TraceElement,
...
from smarthub.util.dt import parse_datetime
...
@bind_hass
def is_on(hass: SmartHub, entity_id: str) -> bool:
...
def _automations_with_x(
    hass: SmartHub, referenced_id: str, property_name: str
) -> list[str]:
...
def _x_in_automation(
    hass: SmartHub, entity_id: str, property_name: str
) -> list[str]:
...
@callback
def blueprint_in_automation(hass: SmartHub, entity_id: str) -> str | None:
...
class AutomationEntity(BaseAutomationEntity, RestoreEntity):
    ...
    async def async_trigger(
        ...
        with trace_automation(
            ...
            try:
                ...
                automation_trace.set_error(err)
            except (vol.Invalid, SmartHubError) as err:
                self._logger.error(
    ...
    async def async_will_remove_from_hass(self) -> None:
        """Remove listeners when removing automation from SmartHub."""
        await super().async_will_remove_from_hass()
        await self._async_disable()
...
async def _create_automation_entities(
    hass: SmartHub, automation_configs: list[AutomationEntityConfig]
...
async def _async_process_if(
    hass: SmartHub, name: str, config: dict[str, Any]
) -> IfAction | None:
    """Process if checks."""
