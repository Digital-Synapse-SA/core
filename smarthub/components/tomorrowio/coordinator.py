...
def async_get_entries_by_api_key(
    hass: SmartHub, api_key: str, exclude_entry: ConfigEntry | None = None
) -> list[ConfigEntry]:
...
def async_set_update_interval(
    hass: SmartHub, api: TomorrowioV4, exclude_entry: ConfigEntry | None = None
) -> timedelta:
...
class TomorrowioDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    def __init__(
        self, hass: SmartHub, config_entry: ConfigEntry, api: TomorrowioV4
    ) -> None:
        """Initialize."""
        self._api = api
