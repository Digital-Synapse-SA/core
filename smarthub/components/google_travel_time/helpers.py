...
from smarthub.helpers.location import find_coordinates
...
def convert_to_waypoint(hass: SmartHub, location: str) -> Waypoint | None:
...
async def validate_config_entry(
    hass: SmartHub, api_key: str, origin: str, destination: str
) -> None:
...
def create_routes_api_disabled_issue(hass: SmartHub, entry: ConfigEntry) -> None:
    """Create an issue for the Routes API being disabled."""
    async_create_issue(
        hass,
        DOMAIN,
        f"routes_api_disabled_{entry.entry_id}",
        learn_more_url="https://www.smart-hub.io/integrations/google_travel_time#setup",
        is_fixable=False,
...
def delete_routes_api_disabled_issue(hass: SmartHub, entry: ConfigEntry) -> None:
