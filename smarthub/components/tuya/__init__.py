...
from smarthub.helpers.dispatcher import dispatcher_send
...
type TuyaConfigEntry = ConfigEntry[SmartHubTuyaData]


class SmartHubTuyaData(NamedTuple):
    """Tuya data stored in the SmartHub data object."""
...
async def async_setup_entry(hass: SmartHub, entry: TuyaConfigEntry) -> bool:
    ...
    entry.runtime_data = SmartHubTuyaData(manager=manager, listener=listener)
...
async def cleanup_device_registry(hass: SmartHub, device_manager: Manager) -> None:
...
async def async_unload_entry(hass: SmartHub, entry: TuyaConfigEntry) -> bool:
...
async def async_remove_entry(hass: SmartHub, entry: TuyaConfigEntry) -> None:
