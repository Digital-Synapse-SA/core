...
async def async_process_repairs_platforms(hass: SmartHub) -> None:
...
def _register_repairs_platform(
    hass: SmartHub, integration_domain: str, platform: RepairsProtocol
) -> None:
    """Register a repairs platform."""
    if not hasattr(platform, "async_create_fix_flow"):
        raise SmartHubError(f"Invalid repairs platform {platform}")
