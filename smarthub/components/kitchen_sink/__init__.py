...
from smarthub.components.recorder import DOMAIN as RECORDER_DOMAIN, get_instance
...
async def _notify_backup_listeners(hass: SmartHub) -> None:
    for listener in hass.data.get(DATA_BACKUP_AGENT_LISTENERS, []):
        listener()


def _create_issues(hass: SmartHub) -> None:
...
async def _insert_statistics(hass: SmartHub) -> None:
...
async def _insert_wrong_wind_direction_statistics(hass: SmartHub) -> None:
