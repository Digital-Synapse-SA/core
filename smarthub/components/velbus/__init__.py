...
from smarthub.const import CONF_PORT, Platform
from smarthub.core import SmartHub
from smarthub.exceptions import ConfigEntryNotReady, PlatformNotReady
...
async def velbus_scan_task(
    controller: Velbus, hass: SmartHub, entry_id: str
) -> None:
...
def _migrate_device_identifiers(hass: SmartHub, entry_id: str) -> None:
...
async def async_setup_entry(hass: SmartHub, entry: VelbusConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: VelbusConfigEntry) -> bool:
...
async def async_remove_entry(hass: SmartHub, entry: VelbusConfigEntry) -> None:
...
async def async_migrate_entry(
    hass: SmartHub, config_entry: VelbusConfigEntry
) -> bool:
    """Migrate old entry."""
    _LOGGER.error(
