...
# TYPES is deprecated as of SmartHub 2022.2, use DeviceAutomationType instead
TYPES = {
...
@overload
async def async_get_device_automation_platform(
    hass: SmartHub, domain: str, automation_type: DeviceAutomationType
...
def _async_set_entity_device_automation_metadata(
    hass: SmartHub, automation: dict[str, Any]
) -> None:
...
def async_get_entity_registry_entry_or_raise(
    hass: SmartHub, entity_registry_id: str
) -> er.RegistryEntry:
...
def async_validate_entity_schema(
    hass: SmartHub, config: ConfigType, schema: VolSchemaType
) -> ConfigType:
...
def handle_device_errors(
    func: Callable[[SmartHub, ActiveConnection, dict[str, Any]], Awaitable[None]],
) -> Callable[
