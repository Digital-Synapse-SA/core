...
from smarthub.components import assist_pipeline, conversation
from smarthub.config_entries import ConfigEntry
from smarthub.const import CONF_LLM_HASS_API, MATCH_ALL
from smarthub.core import SmartHub
from smarthub.exceptions import SmartHubError
from smarthub.helpers import device_registry as dr, intent, llm
...
async def _transform_stream(
    ...
    async for event in result:
        ...
        if isinstance(event, ResponseOutputItemAddedEvent):
            ...
            raise SmartHubError(f"OpenAI response incomplete: {reason}")
            ...
            raise SmartHubError(f"OpenAI response failed: {reason}")
        elif isinstance(event, ResponseErrorEvent):
            raise SmartHubError(f"OpenAI response error: {event.message}")
...
class OpenAIConversationEntity(
    ...
    async def async_added_to_hass(self) -> None:
        """When entity is added to SmartHub."""
    ...
    async def async_will_remove_from_hass(self) -> None:
        """When entity will be removed from SmartHub."""
    ...
    async def _async_handle_chat_log(
        ...
        for _iteration in range(MAX_TOOL_ITERATIONS):
            ...
            try:
                ...
                raise SmartHubError("Rate limited or insufficient funds") from err
            except openai.OpenAIError as err:
                LOGGER.error("Error talking to OpenAI: %s", err)
                raise SmartHubError("Error talking to OpenAI") from err
    ...
    async def _async_entry_update_listener(
        self, hass: SmartHub, entry: ConfigEntry
    ) -> None:
