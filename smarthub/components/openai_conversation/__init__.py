...
from smarthub.const import CONF_API_KEY, Platform
...
from smarthub.helpers import config_validation as cv, selector
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    async def render_image(call: ServiceCall) -> ServiceResponse:
        ...
        try:
            ...
            raise SmartHubError(f"Error generating image: {err}") from err

        if not response.data or not response.data[0].url:
            raise SmartHubError("No image returned")
    ...
    async def send_prompt(call: ServiceCall) -> ServiceResponse:
        ...
        def append_files_to_content() -> None:
            for filename in call.data[CONF_FILENAMES]:
                ...
                if not Path(filename).exists():
                    raise SmartHubError(f"`{filename}` does not exist")
        ...
        try:
            ...
            raise SmartHubError(f"Error generating content: {err}") from err
...
async def async_setup_entry(hass: SmartHub, entry: OpenAIConfigEntry) -> bool:
