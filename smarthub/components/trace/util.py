...
async def async_get_trace(
    hass: SmartHub, key: str, run_id: str
) -> dict[str, BaseTrace]:
...
def _get_debug_traces(hass: SmartHub, key: str) -> list[dict[str, Any]]:
...
async def async_list_traces(
    hass: SmartHub, wanted_domain: str, wanted_key: str | None
) -> list[dict[str, Any]]:
...
def async_store_trace(
    hass: SmartHub, trace: ActionTrace, stored_traces: int
) -> None:
...
def _async_store_restored_trace(hass: SmartHub, trace: RestoredTrace) -> None:
...
async def async_restore_traces(hass: SmartHub) -> None:
