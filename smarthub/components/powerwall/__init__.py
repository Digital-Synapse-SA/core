...
from smarthub.const import CONF_IP_ADDRESS, CONF_PASSWORD, Platform
...
async def async_setup_entry(hass: SmartHub, entry: PowerwallConfigEntry) -> bool:
...
async def async_migrate_entity_unique_ids(
    hass: SmartHub, entry: PowerwallConfigEntry, base_info: PowerwallBaseInfo
) -> None:
...
def async_last_update_was_successful(
    hass: SmartHub, entry: PowerwallConfigEntry
) -> bool:
