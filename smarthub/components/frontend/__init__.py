"""Handle the frontend for SmartHub."""
...
from smarthub.components import onboarding, websocket_api
from smarthub.components.http import KEY_HASS, SmartHubView, StaticPathConfig
from smarthub.components.websocket_api import ActiveConnection
from smarthub.config import async_hass_config_yaml
...
from smarthub.helpers import config_validation as cv, service
from smarthub.helpers.icon import async_get_icons
from smarthub.helpers.json import json_dumps_sorted
...
MANIFEST_JSON = Manifest(
    {
        ...
        "related_applications": [
            {"platform": "play", "id": "io.smarthub.companion.android"}
        ],
    }
)
...
def async_remove_panel(
    hass: SmartHub, frontend_url_path: str, *, warn_if_unknown: bool = True
) -> None:
...
def add_extra_js_url(hass: SmartHub, url: str, es5: bool = False) -> None:
...
def remove_extra_js_url(hass: SmartHub, url: str, es5: bool = False) -> None:
...
async def _async_setup_themes(
    hass: SmartHub, themes: dict[str, Any] | None
) -> None:
...
class IndexView(web_urldispatcher.AbstractResource):
    ...
    def __init__(self, repo_path: str | None, hass: SmartHub) -> None:
...
class ManifestJSONView(SmartHubView):
...
async def websocket_get_version(
    ...
    for req in integration.requirements:
        if req.startswith("smart-hub-frontend=="):
            frontend = req.removeprefix("smart-hub-frontend==")
