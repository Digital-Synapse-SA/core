...
def modbus_create_issue(
    hass: SmartHub, key: str, subs: list[str], err: str
) -> None:
    ...
    async_create_issue(
        ...
        issue_domain=DOMAIN,
        learn_more_url="https://www.smart-hub.io/integrations/modbus",
    )
    _LOGGER.warning(err)
...
def validate_entity(
    ...
    if 0 < scan_interval < 5:
        err = (
            ...
            "which may cause SmartHub stability issues"
        )
        _LOGGER.warning(err)
...
def check_config(hass: SmartHub, config: dict) -> dict:
