...
async def async_setup_entry(hass: SmartHub, entry: IsyConfigEntry) -> bool:
    """Set up the ISY 994 integration."""
    ...
    def _async_stop_auto_update(event: Event) -> None:
        """Stop the isy auto update on SmartHub Shutdown."""
...
async def _async_update_listener(hass: SmartHub, entry: IsyConfigEntry) -> None:
...
def _async_get_or_create_isy_device_in_registry(
    hass: SmartHub, entry: IsyConfigEntry, isy: ISY
) -> None:
...
async def async_unload_entry(hass: SmartHub, entry: IsyConfigEntry) -> bool:
