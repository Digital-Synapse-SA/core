...
from smarthub.components.lock import LockEntity
...
@callback
def async_setup_lock_services(hass: SmartHub) -> None:
...
class ISYLockEntity(ISYNodeEntity, LockEntity):
    ...
    async def async_lock(self, **kwargs: Any) -> None:
        ...
        if not await self._node.secure_lock():
            raise SmartHubError(f"Unable to lock device {self._node.address}")

    async def async_unlock(self, **kwargs: Any) -> None:
        """Send the unlock command to the ISY device."""
        if not await self._node.secure_unlock():
            raise SmartHubError(f"Unable to unlock device {self._node.address}")
