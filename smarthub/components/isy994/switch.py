...
class ISYSwitchEntity(ISYNodeEntity, SwitchEntity):
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        if not await self._node.turn_off():
            raise SmartHubError(f"Unable to turn off switch {self._node.address}")

    async def async_turn_on(self, **kwargs: Any) -> None:
        """Send the turn on command to the ISY switch."""
        if not await self._node.turn_on():
            raise SmartHubError(f"Unable to turn on switch {self._node.address}")
...
class ISYEnableSwitchEntity(ISYAuxControlEntity, SwitchEntity):
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        if not await self._node.disable():
            raise SmartHubError(f"Unable to disable device {self._node.address}")

    async def async_turn_on(self, **kwargs: Any) -> None:
        """Send the turn on command to the ISY switch."""
        if not await self._node.enable():
            raise SmartHubError(f"Unable to enable device {self._node.address}")
