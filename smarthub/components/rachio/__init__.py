...
from smarthub.const import CONF_API_KEY, CONF_WEBHOOK_ID, Platform
...
async def async_unload_entry(hass: SmartHub, entry: RachioConfigEntry) -> bool:
...
async def async_remove_entry(hass: SmartHub, entry: RachioConfigEntry) -> None:
    """Remove a rachio config entry."""
    if CONF_CLOUDHOOK_URL in entry.data:
        await cloud.async_delete_cloudhook(hass, entry.data[CONF_WEBHOOK_ID])


async def async_setup_entry(hass: SmartHub, entry: RachioConfigEntry) -> bool:
