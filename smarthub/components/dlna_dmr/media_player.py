...
from smarthub.const import CONF_DEVICE_ID, CONF_MAC, CONF_TYPE, CONF_URL
...
class DlnaDmrEntity(MediaPlayerEntity):
    ...
    async def async_config_update_listener(
        self, hass: SmartHub, entry: config_entries.ConfigEntry
    ) -> None:
    ...
    def _on_event(
        ...
        """State variable(s) changed, let smart-hub know."""
    ...
    async def async_play_media(
        ...
        if not title:
            title = extra.get("title") or metadata.get("title") or "SmartHub"
