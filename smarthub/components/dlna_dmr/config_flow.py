...
from smarthub.exceptions import IntegrationError
...
def _is_ignored_device(discovery_info: SsdpServiceInfo) -> bool:
    ...
    if len(discovery_info.x_smarthub_matching_domains) > 1:
        LOGGER.debug(
            "Ignoring device supported by multiple integrations: %s",
            discovery_info.x_smarthub_matching_domains,
        )
        return True
...
async def _async_get_mac_address(hass: SmartHub, host: str) -> str | None:
