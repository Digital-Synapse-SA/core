...
from smarthub.const import CONF_PLATFORM, SERVICE_TURN_ON, STATE_UNAVAILABLE
...
def _hass_domain_validator(config: dict[str, Any]) -> dict[str, Any]:
    """Validate platform in config for smarthub domain."""
...
def _platform_validator(config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        platform = importlib.import_module(
            f"smarthub.components.{platform_name}.scene"
        )
    except ImportError:
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    # Ensure SmartHub platform always loaded.
