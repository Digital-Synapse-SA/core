...
SmartHub. User need to record the token in secure place.
...
from smarthub.auth import InvalidAuthError
from smarthub.auth.models import (
...
from smarthub.components.http.ban import log_invalid_auth
...
from smarthub.helpers.config_entry_oauth2_flow import OAuth2AuthorizeCallbackView
...
def create_auth_code(
    hass: SmartHub, client_id: str, credential: Credentials
) -> str:
...
class RevokeTokenView(SmartHubView):
...
class LinkUserView(SmartHubView):
...
def websocket_delete_all_refresh_tokens(
    ...
    if delete_current_token and (
        ...
        # happens if SmartHub is shutting down.
