...
from smarthub.auth import AuthManagerFlowManager, InvalidAuthError
from smarthub.auth.models import AuthFlowContext, AuthFlowResult, Credentials
from smarthub.components import onboarding
from smarthub.components.http import KEY_HASS
from smarthub.components.http.auth import async_user_not_allowed_do_auth
...
from smarthub.helpers.network import is_cloud_connection
from smarthub.util.network import is_local

from . import indieauth

if TYPE_CHECKING:
    from smarthub.auth.providers.trusted_networks import (
...
@callback
def async_setup(
    hass: SmartHub, store_result: Callable[[str, Credentials], str]
) -> None:
...
class WellKnownOAuthInfoView(SmartHubView):
        ...
        return self.json(
            {
                ...
                "service_documentation": (
                    "https://developers.smart-hub.io/docs/auth_api"
                ),
            }
        )


class AuthProvidersView(SmartHubView):
...
class LoginFlowBaseView(SmartHubView):
