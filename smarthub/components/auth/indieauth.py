...
async def verify_redirect_uri(
    hass: SmartHub, client_id: str, redirect_uri: str
) -> bool:
    ...
    if (
        client_id == "https://smart-hub.io/iOS"
        and redirect_uri == "smarthub://auth-callback"
    ):
        return True

    if client_id == "https://smart-hub.io/android" and redirect_uri in (
        "smarthub://auth-callback",
...
async def fetch_redirect_uris(hass: SmartHub, url: str) -> list[str]:
