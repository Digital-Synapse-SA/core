...
@callback
def set_default_log_level(hass: SmartHub, level: int) -> None:
...
@callback
def set_log_levels(hass: SmartHub, logpoints: Mapping[str, int]) -> None:
...
@callback
def _clear_logger_overwrites(hass: SmartHub) -> None:
    """Clear logger overwrites. Used for testing."""
    hass.data[DATA_LOGGER].overrides.clear()


async def get_integration_loggers(hass: SmartHub, domain: str) -> set[str]:
    """Get loggers for an integration."""
    loggers: set[str] = {f"smarthub.components.{domain}"}
...
class LoggerSettings:
    ...
    def __init__(self, hass: SmartHub, yaml_config: ConfigType) -> None:
    ...
    async def async_update(
        self, hass: SmartHub, domain: str, settings: LoggerSetting
    ) -> None:
        """Update settings."""
        stored_log_config = self._stored_config[STORAGE_LOG_KEY]
    ...
    async def async_get_levels(self, hass: SmartHub) -> dict[str, int]:
