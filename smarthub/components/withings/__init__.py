...
from smarthub.components.webhook import (
...
async def async_setup_entry(hass: SmartHub, entry: WithingsConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: WithingsConfigEntry) -> bool:
...
class WithingsWebhookManager:
    ...
    def __init__(self, hass: SmartHub, entry: WithingsConfigEntry) -> None:
...
async def _async_cloudhook_generate_url(
    hass: SmartHub, entry: WithingsConfigEntry
) -> str:
...
async def async_remove_entry(hass: SmartHub, entry: WithingsConfigEntry) -> None:
...
def json_message_response(message: str, message_code: int) -> Response:
    ...
    return SmartHubView.json({"message": message, "code": message_code})


def get_webhook_handler(
    withings_data: WithingsData,
) -> Callable[[SmartHub, str, Request], Awaitable[Response | None]]:
