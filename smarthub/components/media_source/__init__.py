...
from smarthub.components import frontend, websocket_api
...
class MediaSourceProtocol(Protocol):
    ...
    async def async_get_media_source(self, hass: SmartHub) -> MediaSource:
        """Set up media source."""
...
@callback
def _get_media_item(
    hass: SmartHub, media_content_id: str | None, target_media_player: str | None
) -> MediaSourceItem:
    """Return media item."""
    if media_content_id:
