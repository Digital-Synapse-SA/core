...
async def async_setup(hass: SmartHub) -> None:
    ...
    # SmartHub starts and the dashboard is discovered.
...
class ESPHomeDashboardManager:
    ...
    async def async_setup(self) -> None:
        ...
        if is_hassio(self._hass):
            from smarthub.components.hassio import get_addons_info  # noqa: PLC0415
...
@callback
def async_get_dashboard(hass: SmartHub) -> ESPHomeDashboardCoordinator | None:
...
async def async_set_dashboard_info(
    hass: SmartHub, addon_slug: str, host: str, port: int
) -> None:
    """Set the dashboard info."""
