...
async def async_setup_entry(hass: SmartHub, entry: TVTrainConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: TVTrainConfigEntry) -> bool:
    """Unload Trafikverket Weatherstation config entry."""

    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def update_listener(hass: SmartHub, entry: TVTrainConfigEntry) -> None:
    """Handle options update."""
    await hass.config_entries.async_reload(entry.entry_id)


async def async_migrate_entry(hass: SmartHub, entry: TVTrainConfigEntry) -> bool:
