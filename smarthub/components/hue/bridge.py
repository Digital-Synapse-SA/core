...
from smarthub.const import CONF_API_KEY, CONF_API_VERSION, CONF_HOST, Platform
...
class HueBridge:
    ...
    def __init__(self, hass: core.SmartHub, config_entry: HueConfigEntry) -> None:
    ...
    async def async_request_call(self, task: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            if "may not have effect" in str(err):
                ...
                return None
            raise SmartHubError(msg) from err
...
async def _update_listener(hass: core.SmartHub, entry: HueConfigEntry) -> None:
    """Handle ConfigEntry options update."""
    await hass.config_entries.async_reload(entry.entry_id)


def create_config_flow(hass: core.SmartHub, host: str) -> None:
    """Start a config flow."""
