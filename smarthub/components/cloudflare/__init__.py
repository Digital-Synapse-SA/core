...
from smarthub.util.network import is_ipv4_address
...
async def async_setup_entry(hass: SmartHub, entry: CloudflareConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: CloudflareConfigEntry) -> bool:
...
async def _async_update_cloudflare(
    ...
    if not location_info or not is_ipv4_address(location_info.ip):
        raise SmartHubError("Could not get external IPv4 address")
