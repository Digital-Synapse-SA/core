...
async def async_discover_oauth_config(
    hass: SmartHub, mcp_server_url: str
) -> AuthorizationServer:
...
async def validate_input(
    hass: SmartHub, data: dict[str, Any], token_manager: TokenManager | None = None
) -> dict[str, Any]:
...
class InvalidUrl(SmartHubError):
...
class TimeoutConnectError(SmartHubError):
    """Error to indicate we cannot connect."""
...
class MissingCapabilities(SmartHubError):
