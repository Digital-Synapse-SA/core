...
from smarthub.components import conversation, stt, tts, wake_word, websocket_api
from smarthub.const import ATTR_SUPPORTED_FEATURES, MATCH_ALL
from smarthub.core import Context, SmartHub, callback
from smarthub.exceptions import SmartHubError
from smarthub.helpers import chat_session, intent
from smarthub.helpers.collection import (
    CHANGE_UPDATED,
    CollectionError,
    ItemNotFound,
...
from smarthub.helpers.storage import Store
from smarthub.helpers.typing import UNDEFINED, UndefinedType, VolDictType
...
from smarthub.util.limited_size_dict import LimitedSizeDict
...
def _async_resolve_default_pipeline_settings(
    ...
    *,
    ...
    The default pipeline will use the smarthub conversation agent and the
    ...
    # Find a matching language supported by the SmartHub conversation agent
...
async def _async_create_default_pipeline(
    hass: SmartHub, pipeline_store: PipelineStorageCollection
) -> Pipeline:
...
@callback
def async_get_pipeline(hass: SmartHub, pipeline_id: str | None = None) -> Pipeline:
...
@callback
def async_get_pipelines(hass: SmartHub) -> list[Pipeline]:
    """Get all pipelines."""
    pipeline_data = hass.data[KEY_ASSIST_PIPELINE]
    ...
    async def speech_to_text(
        ...
        try:
            ...
            raise SpeechToTextError(
                ...
                message="SmartHub Cloud authentication failed",
            ) from src_error
        except Exception as src_error:
...
class PipelineStorageCollectionWebsocket(
    ...
    @callback
    def async_setup(self, hass: SmartHub) -> None:
    ...
    async def ws_delete_item(
        self, hass: SmartHub, connection: websocket_api.ActiveConnection, msg: dict
    ) -> None:
        """Delete an item."""
        try:
...
async def async_setup_pipeline_store(hass: SmartHub) -> PipelineData:
...
async def async_run_migrations(hass: SmartHub) -> None:
...
def async_get_pipeline_conversation_data(
    hass: SmartHub, session: chat_session.ChatSession
