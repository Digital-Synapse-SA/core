...
def _get_config_entry(hass: SmartHub) -> ConfigEntry:
...
def async_get_active_network_settings(hass: SmartHub) -> NetworkBackup:
...
async def async_get_last_network_settings(
    hass: SmartHub, config_entry: ConfigEntry | None = None
) -> NetworkBackup | None:
...
async def async_change_channel(
    hass: SmartHub, new_channel: int | Literal["auto"]
) -> None:
