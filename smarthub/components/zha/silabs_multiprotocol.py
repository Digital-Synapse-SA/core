...
def _get_zha_url(hass: SmartHub) -> str | None:
...
async def _get_zha_channel(hass: SmartHub) -> int | None:
...
async def async_change_channel(
    hass: SmartHub, channel: int, delay: float = 0
) -> asyncio.Task | None:
...
async def async_get_channel(hass: SmartHub) -> int | None:
    """Return the channel.
...
async def async_using_multipan(hass: SmartHub) -> bool:
