...
from smarthub import __path__ as HOMEASSISTANT_PATH
from smarthub.components.smarthub_hardware.silabs_multiprotocol_addon import (
    is_multiprotocol_url,
)
from smarthub.components.system_log import LogEntry
...
from smarthub.helpers.dispatcher import async_dispatcher_send, dispatcher_send
from smarthub.helpers.entity_platform import AddEntitiesCallback
from smarthub.helpers.typing import ConfigType
from smarthub.util.logging import SmartHubQueueHandler
...
DEBUG_COMP_ZHA = "smarthub.components.zha"
DEBUG_LIB_ZHA = "zha"
...
class ZHAGatewayProxy(EventBase):
    ...
    def __init__(
        self, hass: SmartHub, config_entry: ConfigEntry, gateway: Gateway
    ) -> None:
        """Initialize the gateway proxy."""
        super().__init__()
        self.hass = hass
        ...
        self._log_queue_handler = SmartHubQueueHandler(log_simple_queue)
...
class LogRelayHandler(logging.Handler):
    ...
    def __init__(self, hass: SmartHub, gateway: ZHAGatewayProxy) -> None:
...
def get_zha_data(hass: SmartHub) -> HAZHAData:
...
def get_zha_gateway(hass: SmartHub) -> Gateway:
...
def get_zha_gateway_proxy(hass: SmartHub) -> ZHAGatewayProxy:
    """Get the ZHA gateway object."""
    if (gateway_proxy := get_zha_data(hass).gateway_proxy) is None:
        raise ValueError("No gateway object exists")
...
@callback
def async_get_zha_device_proxy(hass: SmartHub, device_id: str) -> ZHADeviceProxy:
...
def async_cluster_exists(hass: SmartHub, cluster_id, skip_coordinator=True):
...
def create_zha_config(hass: SmartHub, ha_zha_data: HAZHAData) -> ZHAData:
...
def convert_zha_error_to_ha_error[**_P, _EntityT: ZHAEntity](
    ...
    """Decorate ZHA commands and re-raises ZHAException as SmartHubError."""
    ...
    async def handler(self: _EntityT, *args: _P.args, **kwargs: _P.kwargs) -> None:
        try:
            ...
            raise SmartHubError(err) from err
