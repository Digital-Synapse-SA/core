...
from smarthub.components.cast import DOMAIN as CAST_DOMAIN
from smarthub.components.cast.home_assistant_cast import (
    ATTR_URL_PATH,
    ATTR_VIEW_PATH,
...
async def async_get_media_browser_root_object(
    ...
    return [
        BrowseMedia(
            ...
            thumbnail="https://brands.smart-hub.io/_/lovelace/logo.png",
            can_play=False,
            can_expand=True,
        )
    ]
...
async def async_play_media(
    ...
    if "/" in media_id:
        ...
        try:
            ...
            raise SmartHubError(f"Invalid dashboard {media_id} specified") from err
...
async def _get_dashboard_info(
    hass: SmartHub, url_path: str | None
) -> dict[str, Any]:
