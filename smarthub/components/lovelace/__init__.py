...
from smarthub.components import frontend, onboarding, websocket_api
from smarthub.config import (
    async_hass_config_yaml,
    async_process_component_and_handle_errors,
)
from smarthub.const import CONF_FILENAME, CONF_MODE, CONF_RESOURCES
from smarthub.core import SmartHub, ServiceCall, callback
from smarthub.exceptions import SmartHubError
from smarthub.helpers import collection, config_validation as cv
from smarthub.helpers.frame import report_usage
...
class LovelaceData:
    ...
    def __getitem__(self, name: str) -> Any:
        ...
        https://github.com/smart-hub/core/pull/136313
        """
        report_usage(
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    async def reload_resources_service_handler(service_call: ServiceCall) -> None:
        ...
        if config is None:
            raise SmartHubError("Config validation failed")
...
async def create_yaml_resource_col(
    hass: SmartHub, yaml_resources: list[ConfigType] | None
...
@callback
def _register_panel(
    hass: SmartHub, url_path: str | None, mode: str, config: dict, update: bool
) -> None:
    """Register a panel."""
    kwargs = {
...
async def _create_map_dashboard(
    hass: SmartHub, dashboards_collection: dashboard.DashboardsCollection
) -> None:
    """Create a map dashboard."""
