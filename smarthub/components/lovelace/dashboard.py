...
from smarthub.components.frontend import DATA_PANELS
from smarthub.const import CONF_FILENAME
from smarthub.core import SmartHub, callback
from smarthub.exceptions import SmartHubError
from smarthub.helpers import collection, storage
from smarthub.helpers.json import json_bytes, json_fragment
from smarthub.util.yaml import Secrets, load_yaml_dict
...
class LovelaceConfig(ABC):
    ...
    def __init__(
        self, hass: SmartHub, url_path: str | None, config: dict[str, Any] | None
    ) -> None:
    ...
    async def async_save(self, config: dict[str, Any]) -> None:
        """Save config."""
        raise SmartHubError("Not supported")
...
class LovelaceStorage(LovelaceConfig):
    ...
    def __init__(self, hass: SmartHub, config: dict[str, Any] | None) -> None:
    ...
    async def async_save(self, config: dict[str, Any]) -> None:
        """Save config."""
        if self.hass.config.recovery_mode:
            raise SmartHubError("Saving not supported in recovery mode")
    ...
    async def async_delete(self) -> None:
        """Delete config."""
        if self.hass.config.recovery_mode:
            raise SmartHubError("Deleting not supported in recovery mode")
