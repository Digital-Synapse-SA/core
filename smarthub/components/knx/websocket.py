...
from smarthub.components import panel_custom, websocket_api
from smarthub.components.http import StaticPathConfig
from smarthub.const import CONF_ENTITY_ID, CONF_PLATFORM
...
async def register_panel(hass: SmartHub) -> None:
...
type KnxWebSocketCommandHandler = Callable[
    [SmartHub, KNXModule, websocket_api.ActiveConnection, dict[str, Any]], None
]
type KnxAsyncWebSocketCommandHandler = Callable[
    [SmartHub, KNXModule, websocket_api.ActiveConnection, dict[str, Any]],
    Awaitable[None],
]
...
def ws_create_device(
    ...
    device_registry.async_update_device(
        ...
        configuration_url=f"smarthub://knx/entities/view?device_id={_device.id}",
    )
