...
async def async_setup_entry(hass: SmartHub, entry: FGLairConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: FGLairConfigEntry) -> bool:
...
async def async_migrate_entry(hass: SmartHub, entry: FGLairConfigEntry) -> bool:
    """Migrate old entry."""
    if entry.version > 1:
        return False
