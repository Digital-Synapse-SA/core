"""Config flow for the SmartHub Yellow integration."""
...
from smarthub.core import SmartHub, async_get_hass, callback
from smarthub.helpers import discovery_flow, selector
...
class SmartHubYellowConfigFlow(BaseFirmwareConfigFlow, domain=DOMAIN):
    """Handle a config flow for SmartHub Yellow."""
    ...
    def async_get_options_flow(
        ...
        if firmware_type is ApplicationType.CPC:
            return SmartHubYellowMultiPanOptionsFlowHandler(hass, config_entry)

        return SmartHubYellowOptionsFlowHandler(hass, config_entry)
...
class BaseSmartHubYellowOptionsFlow(OptionsFlow, ABC):
    """Base SmartHub Yellow options flow shared between firmware and multi-PAN."""

    _hw_settings: dict[str, bool] | None = None

    def __init__(self, hass: SmartHub, *args: Any, **kwargs: Any) -> None:
...
class SmartHubYellowMultiPanOptionsFlowHandler(
    BaseSmartHubYellowOptionsFlow, MultiprotocolOptionsFlowHandler
):
    """Handle a multi-PAN options flow for SmartHub Yellow."""
...
class SmartHubYellowOptionsFlowHandler(
    BaseSmartHubYellowOptionsFlow, BaseFirmwareOptionsFlow
):
    """Handle a firmware options flow for SmartHub Yellow."""
