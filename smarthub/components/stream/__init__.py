...
a new Stream object. Stream manages:
  ...
  - SmartHub URLs for viewing a stream
...
from smarthub.const import EVENT_HOMEASSISTANT_STOP, EVENT_LOGGING_CHANGED
...
if TYPE_CHECKING:
    from smarthub.components.camera import DynamicStreamSettings
...
async def async_check_stream_client_error(
    hass: SmartHub, source: str, pyav_options: dict[str, str] | None = None
) -> None:
...
def _check_stream_client_error(
    hass: SmartHub, source: str, options: dict[str, str] | None = None
) -> None:
    """Check if a stream can be successfully opened.
...
def _convert_stream_options(
    ...
    if DOMAIN not in hass.data:
        raise SmartHubError("Stream integration is not set up.")
    ...
    try:
        ...
        raise SmartHubError(f"Invalid stream options: {exc}") from exc
...
class Stream:
    ...
    async def async_record(
        ...
        if not self.hass.config.is_allowed_path(video_path):
            raise SmartHubError(f"Can't write {video_path}, no access to path!")
