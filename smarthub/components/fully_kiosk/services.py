    ...
    for target in device_ids:
        ...
        if device:
            ...
            if not device_entries:
                raise SmartHubError(f"Device '{target}' is not a {DOMAIN} device")
            config_entries.extend(device_entries)
        else:
            raise SmartHubError(f"Device '{target}' not found in device registry")
    coordinators = list[FullyKioskDataUpdateCoordinator]()
    for config_entry in config_entries:
        if config_entry.state != ConfigEntryState.LOADED:
            raise SmartHubError(f"{config_entry.title} is not loaded")
