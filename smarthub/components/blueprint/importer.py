...
from smarthub.helpers import aiohttp_client, config_validation as cv
...
COMMUNITY_TOPIC_PATTERN = re.compile(
    r"^https://community.smart-hub.io/t/[a-z0-9-]+/(?P<topic>\d+)(?:/(?P<post>\d+)|)$"
)
...
WEBSITE_PATTERN = re.compile(
    r"^https://(?P<subdomain>[a-z0-9-]+)\.smart-hub\.io/(?P<path>.+).yaml$"
)
...
class UnsupportedUrl(SmartHubError):
...
async def fetch_blueprint_from_website_url(
    ...
    if (WEBSITE_PATTERN.match(url)) is None:
        raise UnsupportedUrl("Not a SmartHub website URL")
    ...
    suggested_filename = f"smarthub/{parsed_import_url.parts[-1][:-5]}"
    return ImportedBlueprint(suggested_filename, raw_yaml, blueprint)
...
async def fetch_blueprint_from_url(hass: SmartHub, url: str) -> ImportedBlueprint:
    ...
    raise SmartHubError("Unsupported URL")
