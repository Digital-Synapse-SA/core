...
from smarthub.components.http import KEY_HASS, KEY_HASS_USER, SmartHubView
...
@callback
def async_setup_auth_view(hass: SmartHub, user: User) -> None:
    """Auth setup."""
...
class HassIOBaseAuth(SmartHubView):
    """Hass.io view to handle auth requests."""

    def __init__(self, hass: SmartHub, user: User) -> None:
        """Initialize WebView."""
        self.hass = hass
        self.user = user
