"""Update helpers for Supervisor."""

from __future__ import annotations

from aiohasupervisor import SupervisorError
from aiohasupervisor.models import (
    SmartHubUpdateOptions,
    OSUpdate,
    StoreAddonUpdate,
)

from smarthub.core import SmartHub
from smarthub.exceptions import SmartHubError

from .handler import get_supervisor_client


async def update_addon(
    hass: SmartHub,
    addon: str,
    backup: bool,
    addon_name: str | None,
    installed_version: str | None,
) -> None:
    """Update an addon.

    Optionally make a backup before updating.
    """
    client = get_supervisor_client(hass)

    if backup:
        from .backup import backup_addon_before_update  # noqa: PLC0415

        await backup_addon_before_update(hass, addon, addon_name, installed_version)

    try:
        await client.store.update_addon(addon, StoreAddonUpdate(backup=False))
    except SupervisorError as err:
        raise SmartHubError(
            f"Error updating {addon_name or addon}: {err}"
        ) from err


async def update_core(hass: SmartHub, version: str | None, backup: bool) -> None:
    """Update core.

    Optionally make a backup before updating.
    """
    client = get_supervisor_client(hass)

    if backup:
        from .backup import backup_core_before_update  # noqa: PLC0415

        await backup_core_before_update(hass)

    try:
        await client.smarthub.update(
            SmartHubUpdateOptions(version=version, backup=False)
        )
    except SupervisorError as err:
        raise SmartHubError(f"Error updating SmartHub Core: {err}") from err


async def update_os(hass: SmartHub, version: str | None, backup: bool) -> None:
    """Update OS.

    Optionally make a core backup before updating.
    """
    client = get_supervisor_client(hass)

    if backup:
        from .backup import backup_core_before_update  # noqa: PLC0415

        await backup_core_before_update(hass)

    try:
        await client.os.update(OSUpdate(version=version))
    except SupervisorError as err:
        raise SmartHubError(
            f"Error updating SmartHub Operating System: {err}"
        ) from err
