...
from smarthub.helpers.backup import async_get_manager as async_get_backup_manager
...
def _backup_details_to_agent_backup(
    ...
    smarthub_included = details.smarthub is not None
    if not smarthub_included:
        database_included = False
    else:
        database_included = details.smarthub_exclude_database is False
    addons = [
    ...
    return AgentBackup(
        ...
        smarthub_included=smarthub_included,
        smarthub_version=details.smarthub,
        name=details.name,
...
class SupervisorBackupAgent(BackupAgent):
    ...
    def __init__(self, hass: SmartHub, name: str, location: str) -> None:
...
class SupervisorBackupReaderWriter(BackupReaderWriter):
    ...
    async def async_create_backup(
        ...
        *,
        ...
        """Create a backup."""
        if not include_smarthub and include_database:
            raise SmartHubError(
                "Cannot create a backup with database but without SmartHub"
            )
        ...
        # Always include SSL if SmartHub is included
        if include_smarthub:
        ...
        try:
            backup = await self._client.backups.partial_backup(
                supervisor_backups.PartialBackupOptions(
                    ...
                    smarthub=include_smarthub,
                    name=backup_name,
                    password=password,
                    compressed=True,
                    location=locations,
                    smarthub_exclude_database=not include_database,
                    background=True,
    ...
    async def async_restore_backup(
        ...
        *,
        ...
        if (
            ...
            backup
            and restore_smarthub
            and restore_database != backup.database_included
        ):
            raise SmartHubError("Restore database must match backup")
        if not restore_smarthub and restore_database:
            raise SmartHubError("Cannot restore database without SmartHub")
        ...
        try:
            ...
            raise SmartHubError(message) from err
...
async def backup_addon_before_update(
    ...
    try:
        ...
        raise SmartHubError(f"Error creating backup: {err}") from err
    else:
        try:
            ...
            raise SmartHubError(f"Error deleting old backups: {err}") from err


async def backup_core_before_update(hass: SmartHub) -> None:
