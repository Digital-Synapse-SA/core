...
from smarthub.components import panel_custom
from smarthub.components.smarthub import async_set_stop_handler
...
get_supervisor_ip = deprecated_function(
    "smarthub.helpers.hassio.get_supervisor_ip", breaks_in_ha_version="2025.11"
)(_get_supervisor_ip)
...
@callback
@deprecated_function(
    "smarthub.helpers.hassio.is_hassio", breaks_in_ha_version="2025.11"
)
@bind_hass
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:  # noqa: C901
    ...
    async def _async_stop(hass: SmartHub, restart: bool) -> None:
        ...
        if restart:
            await supervisor_client.smarthub.restart()
        else:
            await supervisor_client.smarthub.stop()

    # Set a custom handler for the smarthub.restart and smarthub.stop services
