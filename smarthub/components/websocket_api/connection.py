...
from smarthub.auth.models import RefreshToken, User
...
type MessageHandler = Callable[[SmartHub, ActiveConnection, dict[str, Any]], None]
type BinaryHandler = Callable[[SmartHub, ActiveConnection, bytes], None]
...
class ActiveConnection:
    ...
    def async_handle_exception(self, msg: dict[str, Any], err: Exception) -> None:
        ...
        if isinstance(err, Unauthorized):
            ...
            err_message = "Timeout"
        elif isinstance(err, SmartHubError):
            err_message = str(err)
