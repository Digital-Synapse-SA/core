"""Component to integrate the SmartHub cloud."""
...
from smarthub.components import alexa, google_assistant
from smarthub.config_entries import SOURCE_SYSTEM, ConfigEntry
...
from smarthub.core import Event, HassJob, SmartHub, ServiceCall, callback
from smarthub.exceptions import SmartHubError
from smarthub.helpers import config_validation as cv, entityfilter
...
from smarthub.helpers.service import async_register_admin_service
from smarthub.helpers.typing import ConfigType
from smarthub.loader import async_get_integration, bind_hass
...
class CloudNotAvailable(SmartHubError):
...
@bind_hass
@callback
def async_is_logged_in(hass: SmartHub) -> bool:
...
@bind_hass
@callback
def async_is_connected(hass: SmartHub) -> bool:
...
@bind_hass
@callback
def async_active_subscription(hass: SmartHub) -> bool:
    """Test if user has an active subscription."""
    return async_is_logged_in(hass) and not hass.data[DATA_CLOUD].subscription_expired


async def async_get_or_create_cloudhook(hass: SmartHub, webhook_id: str) -> str:
...
@bind_hass
async def async_create_cloudhook(hass: SmartHub, webhook_id: str) -> str:
    """Create a cloudhook."""
    if not async_is_connected(hass):
        raise CloudNotConnected
...
@bind_hass
async def async_delete_cloudhook(hass: SmartHub, webhook_id: str) -> None:
    """Delete a cloudhook."""
...
@bind_hass
@callback
def async_remote_ui_url(hass: SmartHub) -> str:
...
@callback
def _setup_services(hass: SmartHub, prefs: CloudPreferences) -> None:
...
async def _setup_log_handler(hass: SmartHub) -> FixedSizeQueueLogHandler:
    fmt = (
