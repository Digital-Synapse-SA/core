...
from smarthub.components.alexa import (
    entities as alexa_entities,
    errors as alexa_errors,
)
from smarthub.components.google_assistant import helpers as google_helpers
from smarthub.components.smarthub import exposed_entities
from smarthub.components.http import KEY_HASS, SmartHubView, require_admin
from smarthub.components.http.data_validator import RequestDataValidator
from smarthub.components.system_health import get_info as get_system_health_info
...
from smarthub.util.location import async_detect_location_info
...
_CLOUD_ERRORS: dict[
    ...
    TimeoutError: (
        ...
        "Unable to reach the SmartHub cloud.",
    ),
    aiohttp.ClientError: (
...
def _handle_cloud_errors[_HassViewT: SmartHubView, **_P](
    handler: Callable[
...
def _ws_handle_cloud_errors(
    handler: Callable[
        [SmartHub, websocket_api.ActiveConnection, dict[str, Any]],
...
class GoogleActionsSyncView(SmartHubView):
...
class CloudLoginView(SmartHubView):
    """Login to SmartHub cloud."""
...
class CloudLogoutView(SmartHubView):
    """Log out of the SmartHub cloud."""
...
class CloudRegisterView(SmartHubView):
    """Register on the SmartHub cloud."""
...
class CloudResendConfirmView(SmartHubView):
...
class CloudForgotPasswordView(SmartHubView):
...
class DownloadSupportPackageView(SmartHubView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        hass_info = domain_health.pop("smarthub", {})
    ...
    if changes.get(PREF_ALEXA_REPORT_STATE):
        ...
        try:
            ...
            connection.send_error(
                ...
                "alexa_relink",
                (
                    "Please go to the Alexa app and re-link the SmartHub "
...
async def _account_data(
    hass: SmartHub, cloud: Cloud[CloudClient]
) -> dict[str, Any]:
...
async def google_assistant_get(
    ...
    with suppress(SmartHubError, KeyError):
    ...
    async with asyncio.timeout(10):
        try:
            ...
            connection.send_error(
                msg["id"],
                "alexa_relink",
                "Please go to the Alexa app and re-link the SmartHub skill.",
            )
            return
