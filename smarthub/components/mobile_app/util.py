...
@callback
def webhook_id_from_device_id(hass: SmartHub, device_id: str) -> str | None:
...
@callback
def supports_push(hass: SmartHub, webhook_id: str) -> bool:
...
@callback
def get_notify_service(hass: SmartHub, webhook_id: str) -> str | None:
...
async def async_create_cloud_hook(
    hass: SmartHub, webhook_id: str, entry: ConfigEntry | None
) -> str:
    """Create a cloud hook."""
