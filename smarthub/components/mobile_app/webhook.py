...
from smarthub.components.frontend import MANIFEST_JSON
...
from smarthub.core import EventOrigin, SmartHub
...
WEBHOOK_COMMANDS: Registry[
    str, Callable[[SmartHub, ConfigEntry, Any], Coroutine[Any, Any, Response]]
] = Registry()
...
async def handle_webhook(
    hass: SmartHub, webhook_id: str, request: Request
) -> Response:
...
async def webhook_call_service(
    hass: SmartHub, config_entry: ConfigEntry, data: dict[str, Any]
) -> Response:
...
async def webhook_stream_camera(
    hass: SmartHub, config_entry: ConfigEntry, data: dict[str, str]
) -> Response:
...
async def webhook_enable_encryption(
    hass: SmartHub, config_entry: ConfigEntry, data: Any
) -> Response:
...
async def webhook_update_sensor_states(
    hass: SmartHub, config_entry: ConfigEntry, data: list[dict[str, Any]]
) -> Response:
