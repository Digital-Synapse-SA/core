...
from smarthub.helpers import config_validation as cv, singleton
...
@bind_hass
def dismiss(hass: SmartHub, notification_id: str) -> None:
...
def _async_get_or_create_notifications(hass: SmartHub) -> dict[str, Notification]:
...
@callback
@bind_hass
def async_dismiss(hass: SmartHub, notification_id: str) -> None:
    """Remove a notification."""
    notifications = _async_get_or_create_notifications(hass)
...
@callback
def async_dismiss_all(hass: SmartHub) -> None:
