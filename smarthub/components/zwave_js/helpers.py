...
from smarthub.helpers.group import expand_entity_ids
from smarthub.helpers.typing import ConfigType, VolSchemaType
...
async def async_enable_statistics(driver: Driver) -> None:
    ...
    await driver.async_enable_statistics("SmartHub", HA_VERSION)


async def async_enable_server_logging_if_needed(
    hass: SmartHub, entry: ConfigEntry, driver: Driver
) -> None:
...
def async_get_node_from_device_id(
    hass: SmartHub, device_id: str, dev_reg: dr.DeviceRegistry | None = None
) -> ZwaveNode:
...
def _zwave_js_config_entry(hass: SmartHub, device: dr.DeviceEntry) -> str | None:
...
def async_get_node_status_sensor_entity_id(
    ...
    if not (device := dev_reg.async_get(device_id)):
        raise SmartHubError("Invalid Device ID provided")
...
def get_network_identifier_for_notification(
    hass: SmartHub, config_entry: ConfigEntry, controller: Controller
) -> str:
...
async def async_get_version_info(hass: SmartHub, ws_address: str) -> VersionInfo:
