...
from smarthub.components.persistent_notification import async_create
...
class ControllerEvents:
    ...
    def __init__(self, hass: SmartHub, driver_events: DriverEvents) -> None:
...
class NodeEvents:
    ...
    def __init__(
        self, hass: SmartHub, controller_events: ControllerEvents
    ) -> None:
...
async def client_listen(
    ...
    if not hass.is_stopping:
        if entry.state is not ConfigEntryState.LOADED:
            raise SmartHubError("Listen task ended unexpectedly")
...
async def async_ensure_addon_running(hass: SmartHub, entry: ConfigEntry) -> None:
...
@callback
def _get_addon_manager(hass: SmartHub) -> AddonManager:
