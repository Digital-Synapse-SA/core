...
type OverkizDataConfigEntry = ConfigEntry[SmartHubOverkizData]


async def async_setup_entry(hass: SmartHub, entry: OverkizDataConfigEntry) -> bool:
    ...
    entry.runtime_data = SmartHubOverkizData(
        coordinator=coordinator, platforms=platforms, scenarios=scenarios
    )

    # Map Overkiz entities to SmartHub platform
...
async def async_unload_entry(
    hass: SmartHub, entry: OverkizDataConfigEntry
) -> bool:
...
async def _async_migrate_entries(
    hass: SmartHub, config_entry: OverkizDataConfigEntry
) -> bool:
...
def create_local_client(
    hass: SmartHub, host: str, token: str, verify_ssl: bool
) -> OverkizClient:
...
def create_cloud_client(
    hass: SmartHub, username: str, password: str, server: OverkizServer
) -> OverkizClient:
