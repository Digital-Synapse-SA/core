...
from smarthub.const import CONF_DESCRIPTION, CONF_NAME, SERVICE_RELOAD
...
from smarthub.util import dt as dt_util, raise_if_invalid_filename
from smarthub.util.yaml.loader import load_yaml_dict
...
class ScriptError(SmartHubError):
...
def discover_scripts(hass: SmartHub) -> None:
...
class TimeWrapper:
    ...
    # Class variable, only going to warn once per SmartHub run
    warned = False
    ...
    def sleep(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not TimeWrapper.warned:
            ...
            _LOGGER.warning(
                "Using time.sleep can reduce the performance of SmartHub"
            )
