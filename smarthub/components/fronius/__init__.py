...
from smarthub.const import ATTR_MODEL, ATTR_SW_VERSION, CONF_HOST, Platform
...
async def async_setup_entry(hass: SmartHub, entry: FroniusConfigEntry) -> bool:
    ...
    fronius = Fronius(
        async_get_clientsession(
            ...
            # certificate. See https://github.com/smart-hub/core/issues/138881
            verify_ssl=False,
        ),
        host,
    )
...
async def async_unload_entry(hass: SmartHub, entry: FroniusConfigEntry) -> bool:
    """Unload a config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_remove_config_entry_device(
    hass: SmartHub, config_entry: FroniusConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
...
class FroniusSolarNet:
    ...
    def __init__(
        self, hass: SmartHub, entry: ConfigEntry, fronius: Fronius
    ) -> None:
