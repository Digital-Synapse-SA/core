...
from smarthub.const import APPLICATION_NAME, CONF_DEVICE_ID, CONF_TOKEN
...
async def async_setup_entry(hass: SmartHub, entry: RingConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: RingConfigEntry) -> bool:
    """Unload Ring entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_remove_config_entry_device(
    hass: SmartHub, entry: RingConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
    """Remove a config entry from a device."""
    return True


async def async_migrate_entry(hass: SmartHub, entry: RingConfigEntry) -> bool:
