...
from smarthub.core import CoreState, Event, SmartHub, ServiceCall, callback
...
def _async_get_system_for_service_call(
    hass: SmartHub, call: ServiceCall
) -> SystemType:
...
def _async_register_base_station(
    hass: SmartHub, entry: ConfigEntry, system: SystemType
) -> None:
...
@callback
def _async_standardize_config_entry(hass: SmartHub, entry: ConfigEntry) -> None:
...
async def async_setup_entry(hass: SmartHub, entry: ConfigEntry) -> bool:
    ...
    async def async_set_system_properties(
        ...
        if not isinstance(system, SystemV3):
            raise SmartHubError("Can only set system properties on V3 systems")
    ...
    async def async_reload_entry(_: SmartHub, updated_entry: ConfigEntry) -> None:
...
class SimpliSafe:
    ...
    def __init__(self, hass: SmartHub, entry: ConfigEntry, api: API) -> None:
        """Initialize."""
        self._api = api
        self._hass = hass
