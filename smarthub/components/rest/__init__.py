...
from smarthub.helpers import discovery, template
from smarthub.helpers.entity_component import DEFAULT_SCAN_INTERVAL
...
@callback
def _async_setup_shared_data(hass: SmartHub) -> None:
...
async def _async_process_config(hass: SmartHub, config: ConfigType) -> bool:
...
async def async_get_config_and_coordinator(
    hass: SmartHub, platform_domain: str, discovery_info: DiscoveryInfoType
...
def create_rest_data_from_config(hass: SmartHub, config: ConfigType) -> RestData:
    ...
    if not resource:
        raise SmartHubError("Resource not set for RestData")
