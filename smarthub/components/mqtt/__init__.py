...
from smarthub.const import CONF_DISCOVERY, CONF_PLATFORM, SERVICE_RELOAD
...
from smarthub.loader import async_get_integration, async_get_loaded_integration
from smarthub.setup import SetupPhases, async_pause_setup
...
async def _async_config_entry_updated(hass: SmartHub, entry: ConfigEntry) -> None:
...
@callback
def _async_remove_mqtt_issues(hass: SmartHub, mqtt_data: MqttData) -> None:
...
async def async_check_config_schema(
    hass: SmartHub, config_yaml: ConfigType
) -> None:
...
def _platforms_in_use(hass: SmartHub, entry: ConfigEntry) -> set[str | Platform]:
...
async def async_migrate_entry(hass: SmartHub, entry: ConfigEntry) -> bool:
...
def websocket_mqtt_info(
    hass: SmartHub, connection: websocket_api.ActiveConnection, msg: dict[str, Any]
) -> None:
...
def async_subscribe_connection_status(
    hass: SmartHub, connection_status_callback: ConnectionStatusCallback
) -> Callable[[], None]:
...
def is_connected(hass: SmartHub) -> bool:
...
async def async_remove_config_entry_device(
    hass: SmartHub, config_entry: ConfigEntry, device_entry: DeviceEntry
) -> bool:
