...
class MQTT:
    ...
    def __init__(
        self, hass: SmartHub, config_entry: ConfigEntry, conf: ConfigType
    ) -> None:
        """Initialize SmartHub MQTT client."""
        self.hass = hass
        self.loop = hass.loop
    ...
    @callback
    def _async_ha_started(self, _hass: SmartHub) -> None:
        """Handle HA started."""
    ...
    async def async_init_client(self) -> None:
        ...
        with async_pause_setup(self.hass, SetupPhases.WAIT_IMPORT_PACKAGES):
            await async_import_module(
                self.hass, "smarthub.components.mqtt.async_client"
            )
    ...
    async def _async_resubscribe_and_publish_birth_message(
        ...
        self._subscribe_debouncer.async_schedule()
        await self._ha_started.wait()  # Wait for SmartHub to start
