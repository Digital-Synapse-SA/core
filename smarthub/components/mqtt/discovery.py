...
from smarthub.const import CONF_DEVICE, CONF_PLATFORM
from smarthub.core import HassJobType, SmartHub, callback
from smarthub.helpers import config_validation as cv, discovery_flow
from smarthub.helpers.dispatcher import (
    async_dispatcher_connect,
    async_dispatcher_send,
)
from smarthub.helpers.service_info.mqtt import MqttServiceInfo, ReceivePayloadType
from smarthub.helpers.typing import DiscoveryInfoType
from smarthub.loader import async_get_mqtt
from smarthub.util.json import json_loads_object
...
def clear_discovery_hash(hass: SmartHub, discovery_hash: tuple[str, str]) -> None:
    """Clear entry from already discovered list."""
    hass.data[DATA_MQTT].discovery_already_discovered.discard(discovery_hash)


def set_discovery_hash(hass: SmartHub, discovery_hash: tuple[str, str]) -> None:
...
async def async_start(  # noqa: C901
    hass: SmartHub, discovery_topic: str, config_entry: ConfigEntry
) -> None:
            ...
            if topic_trimmed.endswith("config"):
                _LOGGER.warning(
                    (
                        ...
                        "https://www.smart-hub.io/integrations/mqtt/#discovery-topic"
                    ),
                    topic,
                )
            return
...
async def async_stop(hass: SmartHub) -> None:
    """Stop MQTT Discovery."""
    mqtt_data = hass.data[DATA_MQTT]
