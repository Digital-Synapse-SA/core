...
from smarthub.const import MAX_LENGTH_STATE_STATE, STATE_UNKNOWN, Platform
...
TEMP_DIR_NAME = f"smart-hub-{DOMAIN}"
...
class EnsureJobAfterCooldown:
    ...
    async def _async_job(self) -> None:
        ...
        except SmartHubError as ha_error:
...
def mqtt_config_entry_enabled(hass: SmartHub) -> bool | None:
...
async def async_wait_for_mqtt_client(hass: SmartHub) -> bool:
...
def learn_more_url(platform: str) -> str:
    ...
    return f"https://www.smart-hub.io/integrations/{platform}.mqtt/"
