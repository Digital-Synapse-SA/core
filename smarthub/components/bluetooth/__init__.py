...
from smarthub.config_entries import SOURCE_INTEGRATION_DISCOVERY, ConfigEntry
from smarthub.const import EVENT_HOMEASSISTANT_STOP
from smarthub.core import Event, HassJob, SmartHub, callback as hass_callback
...
from smarthub.helpers.issue_registry import async_delete_issue
from smarthub.loader import async_get_bluetooth
...
async def _async_start_adapter_discovery(
    hass: SmartHub,
    manager: SmartHubBluetoothManager,
...
def async_migrate_entries(
    hass: SmartHub, adapters: dict[str, AdapterDetails], default_adapter: str
) -> None:
...
async def async_update_listener(hass: SmartHub, entry: ConfigEntry) -> None:
