...
def async_filter_entities(hass: SmartHub, entity_ids: list[str]) -> list[str]:
...
def _async_config_entries_for_ids(
    hass: SmartHub, entity_ids: list[str] | None, device_ids: list[str] | None
) -> set[str]:
...
def is_sensor_continuous(
    hass: SmartHub, ent_reg: er.EntityRegistry, entity_id: str
) -> bool:
