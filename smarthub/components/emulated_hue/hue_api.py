"""Support for a Hue API to control SmartHub."""
...
from smarthub.components.fan import ATTR_PERCENTAGE, FanEntityFeature
from smarthub.components.http import KEY_HASS, SmartHubView
from smarthub.components.humidifier import ATTR_HUMIDITY, SERVICE_SET_HUMIDITY
...
from smarthub.core import Event, EventStateChangedData, State
...
from smarthub.util.network import is_local
...
class HueUnauthorizedUser(SmartHubView):
...
class HueUsernameView(SmartHubView):
...
class HueAllGroupsStateView(SmartHubView):
...
class HueGroupView(SmartHubView):
...
class HueAllLightsStateView(SmartHubView):
...
class HueFullStateView(SmartHubView):
...
class HueConfigView(SmartHubView):
...
class HueOneLightStateView(SmartHubView):
...
class HueOneLightChangeView(SmartHubView):
...
async def wait_for_state_change_or_timeout(
    hass: core.SmartHub, entity_id: str, timeout: float
) -> None:
