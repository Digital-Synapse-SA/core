...
from smarthub.const import CONF_ADDRESS, Platform
...
def add_insteon_events(hass: SmartHub, device: Device) -> None:
...
def register_new_device_callback(hass: SmartHub) -> None:
    ...
    async def async_create_new_entities(address):
        _LOGGER.debug(
            "Adding new INSTEON device to SmartHub with address %s", address
        )
...
async def async_get_usb_ports(hass: SmartHub) -> dict[str, str]:
    """Return a dict of USB ports and their friendly names."""
