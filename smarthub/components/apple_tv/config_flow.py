...
from smarthub.const import CONF_ADDRESS, CONF_NAME, CONF_PIN
...
async def device_scan(
    hass: SmartHub, identifier: str | None, loop: asyncio.AbstractEventLoop
...
class AppleTVConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def device_identifier(self) -> str | None:
        ...
        A device has multiple unique identifiers, but SmartHub only supports one
...
class DeviceAlreadyConfigured(SmartHubError):
