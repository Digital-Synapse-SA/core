"""Support for functionality to have conversations with SmartHub."""
...
def async_get_conversation_languages(
    hass: SmartHub, agent_id: str | None = None
) -> set[str] | Literal["*"]:
...
async def async_prepare_agent(
    hass: SmartHub, agent_id: str | None, language: str
) -> None:
    """Prepare given agent."""
    agent = async_get_agent(hass, agent_id)
...
async def async_handle_sentence_triggers(
    hass: SmartHub, user_input: ConversationInput
) -> str | None:
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    from smarthub.components.assist_pipeline import (  # noqa: PLC0415
    ...
    async def handle_process(service: ServiceCall) -> ServiceResponse:
        ...
        try:
            ...
            raise SmartHubError(f"Error processing {text}: {err}") from err
