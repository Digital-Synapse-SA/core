"""Standard conversation implementation for SmartHub."""
...
from smarthub.const import EVENT_STATE_CHANGED, MATCH_ALL
...
from smarthub.helpers.event import async_track_state_added_domain
from smarthub.util import language as language_util
...
class IntentMatchingStage(Enum):
    ...
    """Match against unexposed entities in SmartHub."""

    FUZZY = auto()
    """Capture names that are not known to SmartHub."""
...
async def async_setup_default_agent(
    ...
    @core.callback
    def async_hass_started(hass: core.SmartHub) -> None:
...
class DefaultAgent(ConversationEntity):
    ...
    def __init__(
        self, hass: core.SmartHub, config_intents: dict[str, Any]
    ) -> None:
    ...
    def _get_unexposed_entity_names(self, text: str) -> TextSlotList:
        """Get filtered slot list with unexposed entity names in SmartHub."""
