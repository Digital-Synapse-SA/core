"""Native SmartHub iOS app component."""
...
from smarthub.util.json import load_json_object
...
def devices_with_push(hass: SmartHub) -> dict[str, str]:
...
def enabled_push_ids(hass: SmartHub) -> list[str]:
...
def devices(hass: SmartHub) -> dict[str, dict[str, Any]]:
    """Return a dictionary of all identified devices."""
    return hass.data[DOMAIN][ATTR_DEVICES]  # type: ignore[no-any-return]


def device_name_for_push_id(hass: SmartHub, push_id: str) -> str | None:
...
class iOSPushConfigView(SmartHubView):
...
class iOSConfigView(SmartHubView):
...
class iOSIdentifyDeviceView(SmartHubView):
