...
from smarthub.components.bluetooth.match import ADDRESS, BluetoothCallbackMatcher
from smarthub.const import CONF_ADDRESS, EVENT_HOMEASSISTANT_STOP, Platform
...
async def async_setup_entry(hass: SmartHub, entry: IdasenDeskConfigEntry) -> bool:
...
async def _async_update_listener(
    hass: SmartHub, entry: IdasenDeskConfigEntry
) -> None:
    """Handle options update."""
    await hass.config_entries.async_reload(entry.entry_id)


async def async_unload_entry(hass: SmartHub, entry: IdasenDeskConfigEntry) -> bool:
