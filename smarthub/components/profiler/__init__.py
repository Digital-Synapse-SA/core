...
async def async_setup_entry(  # noqa: C901
    ...
    async def _async_start_log_objects(call: ServiceCall) -> None:
        if LOG_INTERVAL_SUB in domain_data:
            raise SmartHubError("Object logging already started")
    ...
    async def _async_stop_log_objects(call: ServiceCall) -> None:
        if LOG_INTERVAL_SUB not in domain_data:
            raise SmartHubError("Object logging not running")
...
async def _async_generate_profile(hass: SmartHub, call: ServiceCall):
    # Imports deferred to avoid loading modules
    # in memory since usually only one part of this
    # integration is used at a time
...
async def _async_generate_memory_profile(hass: SmartHub, call: ServiceCall):
    # Imports deferred to avoid loading modules
    # in memory since usually only one part of this
    # integration is used at a time
