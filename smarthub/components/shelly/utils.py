...
def async_remove_shelly_entity(
    hass: SmartHub, domain: str, unique_id: str
) -> None:
...
async def get_coap_context(hass: SmartHub) -> COAP:
...
class ShellyReceiver(SmartHubView):
...
async def get_ws_context(hass: SmartHub) -> WsServer:
...
def update_device_fw_info(
    hass: SmartHub, shellydevice: BlockDevice | RpcDevice, entry: ConfigEntry
) -> None:
...
def async_remove_shelly_rpc_entities(
    hass: SmartHub, domain: str, mac: str, keys: list[str]
) -> None:
...
def get_rpc_ws_url(hass: SmartHub) -> str | None:
...
def remove_stale_blu_trv_devices(
    hass: SmartHub, rpc_device: RpcDevice, entry: ConfigEntry
) -> None:
