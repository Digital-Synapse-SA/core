...
from smarthub.components.bluetooth import async_remove_scanner
...
from smarthub.helpers.device_registry import CONNECTION_NETWORK_MAC, format_mac
...
class ShellyBlockCoordinator(ShellyCoordinatorBase[BlockDevice]):
    ...
    def __init__(
        self, hass: SmartHub, entry: ShellyConfigEntry, device: BlockDevice
    ) -> None:
        ...
        if update_type is BlockUpdateType.ONLINE:
            ...
            if self._push_update_failures == MAX_PUSH_UPDATE_FAILURES:
                ...
                ir.async_create_issue(
                    ...
                    learn_more_url="https://www.smart-hub.io/integrations/shelly/#shelly-device-configuration-generation-1",
...
class ShellyRestCoordinator(ShellyCoordinatorBase[BlockDevice]):
    ...
    def __init__(
        self, hass: SmartHub, device: BlockDevice, entry: ShellyConfigEntry
    ) -> None:
...
class ShellyRpcCoordinator(ShellyCoordinatorBase[RpcDevice]):
    ...
    def __init__(
        self, hass: SmartHub, entry: ShellyConfigEntry, device: RpcDevice
    ) -> None:
    ...
    async def _async_update_listener(
        self, hass: SmartHub, entry: ShellyConfigEntry
    ) -> None:
...
async def async_reconnect_soon(hass: SmartHub, entry: ShellyConfigEntry) -> None:
