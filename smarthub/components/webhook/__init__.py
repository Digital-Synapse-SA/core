...
from smarthub.helpers.network import get_url, is_cloud_connection
...
from smarthub.util.aiohttp import MockRequest, MockStreamReader, serialize_response
...
def async_register(
    ...
    name: str,
    webhook_id: str,
    handler: Callable[[SmartHub, str, Request], Awaitable[Response | None]],
    *,
...
@callback
@bind_hass
def async_unregister(hass: SmartHub, webhook_id: str) -> None:
    """Remove a webhook."""
    handlers = hass.data.setdefault(DOMAIN, {})
...
async def async_handle_webhook(
    hass: SmartHub, webhook_id: str, request: Request | MockRequest
) -> Response:
    """Handle a webhook."""
...
class WebhookView(SmartHubView):
