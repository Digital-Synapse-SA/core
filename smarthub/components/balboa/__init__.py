...
async def async_setup_entry(hass: SmartHub, entry: BalboaConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: BalboaConfigEntry) -> bool:
    """Unload a config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def update_listener(hass: SmartHub, entry: BalboaConfigEntry) -> None:
    """Handle options update."""
    await hass.config_entries.async_reload(entry.entry_id)


async def async_setup_time_sync(hass: SmartHub, entry: BalboaConfigEntry) -> None:
    ...
    async def sync_time(now: datetime) -> None:
        ...
        if (now.hour, now.minute) != (spa.time_hour, spa.time_minute):
            _LOGGER.debug("Syncing time with SmartHub")
