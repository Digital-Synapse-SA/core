...
from smarthub.const import CONF_DEVICE, CONF_HOST, CONF_MODEL, CONF_TOKEN, Platform
...
async def async_setup_entry(hass: SmartHub, entry: XiaomiMiioConfigEntry) -> bool:
...
class VacuumCoordinatorDataAttributes:
    ...
    needed.
    See smarthub.components.xiaomi_miio.device.XiaomiCoordinatedMiioEntity
...
def _async_update_data_vacuum(
    hass: SmartHub, device: RoborockVacuum
) -> Callable[[], Coroutine[Any, Any, VacuumCoordinatorData]]:
    def update() -> VacuumCoordinatorData:
        timer = []

        # See https://github.com/smart-hub/core/issues/38285 for reason on
...
async def async_create_miio_device_and_coordinator(
    hass: SmartHub, entry: XiaomiMiioConfigEntry
) -> None:
...
async def async_unload_entry(
    hass: SmartHub, config_entry: XiaomiMiioConfigEntry
) -> bool:
