...
from smarthub.const import EntityCategory, UnitOfElectricPotential
from smarthub.core import SmartHub
from smarthub.exceptions import SmartHubError
from smarthub.helpers import entity_platform
...
class ElkSensor(ElkAttachedEntity, SensorEntity):
    ...
    async def async_counter_refresh(self) -> None:
        ...
        if not isinstance(self, ElkCounter):
            raise SmartHubError("supported only on ElkM1 Counter sensors")
        self._element.get()
    ...
    async def async_zone_bypass(self, code: int | None = None) -> None:
        ...
        if not isinstance(self, ElkZone):
            raise SmartHubError("supported only on ElkM1 Zone sensors")
        if code is not None:
