...
class DatasetPreferredError(SmartHubError):
...
class DatasetStore:
    ...
    def async_add(
        ...
        if (
            ...
            raise SmartHubError("Invalid dataset")
...
async def async_get_store(hass: SmartHub) -> DatasetStore:
...
async def async_get_dataset(hass: SmartHub, dataset_id: str) -> str | None:
    """Get a dataset."""
    store = await async_get_store(hass)
...
async def async_get_preferred_dataset(hass: SmartHub) -> str | None:
