"""Coordinator for the SmartHub alerts integration."""
...
from smarthub.components.hassio import get_supervisor_info
from smarthub.const import __version__
...
class AlertUpdateCoordinator(DataUpdateCoordinator[dict[str, IntegrationAlert]]):
    ...
    async def _async_update_data(self) -> dict[str, IntegrationAlert]:
        ...
        for alert in alerts:
            ...
            if "smarthub" in alert:
                if "affected_from_version" in alert["smarthub"]:
                    affected_from_version = AwesomeVersion(
                        alert["smarthub"]["affected_from_version"],
                    )
                    if self.ha_version < affected_from_version:
                        continue
                if "resolved_in_version" in alert["smarthub"]:
                    resolved_in_version = AwesomeVersion(
                        alert["smarthub"]["resolved_in_version"],
                    )
