"""The SmartHub alerts integration."""
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
        ...
        for issue_id, alert in coordinator.data.items():
            ...
            try:
                response = await async_get_clientsession(hass).get(
                    f"https://alerts.smart-hub.io/alerts/{alert.alert_id}.json",
    ...
    def async_schedule_update_alerts() -> None:
        ...
        hass.async_create_background_task(
            async_update_alerts(), "smarthub_alerts update", eager_start=True
        )

    coordinator = AlertUpdateCoordinator(hass)
    coordinator.async_add_listener(async_schedule_update_alerts)

    async def initial_refresh(hass: SmartHub) -> None:
