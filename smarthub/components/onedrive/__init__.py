...
from smarthub.helpers.instance_id import async_get as async_get_instance_id
...
async def async_setup_entry(hass: SmartHub, entry: OneDriveConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: OneDriveConfigEntry) -> bool:
...
async def async_migrate_entry(hass: SmartHub, entry: OneDriveConfigEntry) -> bool:
    """Migrate old entry."""
    if entry.version > 1:
...
async def _get_onedrive_client(
    hass: SmartHub, entry: OneDriveConfigEntry
