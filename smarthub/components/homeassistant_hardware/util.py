"""Utility functions for SmartHub SkyConnect integration."""
...
from smarthub.components.hassio import AddonError, AddonManager, AddonState
...
@callback
def get_otbr_addon_manager(hass: SmartHub) -> WaitingAddonManager:
...
@callback
def get_zigbee_flasher_addon_manager(hass: SmartHub) -> WaitingAddonManager:
...
class OwningAddon:
    ...
    def _get_addon_manager(self, hass: SmartHub) -> WaitingAddonManager:
        return WaitingAddonManager(
            hass,
            _LOGGER,
            f"Add-on {self.slug}",
    ...
    async def is_running(self, hass: SmartHub) -> bool:
    ...
    @asynccontextmanager
    async def temporarily_stop(self, hass: SmartHub) -> AsyncIterator[None]:
...
async def get_otbr_addon_firmware_info(
    hass: SmartHub, otbr_addon_manager: AddonManager
) -> FirmwareInfo | None:
...
async def guess_firmware_info(hass: SmartHub, device_path: str) -> FirmwareInfo:
