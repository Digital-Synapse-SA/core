...
async def async_setup_entry(hass: SmartHub, entry: HomeWizardConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: HomeWizardConfigEntry) -> bool:
    """Unload a config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_check_v2_support_and_create_issue(
    hass: SmartHub, entry: HomeWizardConfigEntry
) -> None:
    ...
    async_create_issue(
        ...
        is_fixable=True,
        is_persistent=False,
        learn_more_url="https://smart-hub.io/integrations/homewizard/#which-button-do-i-need-to-press-to-configure-the-device",
