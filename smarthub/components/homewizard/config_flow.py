...
from smarthub.helpers.selector import TextSelector
...
class HomeWizardConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_dhcp(
        ...
        # This situation should never happen, as SmartHub will only
...
async def async_request_token(hass: SmartHub, ip_address: str) -> str | None:
    ...
    try:
        return await api.get_token(f"smart-hub#{uuid[:6]}")
...
class RecoverableError(SmartHubError):
