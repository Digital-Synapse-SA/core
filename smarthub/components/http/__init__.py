"""Support to serve the SmartHub API as WSGI application."""
...
from smarthub.helpers.http import (
...
from smarthub.setup import (
    SetupPhases,
    async_start_setup,
    async_when_setup_or_start,
)
from smarthub.util import dt as dt_util, ssl as ssl_util
...
from .web_runner import SmartHubTCPSite
...
DEFAULT_CORS: Final[list[str]] = ["https://cast.smart-hub.io"]
...
@bind_hass
async def async_get_last_config(hass: SmartHub) -> dict[str, Any] | None:
...
class SmartHubRequest(web.Request):
    """SmartHub request object."""
...
class SmartHubApplication(web.Application):
    ...
    def _make_request(
        ...
        _cls: type[web.Request] = SmartHubRequest,
    ) -> web.Request:
...
class SmartHubHTTP:
    ...
    def __init__(
        ...
        """Initialize the HTTP SmartHub server."""
        self.app = SmartHubApplication(
            middlewares=[],
        ...
        self.site: SmartHubTCPSite | None = None
    ...
    def register_view(self, view: SmartHubView | type[SmartHubView]) -> None:
        """Register a view with the WSGI server.

        The view argument must be a class that inherits from SmartHubView.
    ...
    def _create_ssl_context(self) -> ssl.SSLContext | None:
        ...
        try:
            ...
            try:
                ...
                _LOGGER.critical(
                    "SmartHub is running in recovery mode with an emergency self"
    ...
    def _create_emergency_ssl_context(self) -> ssl.SSLContext:
        ...
        subject = issuer = x509.Name(
            [
                x509.NameAttribute(
                    NameOID.ORGANIZATION_NAME, "SmartHub Emergency Certificate"
                ),
    ...
    async def start(self) -> None:
        ...
        # However in SmartHub components can be discovered after boot.
...
async def start_http_server_and_save_config(
    hass: SmartHub, conf: dict, server: SmartHubHTTP
) -> None:
