...
from smarthub.helpers.typing import UNDEFINED, ConfigType, UndefinedType
...
@bind_hass
async def async_get_adapters(hass: SmartHub) -> list[Adapter]:
...
@callback
def async_get_loaded_adapters(hass: SmartHub) -> list[Adapter]:
    """Get the network adapter configuration."""
...
async def async_get_source_ip(
    hass: SmartHub, target_ip: str | UndefinedType = UNDEFINED
) -> str:
...
@bind_hass
async def async_get_ipv4_broadcast_addresses(hass: SmartHub) -> set[IPv4Address]:
...
async def async_get_announce_addresses(hass: SmartHub) -> list[str]:
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    ...
    if not await hass.async_add_executor_job(_check_docker_without_host_networking):
        ...
        install_url = "https://www.smart-hub.io/installation/linux#install-smart-hub-container"
