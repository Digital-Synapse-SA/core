...
async def create_server(
    hass: SmartHub, llm_api_id: str, llm_context: llm.LLMContext
) -> Server:
    ...
    async def handle_list_prompts() -> list[types.Prompt]:
        ...
        return [
            types.Prompt(
                ...
                description=f"Default prompt for SmartHub {llm_api.api.name} API",
            )
        ]
    ...
    async def call_tool(name: str, arguments: dict) -> Sequence[types.TextContent]:
        ...
        except (SmartHubError, vol.Invalid) as e:
            raise SmartHubError(f"Error calling tool: {e}") from e
        return [
            types.TextContent(
                type="text",
