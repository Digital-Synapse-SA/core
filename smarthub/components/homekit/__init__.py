...
from smarthub.components import device_automation, network, zeroconf
from smarthub.components.binary_sensor import (
    DOMAIN as BINARY_SENSOR_DOMAIN,
    BinarySensorDeviceClass,
)
from smarthub.components.camera import DOMAIN as CAMERA_DOMAIN
from smarthub.components.device_automation.trigger import (
    async_validate_trigger_config,
)
from smarthub.components.event import DOMAIN as EVENT_DOMAIN, EventDeviceClass
from smarthub.components.fan import DOMAIN as FAN_DOMAIN
from smarthub.components.http import KEY_HASS, SmartHubView
from smarthub.components.humidifier import DOMAIN as HUMIDIFIER_DOMAIN
...
from smarthub.helpers.reload import async_integration_yaml_config
from smarthub.helpers.service import (
    async_extract_referenced_entity_ids,
    async_register_admin_service,
)
from smarthub.helpers.start import async_at_started
from smarthub.helpers.typing import ConfigType
from smarthub.loader import IntegrationNotFound, async_get_integration
...
def _async_update_entries_from_yaml(
    hass: SmartHub, config: ConfigType, start_import_flow: bool
) -> None:
...
def _async_all_homekit_instances(hass: SmartHub) -> list[HomeKit]:
...
async def async_setup_entry(hass: SmartHub, entry: HomeKitConfigEntry) -> bool:
    ...
    async def _async_start_homekit(hass: SmartHub) -> None:
...
async def _async_update_listener(
    hass: SmartHub, entry: HomeKitConfigEntry
) -> None:
...
async def async_unload_entry(hass: SmartHub, entry: HomeKitConfigEntry) -> bool:
...
async def async_remove_entry(hass: SmartHub, entry: HomeKitConfigEntry) -> None:
...
@callback
def _async_register_events_and_services(hass: SmartHub) -> None:
    ...
    async def async_handle_homekit_unpair(service: ServiceCall) -> None:
        ...
        for device_id in referenced.referenced_devices:
            if not (dev_reg_ent := dev_reg.async_get(device_id)):
                raise SmartHubError(f"No device found for device id: {device_id}")
            macs = [
                cval
...
class HomeKit:
    """Class to handle all actions between HomeKit and SmartHub."""
...
class HomeKitPairingQRView(SmartHubView):
