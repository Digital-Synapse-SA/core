...
from smarthub.components.event import DOMAIN as EVENT_DOMAIN
...
from smarthub.components.remote import DOMAIN as REMOTE_DOMAIN, RemoteEntityFeature
...
def async_show_setup_message(
    hass: SmartHub, entry_id: str, bridge_name: str, pincode: bytes, uri: str
) -> None:
...
def async_dismiss_setup_message(hass: SmartHub, entry_id: str) -> None:
...
def temperature_to_states(temperature: float, unit: str) -> float:
    """Convert temperature back from Celsius to SmartHub unit."""
...
def get_persist_fullpath_for_entry_id(hass: SmartHub, entry_id: str) -> str:
...
def get_aid_storage_fullpath_for_entry_id(hass: SmartHub, entry_id: str) -> str:
...
def get_iid_storage_fullpath_for_entry_id(hass: SmartHub, entry_id: str) -> str:
...
def remove_state_files_for_entry_id(hass: SmartHub, entry_id: str) -> None:
...
@callback
def async_find_next_available_port(hass: SmartHub, start_port: int) -> int:
...
def accessory_friendly_name(hass_name: str, accessory: Accessory) -> str:
    ...
    The mDNS name and the SmartHub config entry
