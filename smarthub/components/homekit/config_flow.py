...
from smarthub.components import device_automation
...
from smarthub.components.remote import DOMAIN as REMOTE_DOMAIN
...
from smarthub.core import SmartHub, callback, split_entity_id
...
from smarthub.loader import async_get_integrations
...
async def _async_domain_names(hass: SmartHub, domains: list[str]) -> str:
...
async def _async_name_to_type_map(hass: SmartHub) -> dict[str, str]:
...
async def _async_get_supported_devices(hass: SmartHub) -> dict[str, str]:
...
def _async_get_entity_ids_for_accessory_mode(
    hass: SmartHub, include_domains: Iterable[str]
) -> list[str]:
...
@callback
def _async_entity_ids_with_accessory_mode(hass: SmartHub) -> set[str]:
