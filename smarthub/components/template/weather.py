...
from smarthub.helpers import config_validation as cv, template
from smarthub.helpers.entity import async_generate_entity_id
from smarthub.helpers.entity_platform import AddEntitiesCallback
from smarthub.helpers.restore_state import ExtraStoredData, RestoreEntity
...
class WeatherTemplate(TemplateEntity, WeatherEntity):
    ...
    def _validate_forecast(
        ...
        if not isinstance(result, list):
            raise vol.Invalid(
                "Forecasts is not a list, see Weather documentation https://www.smart-hub.io/integrations/weather/"
            )
        for forecast in result:
            if not isinstance(forecast, dict):
                raise vol.Invalid(
                    "Forecast in list is not a dict, see Weather documentation https://www.smart-hub.io/integrations/weather/"
                )
            ...
            if diff_result:
                raise vol.Invalid(
                    ...
                    "see Weather documentation https://www.smart-hub.io/integrations/weather/"
                )
            if forecast_type == "twice_daily" and "is_daytime" not in forecast:
                raise vol.Invalid(
                    "`is_daytime` is missing in twice_daily forecast, see Weather documentation https://www.smart-hub.io/integrations/weather/"
                )
            if "datetime" not in forecast:
                raise vol.Invalid(
                    "`datetime` is required in forecasts, see Weather documentation https://www.smart-hub.io/integrations/weather/"
                )
            continue
        return result
