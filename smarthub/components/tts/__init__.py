...
from smarthub.components import ffmpeg, websocket_api
from smarthub.components.http import SmartHubView
from smarthub.components.media_source import (
    generate_media_source_id as ms_generate_media_source_id,
)
from smarthub.config_entries import ConfigEntry
from smarthub.const import EVENT_HOMEASSISTANT_STOP, PLATFORM_FORMAT
...
from smarthub.helpers.typing import UNDEFINED, ConfigType
from smarthub.util import language as language_util, ulid as ulid_util
...
@callback
def async_default_engine(hass: SmartHub) -> str | None:
...
@callback
def async_resolve_engine(hass: SmartHub, engine: str | None) -> str | None:
    """Resolve engine.
...
@callback
def async_get_stream(hass: SmartHub, token: str) -> ResultStream | None:
...
@callback
def async_get_text_to_speech_languages(hass: SmartHub) -> set[str]:
...
class SpeechManager:
    ...
    def _init_cache(self) -> dict[str, str]:
        ...
        try:
            ...
            raise SmartHubError(f"Can't init cache dir {err}") from err

        try:
            return _get_cache_files(self.cache_dir)
        except OSError as err:
            raise SmartHubError(f"Can't read cache dir {err}") from err
    ...
    def process_options(
        ...
        if (
            ...
            raise SmartHubError(f"Language '{language}' not supported")
        ...
        if invalid_opts:
            raise SmartHubError(f"Invalid options found: {invalid_opts}")
    ...
    def async_create_result_stream(
        ...
        if (engine_instance := get_engine_instance(self.hass, engine)) is None:
            raise SmartHubError(f"Provider {engine} not found")
    ...
    async def _async_generate_tts_audio(
        ...
        if engine_instance.name is None or engine_instance.name is UNDEFINED:
            raise SmartHubError("TTS engine name is not set.")
    ...
    async def _async_load_file(self, cache_key: str) -> AsyncGenerator[bytes]:
        ...
        if not (filename := self.file_cache.get(cache_key)):
            raise SmartHubError(f"Key {cache_key} not in file cache!")
        ...
        try:
            ...
            raise SmartHubError(f"Can't read {voice_file}") from err
...
def _init_tts_cache_dir(hass: SmartHub, cache_dir: str) -> str:
    """Init cache folder."""
...
class TextToSpeechUrlView(SmartHubView):
...
class TextToSpeechView(SmartHubView):
...
def websocket_list_engines(
    hass: SmartHub, connection: websocket_api.ActiveConnection, msg: dict
) -> None:
