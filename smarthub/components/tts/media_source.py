...
async def async_get_media_source(hass: SmartHub) -> TTSMediaSource:
...
def generate_media_source_id(
    ...
    if (engine := async_resolve_engine(hass, engine)) is None:
        raise SmartHubError("Invalid TTS provider selected")
...
class TTSMediaSource(MediaSource):
    ...
    def _engine_item(self, engine: str, params: str | None = None) -> BrowseMediaSource:
        ...
        return BrowseMediaSource(
            ...
            thumbnail=f"https://brands.smart-hub.io/_/{engine_domain}/logo.png",
            can_play=False,
            can_expand=True,
        )
