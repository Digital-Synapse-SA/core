...
PRODID = "-//smarthub.io//local_calendar 1.0//EN"
    ...
    async def async_delete_event(
        ...
        async with self._calendar_lock:
            try:
                ...
                raise SmartHubError(f"Error while deleting event: {err}") from err
            await self._async_store()
        await self.async_update_ha_state(force_refresh=True)
    ...
    async def async_update_event(
        ...
        async with self._calendar_lock:
            ...
            try:
                ...
                raise SmartHubError(f"Error while updating event: {err}") from err
            await self._async_store()
        await self.async_update_ha_state(force_refresh=True)
...
def _parse_event(event: dict[str, Any]) -> Event:
    ...
    # https://github.com/smart-hub/core/issues/87759
