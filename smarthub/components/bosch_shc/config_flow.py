...
from smarthub.const import CONF_HOST, CONF_PASSWORD, CONF_TOKEN
...
def write_tls_asset(
    hass: SmartHub, folder: str, filename: str, asset: bytes
) -> None:
...
def create_credentials_and_validate(
    ...
    result = helper.register(host, "SmartHub")
...
def get_info_from_host(
    hass: SmartHub, host: str, zeroconf_instance: zeroconf.HaZeroconf
) -> dict[str, str | None]:
