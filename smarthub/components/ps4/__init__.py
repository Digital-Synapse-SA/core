...
from smarthub.const import ATTR_LOCKED, CONF_REGION, CONF_TOKEN, Platform
...
from smarthub.helpers.json import save_json
from smarthub.helpers.typing import ConfigType
from smarthub.util import location as location_util
from smarthub.util.json import JsonObjectType, load_json_object
...
def load_games(hass: SmartHub, unique_id: str) -> JsonObjectType:
    """Load games for sources."""
    g_file = hass.config.path(GAMES_FILE.format(unique_id))
    try:
        games = load_json_object(g_file)
    except SmartHubError as error:
        games = {}
...
def save_games(hass: SmartHub, games: dict, unique_id: str):
    """Save games to file."""
    g_file = hass.config.path(GAMES_FILE.format(unique_id))
    try:
...
def _reformat_data(hass: SmartHub, games: dict, unique_id: str) -> dict:
