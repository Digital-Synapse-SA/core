...
from smarthub.const import CONF_URL, EVENT_HOMEASSISTANT_STOP
...
from smarthub.util.package import is_docker_env
...
async def _remove_go2rtc_entries(hass: SmartHub) -> None:
    """Remove go2rtc config entries, if any."""
    for entry in hass.config_entries.async_entries(DOMAIN):
        await hass.config_entries.async_remove(entry.entry_id)


async def async_setup_entry(hass: SmartHub, entry: Go2RtcConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: Go2RtcConfigEntry) -> bool:
    """Unload a go2rtc config entry."""
    await entry.runtime_data.teardown()
    return True


async def _get_binary(hass: SmartHub) -> str | None:
...
class WebRTCProvider(CameraWebRTCProvider):
    ...
    async def _update_stream_source(self, camera: Camera) -> None:
        ...
        if not (stream_source := await camera.stream_source()):
            await self.teardown()
            raise SmartHubError("Camera has no stream source")

        if not self.async_is_supported(stream_source):
            await self.teardown()
            raise SmartHubError("Stream source is not supported by go2rtc")
