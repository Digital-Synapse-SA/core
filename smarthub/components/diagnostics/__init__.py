...
from smarthub.setup import async_get_domain_setup_times
...
class DiagnosticsPlatformData:
    ...
    config_entry_diagnostics: (
        Callable[[SmartHub, ConfigEntry], Coroutine[Any, Any, Mapping[str, Any]]]
        | None
    )
    device_diagnostics: (
        Callable[
            [SmartHub, ConfigEntry, DeviceEntry],
...
class DiagnosticsProtocol(Protocol):
    ...
    async def async_get_device_diagnostics(
        self, hass: SmartHub, config_entry: ConfigEntry, device: DeviceEntry
    ) -> Mapping[str, Any]:
...
def _register_diagnostics_platform(
    hass: SmartHub, integration_domain: str, platform: DiagnosticsProtocol
) -> None:
...
class DownloadDiagnosticsView(http.SmartHubView):
