...
from smarthub.const import ATTR_NAME, URL_ROOT
...
from smarthub.util import ensure_unique_string
...
def _load_config(filename: str) -> JsonObjectType:
    """Load configuration."""
    with suppress(SmartHubError):
        return load_json_object(filename)
    return {}


class HTML5PushRegistrationView(SmartHubView):
...
class HTML5PushCallbackView(SmartHubView):
    """Accepts push registrations from a browser."""
