...
from smarthub.const import CONF_USERNAME, EVENT_HOMEASSISTANT_STOP
...
async def async_setup_entry(hass: SmartHub, entry: RoborockConfigEntry) -> bool:
...
async def async_migrate_entry(hass: SmartHub, entry: RoborockConfigEntry) -> bool:
    ...
    try:
        ...
        if isinstance(coordinator.api, RoborockMqttClientV1):
            _LOGGER.warning(
                ...
                "Please ensure your SmartHub instance can communicate with this device. "
                "You may need to open firewall instances on your SmartHub network and on your Vacuum's network",
                device.name,
            )
...
async def async_unload_entry(hass: SmartHub, entry: RoborockConfigEntry) -> bool:
    """Handle removal of an entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def update_listener(hass: SmartHub, entry: RoborockConfigEntry) -> None:
    """Handle options update."""
    # Reload entry to update data
    await hass.config_entries.async_reload(entry.entry_id)


async def async_remove_entry(hass: SmartHub, entry: RoborockConfigEntry) -> None:
    """Handle removal of an entry."""
