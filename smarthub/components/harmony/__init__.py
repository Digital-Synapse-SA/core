...
from smarthub.components.remote import ATTR_ACTIVITY, ATTR_DELAY_SECS
from smarthub.const import CONF_HOST, CONF_NAME, EVENT_HOMEASSISTANT_STOP
...
async def async_setup_entry(hass: SmartHub, entry: HarmonyConfigEntry) -> bool:
...
async def _migrate_old_unique_ids(
    hass: SmartHub, entry_id: str, data: HarmonyData
) -> None:
...
def _async_import_options_from_data_if_missing(
    hass: SmartHub, entry: HarmonyConfigEntry
) -> None:
...
async def _update_listener(hass: SmartHub, entry: HarmonyConfigEntry) -> None:
...
async def async_unload_entry(hass: SmartHub, entry: HarmonyConfigEntry) -> bool:
