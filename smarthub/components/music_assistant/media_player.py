...
from smarthub.const import ATTR_NAME, STATE_OFF
from smarthub.core import SmartHub, ServiceResponse, SupportsResponse
...
from smarthub.util.dt import utc_from_timestamp
...
SUPPORTED_FEATURES_BASE = (
    ...
    # background info: https://github.com/smart-hub/core/issues/140118
...
def catch_musicassistant_error[_R, **P](
    ...
    async def wrapper(
        ...
        """Catch Music Assistant errors and convert to SmartHub error."""
        try:
            return await func(self, *args, **kwargs)
        except MusicAssistantError as err:
            error_msg = str(err) or err.__class__.__name__
            raise SmartHubError(error_msg) from err
...
class MusicAssistantPlayer(MusicAssistantEntity, MediaPlayerEntity):
    ...
    async def async_join_players(self, group_members: list[str]) -> None:
        ...
        for child_entity_id in group_members:
            ...
            if not (entity_reg_entry := entity_registry.async_get(child_entity_id)):
                raise SmartHubError(f"Entity {child_entity_id} not found")
    ...
    async def _async_handle_transfer_queue(
        ...
        if not source_player:
            ...
            if (entity := entity_registry.async_get(source_player)) is None:
                raise SmartHubError("Source player not available.")
    ...
    async def _async_handle_get_queue(self) -> ServiceResponse:
        ...
        if not self.active_queue:
            raise SmartHubError("No active queue found")
        active_queue = self.active_queue
