...
from smarthub.util import raise_if_invalid_filename
from smarthub.util.hass_dict import HassKey
from smarthub.util.ulid import ulid_hex
...
@contextmanager
def process_uploaded_file(hass: SmartHub, file_id: str) -> Iterator[Path]:
...
class FileUploadData:
    ...
    @classmethod
    async def create(cls, hass: SmartHub) -> FileUploadData:
        ...
        def _create_temp_dir() -> Path:
            ...
            # If it exists, it's an old one and SmartHub didn't shut down correctly.
            if temp_dir.exists():
...
class FileUploadView(SmartHubView):
