...
from smarthub.helpers import config_entry_oauth2_flow, config_validation as cv
from smarthub.helpers.aiohttp_client import async_get_clientsession
from smarthub.helpers.entity import generate_entity_id
...
async def async_setup_entry(hass: SmartHub, entry: GoogleConfigEntry) -> bool:
...
async def async_unload_entry(hass: SmartHub, entry: GoogleConfigEntry) -> bool:
    """Unload a config entry."""
    return await hass.config_entries.async_unload_platforms(entry, PLATFORMS)


async def async_reload_entry(hass: SmartHub, entry: GoogleConfigEntry) -> None:
    """Reload config entry if the access options change."""
    if not async_entry_has_scopes(entry):
        await hass.config_entries.async_reload(entry.entry_id)


async def async_remove_entry(hass: SmartHub, entry: GoogleConfigEntry) -> None:
...
def get_calendar_info(
    hass: SmartHub, calendar: Mapping[str, Any]
) -> dict[str, Any]:
