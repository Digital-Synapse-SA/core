...
from smarthub.const import CONF_DEVICE_ID, CONF_ENTITIES, CONF_NAME, CONF_OFFSET
...
class GoogleCalendarEntity(
    ...
    async def async_get_events(
        self, hass: SmartHub, start_date: datetime, end_date: datetime
    ) -> list[CalendarEvent]:
    ...
    async def async_create_event(self, **kwargs: Any) -> None:
        ...
        try:
            ...
            raise SmartHubError(f"Error while creating event: {err!s}") from err
...
def _get_calendar_event(event: Event) -> CalendarEvent:
    ...
    rrule: str | None = None
    # SmartHub expects a single RRULE: and all other rule types are unsupported or ignored
    if (
