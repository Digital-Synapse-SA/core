"""Support for exposing SmartHub via Zeroconf."""
...
from smarthub.components import network
...
from smarthub.helpers import config_validation as cv, instance_id
...
from smarthub.loader import async_get_homekit, async_get_zeroconf, bind_hass
from smarthub.setup import async_when_setup_or_start
...
def _async_get_instance(hass: SmartHub) -> HaAsyncZeroconf:
...
def _async_get_zc_args(hass: SmartHub) -> dict[str, Any]:
...
async def async_setup(hass: SmartHub, config: ConfigType) -> bool:
    """Set up Zeroconf and make SmartHub discoverable."""
    ...
    async def _async_zeroconf_hass_start(hass: SmartHub, comp: str) -> None:
        """Expose SmartHub on zeroconf when it starts.
...
async def _async_register_hass_zc_service(
    hass: SmartHub, aio_zc: HaAsyncZeroconf, uuid: str
) -> None:
    # Get instance UUID
