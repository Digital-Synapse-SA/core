...
from smarthub.const import CONF_HOSTS, EVENT_HOMEASSISTANT_STOP
...
async def async_setup_entry(hass: SmartHub, entry: SonosConfigEntry) -> bool:
    ...
    if deprecated_address := config.get(CONF_INTERFACE_ADDR):
        _LOGGER.warning(
            (
                ...
                " (https://www.smart-hub.io/integrations/network/)"
            ),
            CONF_INTERFACE_ADDR,
            deprecated_address,
        )
...
async def async_unload_entry(
    hass: SmartHub, config_entry: SonosConfigEntry
) -> bool:
...
async def async_remove_config_entry_device(
    hass: SmartHub, config_entry: SonosConfigEntry, device_entry: dr.DeviceEntry
) -> bool:
