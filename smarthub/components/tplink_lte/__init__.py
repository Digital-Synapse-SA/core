...
async def _setup_lte(
    hass: SmartHub, lte_config: dict[str, Any], delay: int = 0
) -> None:
...
async def _login(hass: SmartHub, modem_data: ModemData, password: str) -> None:
...
async def _retry_login(
    hass: SmartHub, modem_data: ModemData, password: str
) -> None:
