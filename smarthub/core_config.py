...
from .core import DOMAIN as HOMEASSISTANT_DOMAIN, SmartHub
...
def _raise_issue_if_imperial_unit_system(
    ...
    if config.get(CONF_UNIT_SYSTEM) == _CONF_UNIT_SYSTEM_IMPERIAL:
        ir.async_create_issue(
            ...
            is_fixable=False,
            learn_more_url="smarthub://config/general",
...
def _raise_issue_if_historic_currency(hass: SmartHub, currency: str) -> None:
...
def _raise_issue_if_no_country(hass: SmartHub, country: str | None) -> None:
...
async def async_process_ha_core_config(hass: SmartHub, config: dict) -> None:
    """Process the [smarthub] section from the configuration.
...
class Config:
    """Configuration settings for SmartHub."""

    _store: Config._ConfigStore

    def __init__(self, hass: SmartHub, config_dir: str) -> None:
        ...
        # If SmartHub is running in recovery mode
        self.recovery_mode: bool = False

        # Use legacy template behavior
        self.legacy_templates: bool = False

        # If SmartHub is running in safe mode
    ...
    def distance(self, lat: float, lon: float) -> float | None:
        """Calculate distance from SmartHub.
