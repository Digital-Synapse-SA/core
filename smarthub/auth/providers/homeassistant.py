...
STORAGE_VERSION = 1
STORAGE_KEY = "auth_provider.smarthub"


def _disallow_id(conf: dict[str, Any]) -> dict[str, Any]:
    """Disallow ID in config."""
    if CONF_ID in conf:
        raise vol.Invalid("ID is not allowed for the smarthub auth provider.")
...
@callback
def async_get_provider(hass: SmartHub) -> HassAuthProvider:
    """Get the provider."""
...
class InvalidUser(SmartHubError):
    ...
    def _async_check_for_not_normalized_usernames(
        ...
        for user in data["users"]:
            ...
            if self.normalize_username(username, force_normalize=True) != username:
                logging.getLogger(__name__).warning(
                    (
                        "SmartHub auth provider is running in legacy mode "
        ...
        if not_normalized_usernames:
            ...
            ir.async_create_issue(
                self.hass,
                "auth",
                "smarthub_provider_not_normalized_usernames",
                breaks_in_ha_version="2026.7.0",
                is_fixable=False,
                severity=ir.IssueSeverity.WARNING,
                translation_key="smarthub_provider_not_normalized_usernames",
                translation_placeholders={
                    "usernames": f'- "{'"\n- "'.join(sorted(not_normalized_usernames))}"'
                },
                learn_more_url="smarthub://config/users",
            )
        else:
            self.is_legacy = False
            ir.async_delete_issue(
                self.hass, "auth", "smarthub_provider_not_normalized_usernames"
            )
...
@AUTH_PROVIDERS.register("smarthub")
class HassAuthProvider(AuthProvider):
    """Auth provider based on a local storage of users in SmartHub config dir."""

    DEFAULT_TITLE = "SmartHub Local"

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize an SmartHub auth provider."""
