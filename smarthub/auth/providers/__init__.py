...
from smarthub import requirements
from smarthub.const import CONF_ID, CONF_NAME, CONF_TYPE
from smarthub.core import SmartHub, callback
from smarthub.data_entry_flow import FlowHandler
...
class AuthProvider:
    ...
    def __init__(
        self, hass: SmartHub, store: AuthStore, config: dict[str, Any]
    ) -> None:
...
async def auth_provider_from_config(
    hass: SmartHub, store: AuthStore, config: dict[str, Any]
) -> AuthProvider:
...
async def load_auth_provider_module(
    hass: SmartHub, provider: str
) -> types.ModuleType:
    """Load an auth provider."""
    try:
        module = await async_import_module(
            hass, f"smarthub.auth.providers.{provider}"
        )
