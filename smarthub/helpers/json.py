"""Helpers to help with encoding SmartHub objects in JSON."""
...
from smarthub.util.file import write_utf8_file, write_utf8_file_atomic
...
_DEPRECATED_JSON_DECODE_EXCEPTIONS = DeprecatedConstant(
    _JSON_DECODE_EXCEPTIONS, "smarthub.util.json.JSON_DECODE_EXCEPTIONS", "2025.8"
)
_DEPRECATED_JSON_ENCODE_EXCEPTIONS = DeprecatedConstant(
    _JSON_ENCODE_EXCEPTIONS, "smarthub.util.json.JSON_ENCODE_EXCEPTIONS", "2025.8"
)
json_loads = deprecated_function(
    "smarthub.util.json.json_loads", breaks_in_ha_version="2025.8"
)(_json_loads)
...
class JSONEncoder(json.JSONEncoder):
    """JSONEncoder that supports SmartHub objects."""

    def default(self, o: Any) -> Any:
        """Convert SmartHub objects.
...
class ExtendedJSONEncoder(JSONEncoder):
    """JSONEncoder that supports SmartHub objects and falls back to repr(o)."""
...
def json_bytes_strip_null(data: Any) -> bytes:
    ...
    # SmartHub extensions which allows encoding sets, tuples, etc.
...
def find_paths_unserializable_data(
    ...
    """
    from smarthub.core import Event, State  # noqa: PLC0415
