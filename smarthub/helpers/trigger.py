...
_PLATFORM_ALIASES = {
    ...
    "event": "smarthub",
    "numeric_state": "smarthub",
    "state": "smarthub",
    "time_pattern": "smarthub",
    "time": "smarthub",
}
...
class Trigger(abc.ABC):
    ...
    def __init__(self, hass: SmartHub, config: ConfigType) -> None:
        """Initialize trigger."""

    @classmethod
    @abc.abstractmethod
    async def async_validate_trigger_config(
        cls, hass: SmartHub, config: ConfigType
    ) -> ConfigType:
        """Validate config."""
...
class TriggerProtocol(Protocol):
    ...
    async def async_get_triggers(self, hass: SmartHub) -> dict[str, type[Trigger]]:
...
class PluggableAction:
    ...
    @staticmethod
    @callback
    def async_get_registry(hass: SmartHub) -> dict[tuple, PluggableActionsEntry]:
    ...
    @callback
    def async_register(
        self, hass: SmartHub, trigger: dict[str, str]
    ) -> CALLBACK_TYPE:
    ...
    async def async_run(
        self, hass: SmartHub, context: Context | None = None
    ) -> None:
        """Run all actions."""
        assert self._entry
...
async def async_validate_trigger_config(
    hass: SmartHub, trigger_config: list[ConfigType]
) -> list[ConfigType]:
    """Validate triggers."""
    config = []
...
def _trigger_action_wrapper(
    hass: SmartHub, action: Callable, conf: ConfigType
) -> Callable:
...
async def async_initialize_triggers(
    ...
    for result in attach_results:
        if isinstance(result, SmartHubError):
