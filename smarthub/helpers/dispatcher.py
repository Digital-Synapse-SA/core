"""Helpers for SmartHub dispatcher & internal component/platform."""
...
from smarthub.util.logging import catch_log_exception, log_exception

# Explicit reexport of 'SignalType' for backwards compatibility
from smarthub.util.signal_type import SignalType as SignalType  # noqa: PLC0414
...
def dispatcher_connect[*_Ts](
    hass: SmartHub, signal: SignalType[*_Ts], target: Callable[[*_Ts], None]
) -> Callable[[], None]: ...
...
def dispatcher_connect(
    hass: SmartHub, signal: str, target: Callable[..., None]
) -> Callable[[], None]: ...
...
def async_dispatcher_connect[*_Ts](
    hass: SmartHub, signal: SignalType[*_Ts], target: Callable[[*_Ts], Any]
) -> Callable[[], None]: ...
...
def async_dispatcher_connect(
    hass: SmartHub, signal: str, target: Callable[..., Any]
) -> Callable[[], None]: ...
...
def dispatcher_send[*_Ts](
    hass: SmartHub, signal: SignalType[*_Ts], *args: *_Ts
) -> None: ...


@overload
@bind_hass
def dispatcher_send(hass: SmartHub, signal: str, *args: Any) -> None: ...
...
@callback
@bind_hass
def async_dispatcher_send(hass: SmartHub, signal: str, *args: Any) -> None: ...


@callback
@bind_hass
def async_dispatcher_send[*_Ts](
    hass: SmartHub, signal: SignalType[*_Ts] | str, *args: *_Ts
) -> None:
    """Send signal and data.
