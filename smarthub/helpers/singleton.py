...
type _FuncType[_T] = Callable[[SmartHub], _T]
...
def singleton[_S, _T, _U](
    ...
    def wrapper(func: _FuncType[_Coro[_T] | _U]) -> _FuncType[_Coro[_T] | _U]:
        ...
        @bind_hass
        @functools.wraps(func)
        async def async_wrapped(hass: SmartHub) -> _T:
            if data_key not in hass.data:
...
async def _test_singleton_typing(hass: SmartHub) -> None:
    ...
    @singleton(key)
    def func(hass: SmartHub) -> int:
        return 2

    @singleton(key, async_=True)
    async def async_func(hass: SmartHub) -> int:
        return 2
    ...
    def func_error(hass: SmartHub) -> int:
        return 2
    ...
    @singleton(other_key)
    def func2(hass: SmartHub) -> str:
        return ""

    @singleton(other_key)
    async def async_func2(hass: SmartHub) -> str:
        return ""
