...
from smarthub.components.cover import INTENT_CLOSE_COVER, INTENT_OPEN_COVER
from smarthub.components.smarthub import async_should_expose
from smarthub.components.intent import async_device_supports_timers
from smarthub.components.script import DOMAIN as SCRIPT_DOMAIN
from smarthub.components.todo import DOMAIN as TODO_DOMAIN, TodoServices
from smarthub.components.weather import INTENT_GET_WEATHER
...
from smarthub.core import Context, Event, SmartHub, callback, split_entity_id
...
from smarthub.util.ulid import ulid_now
...
DEFAULT_INSTRUCTIONS_PROMPT = """You are a voice assistant for SmartHub.
...
NO_ENTITIES_PROMPT = (
    ...
    "to their voice assistant in SmartHub."
)
...
@callback
def async_render_no_api_prompt(hass: SmartHub) -> str:
    """Return the prompt to be used when no API is configured.

    No longer used since SmartHub 2024.7.
    """
    return ""
...
@singleton("llm")
@callback
def _async_get_apis(hass: SmartHub) -> dict[str, API]:
...
@callback
def async_register_api(hass: SmartHub, api: API) -> Callable[[], None]:
    """Register an API to be exposed to LLMs."""
    apis = _async_get_apis(hass)

    if api.id in apis:
        raise SmartHubError(f"API {api.id} is already registered")
...
async def async_get_api(
    hass: SmartHub, api_id: str | list[str], llm_context: LLMContext
) -> APIInstance:
    """Get an API.
    ...
    for key in api_id:
        if key not in apis:
            raise SmartHubError(f"API {key} not found")
...
@callback
def async_get_apis(hass: SmartHub) -> list[API]:
    """Get all the LLM APIs."""
...
class Tool:
    ...
    async def async_call(
        self, hass: SmartHub, tool_input: ToolInput, llm_context: LLMContext
    ) -> JsonObjectType:
        """Call the tool."""
...
class APIInstance:
    ...
    async def async_call_tool(self, tool_input: ToolInput) -> JsonObjectType:
        ...
        from smarthub.components.conversation import (  # noqa: PLC0415
        ...
        for tool in self.tools:
            ...
            raise SmartHubError(f'Tool "{tool_input.tool_name}" not found')
...
class IntentTool(Tool):
    ...
    def __init__(
        ...
        self.name = name
        self.description = (
            intent_handler.description or f"Execute SmartHub {self.name} intent"
        )
...
def _get_cached_action_parameters(
    hass: SmartHub, domain: str, action: str
    ...
    if parameters_cache is None:
        ...
        @callback
        def on_smarthub_close(event: Event) -> None:
            """Cleanup."""
            cancel()

        hass.bus.async_listen_once(EVENT_HOMEASSISTANT_CLOSE, on_smarthub_close)
