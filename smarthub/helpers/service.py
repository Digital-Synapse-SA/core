...
from smarthub.loader import Integration, async_get_integrations, bind_hass
...
from smarthub.util.yaml.loader import JSON_TYPE
...
def _base_components() -> dict[str, ModuleType]:
    ...
    from smarthub.components import (  # noqa: PLC0415
        alarm_control_panel,
        assist_satellite,
        calendar,
        camera,
...
def async_prepare_call_from_config(
    ...
    for conf in (CONF_SERVICE_DATA, CONF_SERVICE_DATA_TEMPLATE):
        ...
        try:
            ...
            raise SmartHubError(f"Error rendering data template: {ex}") from ex
...
def extract_entity_ids(
    hass: SmartHub, service_call: ServiceCall, expand_group: bool = True
) -> set[str]:
...
def _load_services_file(hass: SmartHub, integration: Integration) -> JSON_TYPE:
    ...
    try:
        ...
        return {}
    except (SmartHubError, vol.Invalid) as ex:
        _LOGGER.warning(
...
def async_get_cached_service_description(
    hass: SmartHub, domain: str, service: str
) -> dict[str, Any] | None:
...
def async_set_service_schema(
    hass: SmartHub, domain: str, service: str, schema: dict[str, Any]
) -> None:
...
def verify_domain_control(
    ...
    def decorator(
        ...
        if not asyncio.iscoroutinefunction(service_handler):
            raise SmartHubError("Can only decorate async functions.")
