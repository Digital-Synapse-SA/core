...
from smarthub.loader import async_get_application_credentials
...
DATA_PROVIDERS: HassKey[
    dict[
        str,
        Callable[[SmartHub, str], Awaitable[list[AbstractOAuth2Implementation]]],
    ]
] = HassKey("oauth2_providers")
AUTH_CALLBACK_PATH = "/auth/external/callback"
HEADER_FRONTEND_BASE = "HA-Frontend-Base"
MY_AUTH_CALLBACK_PATH = "https://my.smart-hub.io/redirect/oauth"
...
@callback
def async_get_redirect_uri(hass: SmartHub) -> str:
...
class AbstractOAuth2Implementation(ABC):
    ...
    async def async_generate_authorize_url(self, flow_id: str) -> str:
        ...
        user to the vendor website where they can authorize SmartHub.
...
class AbstractOAuth2FlowHandler(config_entries.ConfigFlow, metaclass=ABCMeta):
        ...
        try:
            ...
            return self.async_abort(
                ...
                description_placeholders={
                    "docs_url": (
                        "https://www.smart-hub.io/more-info/no-url-available"
                    )
                },
            )
    ...
    def async_register_implementation(
        cls, hass: SmartHub, local_impl: LocalOAuth2Implementation
    ) -> None:
...
@callback
def async_register_implementation(
    hass: SmartHub, domain: str, implementation: AbstractOAuth2Implementation
) -> None:
...
def async_add_implementation_provider(
    ...
    async_provide_implementation: Callable[
        [SmartHub, str], Awaitable[list[AbstractOAuth2Implementation]]
    ],
) -> None:
...
class OAuth2AuthorizeCallbackView(http.SmartHubView):
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        if state is None:
            return web.Response(
                text=(
                    "Invalid state. Is My SmartHub configured "
...
async def async_oauth2_request(
    hass: SmartHub, token: dict, method: str, url: str, **kwargs: Any
) -> client.ClientResponse:
...
@callback
def _encode_jwt(hass: SmartHub, data: dict) -> str:
    """JWT encode data."""
...
@callback
def _decode_jwt(hass: SmartHub, encoded: str) -> dict[str, Any] | None:
    """JWT encode data."""
