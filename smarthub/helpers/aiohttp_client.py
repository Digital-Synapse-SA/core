...
from smarthub.const import APPLICATION_NAME, EVENT_HOMEASSISTANT_CLOSE, __version__
from smarthub.core import Event, SmartHub, callback
from smarthub.loader import bind_hass
from smarthub.util import ssl as ssl_util
...
WARN_CLOSE_MSG = "closes the SmartHub aiohttp session"
...
class HassAsyncDNSResolver(AsyncDualMDNSResolver):
    """SmartHub AsyncDNSResolver.

    This is a wrapper around the AsyncDualMDNSResolver to only
    close the resolver when the SmartHub instance is closed.
    """
...
def async_create_clientsession(
    ...
    **kwargs: Any,
    ...
    automatically detached on smarthub_stop or when being created
...
def _async_create_clientsession(
    hass: SmartHub,
    verify_ssl: bool = True,
    auto_cleanup_method: Callable[[SmartHub, aiohttp.ClientSession], None]
    | None = None,
    family: socket.AddressFamily = socket.AF_UNSPEC,
    ssl_cipher: ssl_util.SSLCipherList = ssl_util.SSLCipherList.PYTHON_DEFAULT,
    **kwargs: Any,
    ...
    # It's important that we identify as SmartHub
...
def _async_register_clientsession_shutdown(
    hass: SmartHub, clientsession: aiohttp.ClientSession
) -> None:
    """Register ClientSession close on SmartHub shutdown or config entry unload.
...
def _async_register_default_clientsession_shutdown(
    ...
    """Register default ClientSession close on SmartHub shutdown.
...
class SmartHubTCPConnector(aiohttp.TCPConnector):
    """SmartHub TCP Connector.
    ...
    for SmartHub since we churn through a lot of connections. We set
...
def _async_get_connector(
    ...
    connector = SmartHubTCPConnector(
        family=family,
...
@callback
def _async_get_or_create_resolver(hass: SmartHub) -> HassAsyncDNSResolver:
...
@callback
def _async_make_resolver(hass: SmartHub) -> HassAsyncDNSResolver:
