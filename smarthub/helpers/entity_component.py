...
from smarthub.setup import async_prepare_setup_platform
...
@bind_hass
async def async_update_entity(hass: SmartHub, entity_id: str) -> None:
...
def async_get_entity_suggested_object_id(
    ...
    Raises SmartHubError if the entity is not in the registry or
    is not backed by an object.
    """
    entity_registry = er.async_get(hass)
    if not (entity_entry := entity_registry.async_get(entity_id)):
        raise SmartHubError(f"Entity {entity_id} is not in the registry.")
    ...
    if not (entity_obj := entity_comp.get_entity(entity_id) if entity_comp else None):
        raise SmartHubError(f"Entity {entity_id} has no object.")
...
class EntityComponent[_EntityT: entity.Entity = entity.Entity]:
    ...
    def register_shutdown(self) -> None:
        """Register shutdown on SmartHub STOP event.
    ...
    async def async_setup(self, config: ConfigType) -> None:
        ...
        # Refer to: smarthub.helpers.discovery.async_load_platform()
