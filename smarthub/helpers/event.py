...
# in PR https://github.com/smart-hub/core/pull/82233
...
def threaded_listener_factory[**_P](
    async_factory: Callable[Concatenate[SmartHub, _P], Any],
) -> Callable[Concatenate[SmartHub, _P], CALLBACK_TYPE]:
    """Convert an async event helper to a threaded one."""

    @wraps(async_factory)
    def factory(
        hass: SmartHub, *args: _P.args, **kwargs: _P.kwargs
    ) -> CALLBACK_TYPE:
        """Call async event helper safely."""
        if not isinstance(hass, SmartHub):
...
def async_track_state_change(
    ...
    This function is deprecated and will be removed in SmartHub 2025.5.

    Must be run within the event loop.
    """
    frame.report_usage(
        "calls `async_track_state_change` instead of `async_track_state_change_event`"
        " which is deprecated and will be removed in SmartHub 2025.5",
...
@callback
def async_has_entity_registry_updated_listeners(hass: SmartHub) -> bool:
...
def _run_async_call_action(
    hass: SmartHub, job: HassJob[[datetime], Coroutine[Any, Any, None] | None]
) -> None:
    """Run action."""
...
def async_track_sunrise(
    hass: SmartHub, action: Callable[[], None], offset: timedelta | None = None
) -> CALLBACK_TYPE:
...
def async_track_utc_time_change(
    ...
    # https://github.com/smart-hub/core/issues/82231
