...
from smarthub.components.device_automation import action as device_action
...
async def async_validate_actions_config(
    hass: SmartHub, actions: list[ConfigType]
) -> list[ConfigType]:
...
class _ScriptRun:
    ...
    def _handle_exception(
        ...
        # Only SmartHub errors can be ignored.
        if not isinstance(exception, exceptions.SmartHubError):
            raise exception
    ...
    def _log_exception(self, exception: Exception) -> None:
        ...
        elif isinstance(exception, exceptions.SmartHubError):
            error_desc = "Error"
    ...
    def _test_conditions(
        ...
        def traced_test_conditions(
            hass: SmartHub, variables: TemplateVarsType
        ) -> bool | None:
            try:
...
@callback
def _schedule_stop_scripts_after_shutdown(hass: SmartHub) -> None:
...
async def _async_stop_scripts_after_shutdown(
    hass: SmartHub, point_in_time: datetime
) -> None:
    """Stop running Script objects started after shutdown."""
...
async def _async_stop_scripts_at_shutdown(hass: SmartHub, event: Event) -> None:
...
class Script:
    ...
    async def async_run(
        ...
        # Prevent spawning new script runs when SmartHub is shutting down
        if DATA_NEW_SCRIPT_RUNS_NOT_ALLOWED in self._hass.data:
            self._log("SmartHub is shutting down, starting script blocked")
            return None
...
def breakpoint_clear(
    hass: SmartHub, key: str, run_id: str | None, node: str
) -> None:
    """Clear a breakpoint."""
...
@callback
def breakpoint_clear_all(hass: SmartHub) -> None:
...
@callback
def breakpoint_list(hass: SmartHub) -> list[dict[str, Any]]:
    """List breakpoints."""
    breakpoints = hass.data[DATA_SCRIPT_BREAKPOINTS]
...
@callback
def debug_continue(hass: SmartHub, key: str, run_id: str) -> None:
...
@callback
def debug_step(hass: SmartHub, key: str, run_id: str) -> None:
...
@callback
def debug_stop(hass: SmartHub, key: str, run_id: str) -> None:
