...
from smarthub.generated import currencies
from smarthub.generated.countries import COUNTRIES
from smarthub.generated.languages import LANGUAGES
from smarthub.util import (
    dt as dt_util,
    raise_if_invalid_path,
    slugify as util_slugify,
)
from smarthub.util.yaml.objects import NodeStrClass
...
class MustValidateInExecutor(SmartHubError):
    """Raised when validation must happen in an executor thread."""


class _Hass(threading.local):
    """Container which makes a SmartHub instance available to validators."""
...
def _async_get_hass_or_none() -> SmartHub | None:
...
def configuration_url(value: Any) -> str:
    """Validate an URL that allows the smarthub schema."""
...
def _custom_serializer(schema: Any, *, allow_section: bool) -> Any:
    """Serialize additional types for voluptuous_serialize."""
    from smarthub import data_entry_flow  # noqa: PLC0415
...
def _no_yaml_config_schema(
    ...
    def raise_issue() -> None:
        ...
        # SmartHubError is raised if called from the wrong thread
        with contextlib.suppress(SmartHubError):
            hass = async_get_hass()
            async_create_issue(
                hass,
...
async def async_validate(
    hass: SmartHub, validator: Callable[[Any], Any], value: Any
) -> Any:
