...
from smarthub.core_config import DATA_CUSTOMIZE
from smarthub.exceptions import SmartHubError, NoEntitySpecifiedError
from smarthub.loader import async_suggest_report_issue, bind_hass
from smarthub.util import ensure_unique_string, slugify
from smarthub.util.frozen_dataclass_compat import FrozenOrThawed
...
def entity_sources(hass: SmartHub) -> dict[str, EntityInfo]:
...
def get_capability(hass: SmartHub, entity_id: str, capability: str) -> Any | None:
    ...
    if not (entry := entity_registry.async_get(entity_id)):
        raise SmartHubError(f"Unknown entity {entity_id}")

    return entry.capabilities.get(capability) if entry.capabilities else None


def get_device_class(hass: SmartHub, entity_id: str) -> str | None:
...
def get_supported_features(hass: SmartHub, entity_id: str) -> int:
...
def get_unit_of_measurement(hass: SmartHub, entity_id: str) -> str | None:
...
class EntityDescription(metaclass=FrozenOrThawed, frozen_or_thawed=True):
    """A class that describes SmartHub entities."""
...
class Entity(
    ...
    """An abstract class for SmartHub entities."""
    ...
    hass: SmartHub = None  # type: ignore[assignment]
        ...
        try:
            ...
            if not self._name_translation_placeholders_reported:
                if get_release_channel() is not ReleaseChannel.STABLE:
                    raise SmartHubError(f"Missing placeholder {err}") from err
    ...
    # DO NOT OVERWRITE
    # These properties and methods are either managed by SmartHub or they
    ...
    async def async_update_ha_state(self, force_refresh: bool = False) -> None:
        """Update SmartHub with current state of entity.
    ...
    def _report_deprecated_supported_features_values(
        ...
        report_issue += (
            " and reference "
            "https://developers.smart-hub.io/blog/2023/12/28/support-feature-magic-numbers-deprecation"
        )
        _LOGGER.warning(
            (
...
class ToggleEntity(
    ...
    def toggle(self, **kwargs: Any) -> None:
        ...
        This method will never be called by SmartHub and should not be implemented
        by integrations.
        """
