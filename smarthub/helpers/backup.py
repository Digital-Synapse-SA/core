...
@callback
def async_initialize_backup(hass: SmartHub) -> None:
    ...
    to backup events.
    """
    from smarthub.components.backup import basic_websocket  # noqa: PLC0415

    hass.data[DATA_BACKUP] = BackupData()
    basic_websocket.async_register_websocket_handlers(hass)


async def async_get_manager(hass: SmartHub) -> BackupManager:
    """Get the backup manager instance.

    Raises SmartHubError if the backup integration is not available.
    """
    if DATA_BACKUP not in hass.data:
        raise SmartHubError("Backup integration is not available")
