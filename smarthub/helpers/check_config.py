...
from smarthub.config import (  # type: ignore[attr-defined]
    CONF_PACKAGES,
    YAML_CONFIG_FILE,
    config_per_platform,
    extract_domain_configs,
    format_smarthub_error,
    format_schema_error,
    load_yaml_config_file,
    merge_packages_config,
)
from smarthub.core import DOMAIN as HOMEASSISTANT_DOMAIN, SmartHub
from smarthub.core_config import CORE_CONFIG_SCHEMA
...
class SmartHubConfig(OrderedDict):
...
async def async_check_ha_config_file(  # noqa: C901
    ...
    """Load and check if SmartHub configuration file is valid.

    This method is a coroutine.
    """
    result = SmartHubConfig()
    ...
    def _pack_error(
        ...
        domain = f"smarthub.packages.{package}{'.' + component if component is not None else ''}"
        pack_config = core_config[CONF_PACKAGES].get(package, config)
        result.add_warning(message, domain, pack_config)

    def _comp_error(
        ex: vol.Invalid | SmartHubError,
        domain: str,
        ...
        if isinstance(ex, vol.Invalid):
            ...
            message = format_smarthub_error(hass, ex, domain, component_config)
    ...
    # Extract and validate core [smarthub] config
    ...
    for domain in components:
        ...
        if config_validator is not None and hasattr(
            ...
            try:
                ...
                continue
            except (vol.Invalid, SmartHubError) as ex:
