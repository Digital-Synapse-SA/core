...
from smarthub.data_entry_flow import UnknownHandler
...
class SchemaConfigFlowHandler(ConfigFlow, ABC):
    ...
    def async_options_flow_finished(
        hass: SmartHub, options: Mapping[str, Any]
    ) -> None:
...
class SchemaOptionsFlowHandler(OptionsFlow):
    ...
    def __init__(
        ...
        async_options_flow_finished: Callable[[SmartHub, Mapping[str, Any]], None]
        | None = None,
        async_setup_preview: Callable[[SmartHub], Coroutine[Any, Any, None]]
        | None = None,
    ) -> None:
...
def wrapped_entity_config_entry_title(
    hass: SmartHub, entity_id_or_uuid: str
) -> str:
