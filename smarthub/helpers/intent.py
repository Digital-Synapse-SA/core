...
from smarthub.components.smarthub.exposed_entities import async_should_expose
from smarthub.const import (
    ATTR_DEVICE_CLASS,
    ATTR_ENTITY_ID,
    ATTR_SUPPORTED_FEATURES,
)
from smarthub.core import Context, SmartHub, State, callback
...
@callback
@bind_hass
def async_register(hass: SmartHub, handler: IntentHandler) -> None:
    """Register an intent with SmartHub."""
...
@callback
@bind_hass
def async_remove(hass: SmartHub, intent_type: str) -> None:
    """Remove an intent from SmartHub."""
    if (intents := hass.data.get(DATA_KEY)) is None:
        return
...
@callback
def async_get(hass: SmartHub) -> Iterable[IntentHandler]:
...
class IntentError(SmartHubError):
