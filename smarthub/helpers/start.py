...
def _async_at_core_state(
    hass: SmartHub,
    at_start_cb: Callable[[SmartHub], Coroutine[Any, Any, None] | None],
    event_type: EventType[NoEventData],
    check_state: Callable[[SmartHub], bool],
) -> CALLBACK_TYPE:
    """Execute a job at_start_cb when SmartHub has the wanted state.

    The job is executed immediately if SmartHub is in the wanted state.
    ...
    def _matched_event(event: Event) -> None:
        """Call the callback when SmartHub started."""
        hass.async_run_hass_job(at_start_job, hass)
        nonlocal unsub
        unsub = None
...
def async_at_start(
    ...
    """Execute a job at_start_cb when SmartHub is starting.

    The job is executed immediately if SmartHub is already starting or started.
    Will wait for EVENT_HOMEASSISTANT_START if it isn't.
    """

    def _is_running(hass: SmartHub) -> bool:
...
def async_at_started(
    ...
    """Execute a job at_start_cb when SmartHub has started.

    The job is executed immediately if SmartHub is already started.
    Will wait for EVENT_HOMEASSISTANT_STARTED if it isn't.
    """

    def _is_started(hass: SmartHub) -> bool:
