"""Template helper methods for rendering strings with SmartHub data."""
...
def _template_state_no_collect(hass: SmartHub, state: State) -> TemplateState:
...
def _template_state(hass: SmartHub, state: State) -> TemplateState:
...
def async_setup(hass: SmartHub) -> bool:
...
def attach(hass: SmartHub, obj: Any) -> None:
    """Recursively attach hass to all template instances in list and dict."""
    return _attach(hass, obj)


def _attach(hass: SmartHub, obj: Any) -> None:
    """Recursively attach hass to all template instances in list and dict."""
...
class Template:
    ...
    def __init__(self, template: str, hass: SmartHub | None = None) -> None:
        """Instantiate a template.

        Note: A valid hass instance should always be passed in. The hass parameter
        will be non optional in SmartHub Core 2025.10.
        """
...
@cache
def _domain_states(hass: SmartHub, name: str) -> DomainStates:
...
class DomainStates:
    ...
    def __init__(self, hass: SmartHub, domain: str) -> None:
...
class TemplateStateBase(State):
    ...
    def __init__(self, hass: SmartHub, collect: bool, entity_id: str) -> None:
    ...
    def format_state(self, rounded: bool, with_unit: bool) -> str:
        ...
        from smarthub.components.sensor import (  # noqa: PLC0415
...
class TemplateState(TemplateStateBase):
    ...
    # Inheritance is done so functions that check against State keep working
    def __init__(self, hass: SmartHub, state: State, collect: bool = True) -> None:
        """Initialize template state."""
...
class TemplateStateFromEntityId(TemplateStateBase):
    ...
    def __init__(
        self, hass: SmartHub, entity_id: str, collect: bool = True
    ) -> None:
        """Initialize template state."""
...
def _get_state_if_valid(hass: SmartHub, entity_id: str) -> TemplateState | None:
...
def _get_state(hass: SmartHub, entity_id: str) -> TemplateState | None:
    return _get_template_state_from_state(hass, entity_id, hass.states.get(entity_id))


def _get_template_state_from_state(
    hass: SmartHub, entity_id: str, state: State | None
) -> TemplateState | None:
    if state is None:
...
def _resolve_state(
    hass: SmartHub, entity_id_or_state: Any
...
def expand(hass: SmartHub, *args: Any) -> Iterable[State]:
...
def device_entities(hass: SmartHub, _device_id: str) -> Iterable[str]:
...
def integration_entities(hass: SmartHub, entry_name: str) -> Iterable[str]:
...
def config_entry_id(hass: SmartHub, entity_id: str) -> str | None:
...
def device_id(hass: SmartHub, entity_id_or_device_name: str) -> str | None:
...
def device_name(hass: SmartHub, lookup_value: str) -> str | None:
...
def device_attr(hass: SmartHub, device_or_entity_id: str, attr_name: str) -> Any:
...
def config_entry_attr(
    hass: SmartHub, config_entry_id_: str, attr_name: str
) -> Any:
...
def is_device_attr(
    hass: SmartHub, device_or_entity_id: str, attr_name: str, attr_value: Any
) -> bool:
    """Test if a device's attribute is a specific value."""
    return bool(device_attr(hass, device_or_entity_id, attr_name) == attr_value)


def issues(hass: SmartHub) -> dict[tuple[str, str], dict[str, Any]]:
...
def issue(hass: SmartHub, domain: str, issue_id: str) -> dict[str, Any] | None:
...
def floors(hass: SmartHub) -> Iterable[str | None]:
    """Return all floors."""
    floor_registry = fr.async_get(hass)
    return [floor.floor_id for floor in floor_registry.async_list_floors()]


def floor_id(hass: SmartHub, lookup_value: Any) -> str | None:
...
def floor_name(hass: SmartHub, lookup_value: str) -> str | None:
...
def floor_areas(hass: SmartHub, floor_id_or_name: str) -> Iterable[str]:
...
def floor_entities(hass: SmartHub, floor_id_or_name: str) -> Iterable[str]:
...
def areas(hass: SmartHub) -> Iterable[str | None]:
    """Return all areas."""
    return list(area_registry.async_get(hass).areas)


def area_id(hass: SmartHub, lookup_value: str) -> str | None:
...
def area_name(hass: SmartHub, lookup_value: str) -> str | None:
...
def area_entities(hass: SmartHub, area_id_or_name: str) -> Iterable[str]:
...
def area_devices(hass: SmartHub, area_id_or_name: str) -> Iterable[str]:
...
def labels(hass: SmartHub, lookup_value: Any = None) -> Iterable[str | None]:
...
def label_id(hass: SmartHub, lookup_value: Any) -> str | None:
...
def label_name(hass: SmartHub, lookup_value: str) -> str | None:
...
def _label_id_or_name(hass: SmartHub, label_id_or_name: str) -> str | None:
...
def label_areas(hass: SmartHub, label_id_or_name: str) -> Iterable[str]:
...
def label_devices(hass: SmartHub, label_id_or_name: str) -> Iterable[str]:
...
def label_entities(hass: SmartHub, label_id_or_name: str) -> Iterable[str]:
...
def closest(hass: SmartHub, *args: Any) -> State | None:
    """Find closest entity.
...
def closest_filter(hass: SmartHub, *args: Any) -> State | None:
...
def distance(hass: SmartHub, *args: Any) -> float | None:
    """Calculate distance.
...
def is_hidden_entity(hass: SmartHub, entity_id: str) -> bool:
...
def is_state(hass: SmartHub, entity_id: str, state: str | list[str]) -> bool:
...
def is_state_attr(hass: SmartHub, entity_id: str, name: str, value: Any) -> bool:
...
def state_attr(hass: SmartHub, entity_id: str, name: str) -> Any:
...
def has_value(hass: SmartHub, entity_id: str) -> bool:
...
def now(hass: SmartHub) -> datetime:
    """Record fetching now."""
    if (render_info := _render_info.get()) is not None:
...
def utcnow(hass: SmartHub) -> datetime:
...
def today_at(hass: SmartHub, time_str: str = "") -> datetime:
...
def relative_time(hass: SmartHub, value: Any) -> Any:
...
def time_since(hass: SmartHub, value: Any | datetime, precision: int = 1) -> Any:
    """Take a datetime and return its "age" as a string.
...
def time_until(hass: SmartHub, value: Any | datetime, precision: int = 1) -> Any:
...
async def async_load_custom_templates(hass: SmartHub) -> None:
...
def _load_custom_templates(hass: SmartHub) -> dict[str, str]:
    result = {}
...
def _get_hass_loader(hass: SmartHub) -> HassLoader:
    return HassLoader({})
...
class TemplateEnvironment(ImmutableSandboxedEnvironment):
    """The SmartHub template environment."""
    ...
    def __init__(
        ...
        def hassfunction[**_P, _R](
            func: Callable[Concatenate[SmartHub, _P], _R],
