...
class _Hass:
    """Container which makes a SmartHub instance available to frame helper."""
...
def get_integration_logger(fallback_name: str) -> logging.Logger:
    ...
    https://github.com/smart-hub/core/issues/24982
    """
    try:
    ...
    if integration_frame.custom_integration:
        ...
        logger_name = f"smarthub.components.{integration_frame.integration}"
...
def get_integration_frame(exclude_integrations: set | None = None) -> IntegrationFrame:
    ...
    while frame is not None:
        ...
        for path in ("custom_components/", "smarthub/components/"):
            try:
...
class MissingIntegrationFrame(SmartHubError):
...
def _report_usage_integration_domain(
    ...
    _LOGGER.log(
        ...
        integration.domain,
        what,
        f"This will stop working in SmartHub {breaks_in_ha_version}, please"
...
def report_non_thread_safe_operation(what: str) -> None:
    ...
    report_usage(
        ...
        "which may cause SmartHub to crash or data to corrupt. "
        "For more information, see "
        "https://developers.smart-hub.io/docs/asyncio_thread_safety/"
        f"#{what.replace('.', '')}",
