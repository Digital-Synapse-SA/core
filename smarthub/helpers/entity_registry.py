...
from smarthub.loader import async_suggest_report_issue
from smarthub.util import slugify, uuid as uuid_util
...
class RegistryEntry:
    ...
    @callback
    def write_unavailable_state(self, hass: SmartHub) -> None:
...
def async_get(hass: SmartHub) -> EntityRegistry:
    """Get entity registry."""
    return EntityRegistry(hass)


async def async_load(hass: SmartHub) -> None:
...
@callback
def _async_setup_cleanup(hass: SmartHub, registry: EntityRegistry) -> None:
    ...
    @callback
    def _on_smarthub_stop(event: Event) -> None:
        """Cancel cleanup."""
        cancel()

    hass.bus.async_listen_once(EVENT_HOMEASSISTANT_STOP, _on_smarthub_stop)


@callback
def _async_setup_entity_restore(hass: SmartHub, registry: EntityRegistry) -> None:
