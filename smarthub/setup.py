"""All methods needed to bootstrap a SmartHub instance."""
...
from .exceptions import DependencyError, SmartHubError
...
def async_notify_setup_error(
    hass: SmartHub, component: str, display_link: str | None = None
) -> None:
...
@core.callback
def async_set_domains_to_be_loaded(hass: core.SmartHub, domains: set[str]) -> None:
...
def setup_component(hass: core.SmartHub, domain: str, config: ConfigType) -> bool:
...
async def async_setup_component(
    hass: core.SmartHub, domain: str, config: ConfigType
) -> bool:
    """Set up a component and all its dependencies.
    ...
    try:
        ...
        for future in futures:
            ...
            # SmartHub is shutting down so the future might
...
async def _async_process_dependencies(
    hass: core.SmartHub, config: ConfigType, integration: loader.Integration
) -> list[str]:
...
async def _async_setup_component(
    ...
    """Set up a component for SmartHub.
...
async def async_prepare_setup_platform(
    hass: core.SmartHub, hass_config: ConfigType, domain: str, platform_name: str
) -> ModuleType | None:
...
def async_when_setup(
    hass: core.SmartHub,
    component: str,
    when_setup_cb: Callable[[core.SmartHub, str], Awaitable[None]],
) -> None:
...
@core.callback
def async_get_loaded_integrations(hass: core.SmartHub) -> set[str]:
...
def async_pause_setup(hass: core.SmartHub, phase: SetupPhases) -> Generator[None]:
...
@callback
def async_get_setup_timings(hass: core.SmartHub) -> dict[str, float]:
...
def async_get_domain_setup_times(
    hass: core.SmartHub, domain: str
) -> Mapping[str | None, dict[SetupPhases, float]]:
    """Return timing data for each integration."""
    return _setup_times(hass).get(domain, {})


async def async_wait_component(hass: SmartHub, domain: str) -> bool:
