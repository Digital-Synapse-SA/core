...
from .core import DOMAIN as HOMEASSISTANT_DOMAIN, SmartHub, callback
from .core_config import _PACKAGE_DEFINITION_SCHEMA, _PACKAGES_CONFIG_SCHEMA
from .exceptions import ConfigValidationError, SmartHubError
...
RE_YAML_ERROR = re.compile(r"smarthub\.util\.yaml")
RE_ASCII = re.compile(r"\033\[[^m]*m")
YAML_CONFIG_FILE = "configuration.yaml"
VERSION_FILE = ".HA_VERSION"
CONFIG_DIR_NAME = ".smarthub"
...
# Learn more at https://www.smart-hub.io/docs/configuration/secrets/
...
async def async_ensure_config_exists(hass: SmartHub) -> bool:
...
async def async_create_default_config(hass: SmartHub) -> bool:
...
async def async_hass_config_yaml(hass: SmartHub) -> dict:
    """Load YAML from a SmartHub configuration file.
...
def load_yaml_config_file(
    ...
    Raises FileNotFoundError or SmartHubError.
    ...
    try:
        ...
        _LOGGER.error(msg)
        raise SmartHubError(msg) from exc
...
def process_ha_config_upgrade(hass: SmartHub) -> None:
...
def async_log_config_validator_error(
    ...
    message = format_smarthub_error(hass, exc, domain, config, link)
...
def _relpath(hass: SmartHub, path: str) -> str:
    """Return path relative to the SmartHub config dir."""
...
def format_smarthub_error(
    ...
    """Format SmartHubError thrown by a custom config validator."""
    if "." in domain:
        integration_domain, _, platform_domain = domain.partition(".")
        message_prefix = (
            f"Invalid config for '{platform_domain}' from integration "
        ...
        message_prefix = f"Invalid config for '{domain}'"
    # SmartHubError raised by custom config validator has no path to the
...
def _log_pkg_error(
    hass: SmartHub, package: str, component: str | None, config: dict, message: str
) -> None:
...
async def merge_packages_config(
    ...
    _log_pkg_error: Callable[
        [SmartHub, str, str | None, dict, str], None
    ] = _log_pkg_error,
) -> dict:
...
def _get_log_message_and_stack_print_pref(
    hass: SmartHub, domain: str, platform_exception: ConfigExceptionInfo
    ...
    if show_stack_trace is None:
        ...
        if isinstance(exception, vol.Invalid):
            ...
            if TYPE_CHECKING:
                assert isinstance(exception, SmartHubError)
            log_message = format_smarthub_error(
                hass, exception, platform_path, platform_config, link
            )
...
def async_drop_config_annotations(
    ...
    # Don't drop annotations from the smarthub integration because it may
...
async def async_process_component_config(
    ...
    if config_validator is not None and hasattr(
        ...
        except (vol.Invalid, SmartHubError) as exc:
            exc_info = ConfigExceptionInfo(
                exc,
...
async def async_check_ha_config_file(hass: SmartHub) -> str | None:
    """Check if SmartHub configuration file is valid.
...
async def async_enable_safe_mode(hass: SmartHub) -> None:
    """Enable safe mode."""
