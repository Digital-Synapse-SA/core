"""The methods for loading SmartHub integrations.
...
from .core import SmartHub, callback
...
BLOCKED_CUSTOM_INTEGRATIONS: dict[str, BlockedIntegration] = {
    # Added in 2024.3.0 because of https://github.com/smart-hub/core/issues/112464
    "start_time": BlockedIntegration(AwesomeVersion("1.1.7"), "breaks SmartHub"),
    # Added in 2024.5.1 because of
    # https://community.smart-hub.io/t/psa-2024-5-upgrade-failure-and-dreame-vacuum-custom-integration/724612
    "dreame_vacuum": BlockedIntegration(
        AwesomeVersion("1.0.4"), "crashes SmartHub"
    ),
    # Added in 2024.5.5 because of
    # https://github.com/sh00t2kill/dolphin-robot/issues/185
    "mydolphin_plus": BlockedIntegration(
        AwesomeVersion("1.0.13"), "crashes SmartHub"
    ),
...
PACKAGE_BUILTIN = "smarthub.components"
CUSTOM_WARNING = (
    "We found a custom integration %s which has not "
    "been tested by SmartHub. This component might "
    "cause stability problems, be sure to disable it if you "
    "experience issues with SmartHub"
)
...
def async_setup(hass: SmartHub) -> None:
...
def _get_custom_components(hass: SmartHub) -> dict[str, Integration]:
...
class ComponentProtocol(Protocol):
    ...
    async def async_setup_entry(
        self, hass: SmartHub, config_entry: ConfigEntry
    ) -> bool:
    ...
    async def async_reset_platform(
        self, hass: SmartHub, integration_name: str
    ) -> None:
        """Release resources."""

    async def async_setup(self, hass: SmartHub, config: ConfigType) -> bool:
        """Set up integration."""

    def setup(self, hass: SmartHub, config: ConfigType) -> bool:
        """Set up integration."""
...
async def async_get_application_credentials(hass: SmartHub) -> list[str]:
...
async def async_get_bluetooth(hass: SmartHub) -> list[BluetoothMatcher]:
...
async def async_get_dhcp(hass: SmartHub) -> list[DHCPMatcher]:
...
async def async_get_usb(hass: SmartHub) -> list[USBMatcher]:
...
async def async_get_ssdp(hass: SmartHub) -> dict[str, list[dict[str, str]]]:
...
async def async_get_mqtt(hass: SmartHub) -> dict[str, list[str]]:
...
@callback
def async_register_preload_platform(hass: SmartHub, platform_name: str) -> None:
...
class Integration:
    """An integration in SmartHub."""

    @classmethod
    def resolve_from_root(
        cls, hass: SmartHub, root_module: ModuleType, domain: str
    ) -> Integration | None:
...
def _resolve_integrations_from_root(
    hass: SmartHub, root_module: ModuleType, domains: Iterable[str]
) -> dict[str, Integration]:
...
@callback
def async_get_loaded_integration(hass: SmartHub, domain: str) -> Integration:
...
async def async_get_integration(hass: SmartHub, domain: str) -> Integration:
    """Get integration."""
    cache = hass.data[DATA_INTEGRATIONS]
...
async def resolve_integrations_dependencies(
    hass: SmartHub, integrations: Iterable[Integration]
) -> dict[str, set[str]]:
...
def _load_file(
    hass: SmartHub, comp_or_platform: str, base_paths: list[str]
...
class ModuleWrapper:
    ...
    def __init__(self, hass: SmartHub, module: ComponentProtocol) -> None:
...
def _async_mount_config_dir(hass: SmartHub) -> None:
...
def _lookup_path(hass: SmartHub) -> list[str]:
...
def is_component_module_loaded(hass: SmartHub, module: str) -> bool:
