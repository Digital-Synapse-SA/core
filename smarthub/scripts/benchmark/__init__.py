...
from smarthub.helpers.entityfilter import convert_include_exclude_filter
from smarthub.helpers.event import (
    async_track_state_change,
    async_track_state_change_event,
)
from smarthub.helpers.json import JSON_DUMP
...
def run(args):
    ...
    # Disable logging
    logging.getLogger("smarthub.core").setLevel(logging.CRITICAL)

    parser = argparse.ArgumentParser(description="Run a SmartHub benchmark.")
...
@benchmark
async def fire_events(hass: core.SmartHub) -> float:
...
@benchmark
async def fire_events_with_filter(hass: core.SmartHub) -> float:
...
@benchmark
async def state_changed_helper(hass: core.SmartHub) -> float:
...
@benchmark
async def state_changed_event_helper(hass: core.SmartHub) -> float:
...
@benchmark
async def state_changed_event_filter_helper(hass: core.SmartHub) -> float:
...
@benchmark
async def filtering_entity_id(hass: core.SmartHub) -> float:
...
@benchmark
async def valid_entity_id(hass: core.SmartHub) -> float:
...
@benchmark
async def json_serialize_states(hass: core.SmartHub) -> float:
