"""SmartHub module to handle restoring backups."""
...
def restore_backup_file_content(config_dir: Path) -> RestoreBackupFileContent | None:
    ...
    try:
        ...
        return RestoreBackupFileContent(
            ...
            restore_smarthub=instruction_content["restore_smarthub"],
        )
    ...
    with (
        ...
        if (
            backup_meta_version := AwesomeVersion(
                backup_meta["smarthub"]["version"]
            )
        ) > HA_VERSION:
            raise ValueError(
                f"You need at least SmartHub version {backup_meta_version} to restore this backup"
            )
        ...
        with securetar.SecureTarFile(
            Path(
                tempdir,
                "extracted",
                f"smarthub.tar{'.gz' if backup_meta['compressed'] else ''}",
            ),
            ...
            if restore_content.restore_smarthub:
                ...
                for entry in KEEP_DATABASE:
                    shutil.copy(
                        Path(tempdir, "smarthub", "data", entry),
                        config_dir,
                    )
