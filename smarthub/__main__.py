...
FAULT_LOG_FILENAME = "smart-hub.log.fault"


def validate_os() -> None:
    """Validate that SmartHub is running in a supported operating system."""
    if not sys.platform.startswith(("darwin", "linux")):
        print(
            "SmartHub only supports Linux, OSX and Windows using WSL",
            file=sys.stderr,
        )
        sys.exit(1)
...
def validate_python() -> None:
    ...
    if sys.version_info[:3] < REQUIRED_PYTHON_VER:
        print(
            "SmartHub requires at least Python "
...
def get_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="SmartHub: Observe, Control, Automate.",
    ...
    parser.add_argument(
        "--recovery-mode",
        action="store_true",
        help="Start SmartHub in recovery mode",
    )
    parser.add_argument(
        "--debug", action="store_true", help="Start SmartHub in debug mode"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Log file to write to.  If not set, CONFIG/smart-hub.log is used",
    )
    parser.add_argument(
