"""Manage config entries in SmartHub."""
...
class ConfigError(SmartHubError):
...
class ConfigEntry[_DataT = Any]:
    ...
    @callback
    def _async_setup_again(self, hass: SmartHub, *_: Any) -> None:
    ...
    async def async_setup_locked(
        self, hass: SmartHub, integration: loader.Integration | None = None
    ) -> None:
    ...
    def async_shutdown(self) -> None:
        """Call when SmartHub is stopping."""
        self.async_cancel_retry_setup()
    ...
    async def async_remove(self, hass: SmartHub) -> None:
    ...
    async def async_migrate(self, hass: SmartHub) -> bool:
        """Migrate an entry.
        ...
        # https://github.com/smart-hub/core/pull/67087#discussion_r812559950
    ...
    async def _async_process_on_unload(self, hass: SmartHub) -> None:
    ...
    def async_get_active_flows(
        self, hass: SmartHub, sources: set[str]
...
class ConfigEntriesFlowManager(
        ...
        *,
        ...
        try:
            async with interrupt(
                cancel_init_future,
                FlowCancelledError,
                "Config entry initialize canceled: SmartHub is shutting down",
            ):
    ...
    async def async_finish_flow(
        ...
        if (
            ...
            # see https://developers.smart-hub.io/blog/2025/03/01/config-flow-unique-id/
            report_usage(
...
class ConfigEntryItems(UserDict[str, ConfigEntry]):
    ...
    def __setitem__(self, entry_id: str, entry: ConfigEntry) -> None:
        ...
        if entry_id in data:
            ...
            # In the future, once we have fixed the tests, this will raise SmartHubError.
...
class ConfigEntries:
    ...
    def __init__(self, hass: SmartHub, hass_config: ConfigType) -> None:
    ...
    async def async_wait_component(self, entry: ConfigEntry) -> bool:
        ...
        Config entries which are created after SmartHub is started can't be waited
...
class ConfigSubentryFlowManager(
    ...
    async def async_finish_flow(
        ...
        if unique_id is not None and not isinstance(unique_id, str):
            raise SmartHubError("unique_id must be a string")
...
async def support_entry_unload(hass: SmartHub, domain: str) -> bool:
...
async def support_remove_from_device(hass: SmartHub, domain: str) -> bool:
...
async def _support_single_config_entry_only(hass: SmartHub, domain: str) -> bool:
...
async def _load_integration(
    hass: SmartHub, domain: str, hass_config: ConfigType
) -> None:
    try:
        integration = await loader.async_get_integration(hass, domain)
...
@callback
def _abort_reauth_flows(hass: SmartHub, domain: str, entry_id: str) -> None:
